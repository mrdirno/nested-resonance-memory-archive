...
Purpose:
  Test six-level food chain (senary consumer).

Research Question:
  Can six trophic levels coexist? Where is the limit?

Background:
  C315: Five-trophic stable (300-30-10-5-3)
  C316: Extend to six levels

Design:
  - 6 populations
  - Compare five- vs six-trophic
...
CYCLE_ID = "C316"
CYCLE_NAME = "Six-Trophic Chain"
MODE = "SIX_TROPHIC"

CYCLES = 30000
INITIAL = [300, 30, 10, 5, 3, 2]  # prey, meso, top, apex, hyper, ultra
...
PARAMS = [
    ...
    {"attack": 0.015, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.12}   # L5 ultra
]

CONDITIONS = [
    {"name": "five_trophic", "n_levels": 5},
...
def run_experiment(condition: dict, seed: int, cycles: int) -> dict:
    """Run six-trophic experiment."""

    label = condition["name"]
    n_levels = condition["n_levels"]

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c316_{label}_seed{seed}.db"
    ...
    # Coexistence: all levels > 1 (very lenient for top)
    coexist = all(finals[i] >= (1 if i >= 4 else 3) for i in range(n_levels))
...
def main():
    print("=" * 80)
    print(f"CYCLE 316: SIX-TROPHIC CHAIN")
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c316_six_trophic_results.json"
