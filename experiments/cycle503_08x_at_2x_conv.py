#!/usr/bin/env python3
"""
CYCLE 503: 0.8× ATTACK AT 2.0× CONVERSION
...
Purpose:
  Extend attack profile to lower values.
"""
...
SEEDS = list(range(8040, 8060))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c503_seed{seed}.db", n_populations=n_levels, mode="08X_2XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 503: 0.8× + 2.0× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c503_08x_at_2x_conv_results.json", 'w') as f:
