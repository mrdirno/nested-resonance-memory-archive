#!/usr/bin/env python3
"""
CYCLE 416: RECOVERY POINT AT 2.0625× CONVERSION
...
Purpose:
  Locate exact critical conversion threshold.

Background:
  At 2.0× conversion: 0.71875× = 70%
  At 2.125× conversion: 0.71875× = 95%

  C416: 0.71875× + 2.0625× - find exact threshold

Design:
  - K declines from 600 to 200 over 40 cycles
  - Attack: 0.71875×, Conversion: 2.0625×
  - 20 seeds
"""
...
CYCLE_ID = "C416"
CYCLE_NAME = "Recovery at 2.0625x Conv"
MODE = "RECOVERY_20625XCONV"
...
# 0.71875× attack + 2.0625× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.00215625, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.61875},
    {"attack": 0.00359375, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.515625},
    {"attack": 0.00575, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.4125},
    {"attack": 0.008625, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.309375},
    {"attack": 0.01078125, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.2475},
    {"attack": 0.0129375, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.20625}
]

SEEDS = list(range(6300, 6320))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run recovery point at 2.0625× conversion experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c416_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 416: RECOVERY POINT AT 2.0625× CONVERSION")
    ...
    print(f"Attack: 0.71875×, Conversion: 2.0625×")
    print("=" * 80)
    print()
    ...
    output = {
        ...
        "conversion_multiplier": 2.0625,
        "n_seeds": len(SEEDS),
        "coexistence_rate": coexist_count / len(results) if results else 0,
        "results": results
    }

    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c416_recovery_at_20625x_conv_results.json"
