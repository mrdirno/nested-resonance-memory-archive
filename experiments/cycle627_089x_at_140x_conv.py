#!/usr/bin/env python3
"""
CYCLE 627: 0.89× ATTACK AT 1.4× CONVERSION
...
SEEDS = list(range(10521, 10541))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c627_seed{seed}.db", n_populations=n_levels, mode="089X_140XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 627: 0.89× + 1.4× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c627_089x_at_140x_conv_results.json", 'w') as f:
