#!/usr/bin/env python3
"""
CYCLE 451: 0.94× ATTACK AT 2.0× CONVERSION
...
Purpose:
  Complete high-resolution profile of 0.95× critical transition.
"""
...
# 0.94× attack + 2.0× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.00282, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.6},
    {"attack": 0.0047, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.5},
    {"attack": 0.00752, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.4},
    {"attack": 0.01128, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.3},
    {"attack": 0.0141, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.24},
    {"attack": 0.01692, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.2}
]
...
def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c451_seed{seed}.db", n_populations=n_levels, mode="094X_2XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 451: 0.94× + 2.0× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c451_094x_at_2x_conv_results.json", 'w') as f:
        json.dump({"cycle_id": CYCLE_ID, "attack_multiplier": 0.94, "conversion_multiplier": 2.0, "coexistence_rate": coexist_count/len(results), "results": results}, f, indent=2)
