#!/usr/bin/env python3
"""
CYCLE 409: DIP ONSET INVESTIGATION
...
Purpose:
  Find where the boundary dip begins.

Background:
  Complex pattern at 2.5× conversion:
  - 0.625×: 100%
  - 0.6875×: 90% (dip)
  - 0.71875×: 100% (recovery)

  C409: 0.65625× - where does dip start?

Design:
  - K declines from 600 to 200 over 40 cycles
  - Attack: 0.65625×, Conversion: 2.5×
  - 20 seeds
"""
...
CYCLE_ID = "C409"
CYCLE_NAME = "Dip Onset Investigation"
MODE = "DIP_ONSET"
...
# 0.65625× attack + 2.5× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.00196875, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.75},
    {"attack": 0.00328125, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.625},
    {"attack": 0.00525, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.5},
    {"attack": 0.007875, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.375},
    {"attack": 0.00984375, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.3},
    {"attack": 0.0118125, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.25}
]

SEEDS = list(range(6160, 6180))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run dip onset investigation experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c409_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 409: DIP ONSET INVESTIGATION")
    ...
    print(f"Attack: 0.65625×, Conversion: 2.5×")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c409_dip_onset_investigation_results.json"
