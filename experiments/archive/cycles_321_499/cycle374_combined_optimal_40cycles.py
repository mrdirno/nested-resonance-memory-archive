#!/usr/bin/env python3
"""
CYCLE 374: COMBINED OPTIMAL PARAMETERS - 40 CYCLES
...
Purpose:
  Test combined optimal parameter configuration.
...
Background:
  ...
  Combined should give >80% survival.

Design:
  - K declines from 600 to 200 over 40 cycles
  - Attack: 1.25×, Conversion: 1.5× (handling baseline)
  - 20 seeds
"""
...
CYCLE_ID = "C374"
CYCLE_NAME = "Combined Optimal 40 Cycles"
MODE = "COMBINED_OPTIMAL_40"
...
# Combined optimal: 1.25× attack, 1.5× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.00375, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.45},
    {"attack": 0.00625, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.375},
    {"attack": 0.01, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.3},
    {"attack": 0.015, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.225},
    {"attack": 0.01875, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.18},
    {"attack": 0.0225, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.15}
]

SEEDS = list(range(5460, 5480))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run combined optimal experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c374_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 374: COMBINED OPTIMAL PARAMETERS - 40 CYCLES")
    ...
    print(f"Attack: 1.25×, Conversion: 1.5×")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c374_combined_optimal_40cycles_results.json"
