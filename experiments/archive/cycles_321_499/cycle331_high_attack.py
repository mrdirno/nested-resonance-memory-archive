...
Purpose:
  Test if high attack (1.3×) performs WORSE than optimum.

Research Question:
  Does the paradigm inversion extend to high values?

Background:
  C328: 89% at 1.0×
  C329: 89% at 0.7×
  C330: 100% at 0.5× (BEST)
  C331: Should be <89% at 1.3×

Design:
  - 7 populations
  - Initial trait = 1.3 (high attack)
  - K=600, L6=N=2
  - 9 seeds
  - Total: 9 experiments
"""
...
CYCLE_ID = "C331"
CYCLE_NAME = "High Attack Test"
MODE = "HIGH_ATTACK"

CYCLES = 30000
K = 600
INITIAL = [300, 30, 10, 5, 3, 2, 2]
INITIAL_TRAIT = 1.3  # High attack
...
SEEDS = list(range(3800, 3809))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run high attack experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c331_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 331: HIGH ATTACK TEST")
    ...
    print(f"Starting trait: {INITIAL_TRAIT} (high attack)")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c331_high_attack_results.json"
