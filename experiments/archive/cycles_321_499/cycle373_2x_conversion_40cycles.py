#!/usr/bin/env python3
"""
CYCLE 373: 2× CONVERSION EFFICIENCY - 40 CYCLES
...
Purpose:
  Test if conversion efficiency continues improving beyond 1.5×.

Background:
  C372: 0.5× conversion - 0% survival
  C359: 1.0× conversion - 50% survival
  C371: 1.5× conversion - 80% survival
  C373: 2.0× conversion - expect ~90%+?

Design:
  - K declines from 600 to 200 over 40 cycles
  - Conversion efficiency: 2.0× baseline
  - 20 seeds
"""
...
CYCLE_ID = "C373"
CYCLE_NAME = "2x Conversion 40 Cycles"
MODE = "2X_CONV_40"
...
# 2.0× conversion efficiency
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.003, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.6},
    {"attack": 0.005, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.5},
    {"attack": 0.008, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.4},
    {"attack": 0.012, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.3},
    {"attack": 0.015, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.24},
    {"attack": 0.018, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.2}
]

SEEDS = list(range(5440, 5460))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run 2× conversion efficiency experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c373_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 373: 2× CONVERSION EFFICIENCY - 40 CYCLES")
    ...
    print(f"Conversion efficiency: 2.0× baseline")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c373_2x_conversion_40cycles_results.json"
