#!/usr/bin/env python3
"""
CYCLE 372: LOW CONVERSION EFFICIENCY - 40 CYCLES
...
Purpose:
  Test low conversion efficiency to confirm monotonic pattern.

Background:
  C359: 40 cycles, baseline - 50% survival
  C371: 40 cycles, 1.5× conversion - 80% survival
  C372: 40 cycles, 0.5× conversion - expect low survival

Design:
  - K declines from 600 to 200 over 40 cycles
  - Conversion efficiency: 0.5× baseline
  - 20 seeds
"""
...
CYCLE_ID = "C372"
CYCLE_NAME = "Low Conversion 40 Cycles"
MODE = "LOW_CONV_40"
...
BASE_PARAMS = [
    ...
    {"attack": 0.003, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.15},
    {"attack": 0.005, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.125},
    {"attack": 0.008, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.1},
    {"attack": 0.012, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.075},
    {"attack": 0.015, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.06},
    {"attack": 0.018, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.05}
]

SEEDS = list(range(5420, 5440))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run low conversion efficiency experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c372_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 372: LOW CONVERSION EFFICIENCY - 40 CYCLES")
    ...
    print(f"Conversion efficiency: 0.5× baseline")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c372_low_conversion_40cycles_results.json"
