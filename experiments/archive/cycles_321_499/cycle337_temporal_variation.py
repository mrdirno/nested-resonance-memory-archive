#!/usr/bin/env python3
"""
CYCLE 337: TEMPORAL ENVIRONMENTAL VARIATION
...
Purpose:
  Test food web response to temporal variation in carrying capacity.

Research Question:
  Can the seven-trophic system track environmental fluctuations?

Background:
  C316-C327: Stable food chain framework
  C328-C336: Evolutionary stasis
  C337: Environmental variation (seasonal dynamics)

Design:
  - K oscillates: K = K_mean + K_amplitude * sin(2π * cycle / period)
  - K_mean = 600, K_amplitude = 200
  - Period = 5000 cycles (~seasonal)
  - 20 seeds
...
CYCLE_ID = "C337"
CYCLE_NAME = "Temporal Environmental Variation"
MODE = "TEMPORAL_VAR"

CYCLES = 30000
K_MEAN = 600
K_AMPLITUDE = 200  # K varies from 400 to 800
PERIOD = 5000  # Cycles per oscillation
INITIAL = [300, 30, 10, 5, 3, 2, 2]

BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},  # K removed - will be dynamic
...
SEEDS = list(range(4400, 4420))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run temporal variation experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c337_seed{seed}.db"
    ...
    for cycle in range(cycles):
        ...
        K = K_MEAN + K_AMPLITUDE * np.sin(2 * np.pi * cycle / PERIOD)
    ...
    # Calculate variance (tracking ability)
    variances = {}
    for i in range(n_levels):
        if len(histories[i]) > 20:
            variances[i] = np.std(histories[i])
        else:
            variances[i] = 0
    ...
    return {
        ...
        "variances": {f"L{i}": float(variances[i]) for i in range(n_levels)},
...
def main():
    print("=" * 80)
    print(f"CYCLE 337: TEMPORAL ENVIRONMENTAL VARIATION")
    ...
    print(f"K oscillates: {K_MEAN} ± {K_AMPLITUDE} (period={PERIOD})")
    print("=" * 80)
    print()
    ...
    for exp_num, seed in enumerate(SEEDS, 1):
        ...
        try:
            ...
            var_str = ", ".join(f"V{i}={result['variances'][f'L{i}']:.1f}" for i in range(3))
            print(f"→ {status}, {var_str}")
    ...
    print("\nPopulation variance (tracking environmental variation):")
    for lvl in range(7):
        coexist_results = [r for r in results if r["coexist"]]
        if coexist_results:
            mean_var = np.mean([r["variances"][f"L{lvl}"] for r in coexist_results])
            print(f"  L{lvl}: σ = {mean_var:.1f}")
        else:
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c337_temporal_variation_results.json"
