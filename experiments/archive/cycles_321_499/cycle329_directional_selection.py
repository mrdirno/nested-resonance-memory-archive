#!/usr/bin/env python3
"""
CYCLE 329: DIRECTIONAL SELECTION TEST
...
Purpose:
  Test if traits evolve toward optimum when started suboptimally.

Research Question:
  Does directional selection drive traits to the optimal value?

Background:
  C328: Stabilizing selection at optimum
  C329: Start with low attack (0.7×) and test if evolves to 1.0×

Design:
  - 7 populations
  - Initial trait = 0.7 (suboptimal)
  - K=600, L6=N=2
  - 9 seeds
  - Total: 9 experiments
"""
...
CYCLE_ID = "C329"
CYCLE_NAME = "Directional Selection Test"
MODE = "DIRECTIONAL_SEL"

CYCLES = 30000
K = 600
INITIAL = [300, 30, 10, 5, 3, 2, 2]
INITIAL_TRAIT = 0.7  # Start suboptimal
...
SEEDS = list(range(3600, 3609))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run directional selection experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c329_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 329: DIRECTIONAL SELECTION TEST")
    ...
    print(f"Starting trait: {INITIAL_TRAIT} (suboptimal)")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c329_directional_results.json"
