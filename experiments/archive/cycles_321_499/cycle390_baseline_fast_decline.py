#!/usr/bin/env python3
"""
CYCLE 390: BASELINE CONFIGURATION - FAST DECLINE (20 CYCLES)
...
Purpose:
  Compare baseline with optimal at fast decline.

Background:
  C357: Baseline at 20 cycles - 25%
  C388: Optimal (0.5× atk + 2.0× conv) at 20 cycles - 85%
  C390: Confirm baseline at 20 cycles for direct comparison

Design:
  - K declines from 600 to 200 over 20 cycles (fast)
  - Attack: 1.0×, Conversion: 1.0× (baseline)
  - 20 seeds
"""
...
CYCLE_ID = "C390"
CYCLE_NAME = "Baseline Fast Decline"
MODE = "BASE_FAST_DECLINE"
...
BASE_PARAMS = [
    ...
    {"attack": 0.018, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.1}
]

SEEDS = list(range(5780, 5800))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run baseline config at fast decline."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c390_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 390: BASELINE CONFIGURATION - FAST DECLINE (20 CYCLES)")
    ...
    print(f"Attack: 1.0×, Conversion: 1.0× (baseline)")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c390_baseline_fast_decline_results.json"
