...
Purpose:
  Test ten-level food chain to extend stability pattern.

Research Question:
  Does the odd-even stability pattern continue at ten levels?

Background:
  C320: Nine-trophic 100% stable
  C321: Eight-trophic structurally unstable
  C322: Test if ten follows the pattern

Design:
  - 10 populations
  - K=2400
  - Compare nine- vs ten-trophic
...
INITIAL_TEN = [1200, 120, 40, 20, 12, 8, 4, 2, 1, 1]
...
PARAMS = [
    ...
    {"attack": 0.022, "h": 0.09, "e_con": 1.0, "e_gain": 1.8, "conv": 0.06},  # L8
    {"attack": 0.024, "h": 0.10, "e_con": 1.1, "e_gain": 2.0, "conv": 0.05}   # L9
]

CONDITIONS = [
    {"name": "nine_trophic", "n_levels": 9, "initial": INITIAL_NINE},
    {"name": "ten_trophic", "n_levels": 10, "initial": INITIAL_TEN}
]
SEEDS = [2900, 2901, 2902]


def run_experiment(condition: dict, seed: int, cycles: int) -> dict:
    """Run ten-trophic experiment."""

    label = condition["name"]
    n_levels = condition["n_levels"]
    initial = condition["initial"]

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c322_{label}_seed{seed}.db"
    ...
    for cycle in range(cycles):
        ...
        if cycle % 100 == 0:
            ...
            if cycle % 10000 == 0:
                pop_str = ", ".join(f"L{i}={histories[i][-1]}" for i in range(n_levels))
                print(f"      Cycle {cycle}: {pop_str}")
...
def main():
    print("=" * 80)
    print(f"CYCLE 322: TEN-TROPHIC CHAIN")
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c322_ten_trophic_results.json"
