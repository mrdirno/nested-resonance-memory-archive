#!/usr/bin/env python3
"""
CYCLE 408: BOUNDARY ANOMALY INVESTIGATION
...
Purpose:
  Investigate why 0.7× attack (95%) outperforms 0.6875× (90%).

Background:
  At 2.5× conversion, unexpected pattern:
  - C405: 0.625× attack - 100%
  - C406: 0.6875× attack - 90% (dip!)
  - C407: 0.7× attack - 95% (recovery!)
  - C400: 0.75× attack - 80%

  C408: 0.71875× attack (midpoint 0.6875-0.75) - expect 85-90%?

Design:
  - K declines from 600 to 200 over 40 cycles
  - Attack: 0.71875×, Conversion: 2.5×
  - 20 seeds
"""
...
CYCLE_ID = "C408"
CYCLE_NAME = "Boundary Anomaly Investigation"
MODE = "BOUND_ANOMALY"
...
# 0.71875× attack + 2.5× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.00215625, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.75},
    {"attack": 0.00359375, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.625},
    {"attack": 0.00575, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.5},
    {"attack": 0.008625, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.375},
    {"attack": 0.01078125, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.3},
    {"attack": 0.0129375, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.25}
]

SEEDS = list(range(6140, 6160))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run boundary anomaly investigation experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c408_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 408: BOUNDARY ANOMALY INVESTIGATION")
    ...
    print(f"Attack: 0.71875×, Conversion: 2.5×")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c408_boundary_anomaly_investigation_results.json"
