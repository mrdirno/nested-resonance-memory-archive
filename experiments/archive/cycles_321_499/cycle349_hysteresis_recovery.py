#!/usr/bin/env python3
"""
CYCLE 349: HYSTERESIS RECOVERY TEST
...
Purpose:
  Test if collapsed systems can recover when K is raised above threshold.

Research Question:
  After collapse at K=200, can reintroduction at K=250 succeed?

Design:
  - Start with standard populations
  - Initial K=200 (below threshold, causes collapse)
  - After collapse detected, reintroduce small population
  - Raise K to 250 (above threshold)
  - Test if recovery is possible
  - 20 seeds
"""
...
CYCLE_ID = "C349"
CYCLE_NAME = "Hysteresis Recovery Test"
MODE = "HYSTERESIS_RECOVERY"

CYCLES = 30000
K_COLLAPSE = 200  # Below threshold
K_RECOVERY = 250  # Above threshold
REINTRO_CYCLE = 5000  # When to reintroduce
INITIAL = [300, 30, 10, 5, 3, 2, 2]
REINTRO = [100, 10, 5, 3, 2, 1, 1]  # Smaller reintroduction
...
SEEDS = list(range(4960, 4980))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run hysteresis recovery experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c349_seed{seed}.db"
    ...
    for cycle in range(cycles):
        ...
        # Check for collapse
        if not collapsed and sum(ns) == 0:
            collapsed = True

        # Reintroduce at REINTRO_CYCLE
        if cycle == REINTRO_CYCLE:
            for lvl in range(n_levels):
                for i in range(REINTRO[lvl]):
        ...
        # Check if recovered (post-reintroduction, all levels present)
        if cycle > REINTRO_CYCLE + 1000 and all(n > 0 for n in ns):
            recovered = True
...
def main():
    print("=" * 80)
    print(f"CYCLE 349: HYSTERESIS RECOVERY TEST")
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c349_hysteresis_recovery_results.json"
