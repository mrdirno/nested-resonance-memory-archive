#!/usr/bin/env python3
"""
CYCLE 308: OMNIVORY / INTRAGUILD PREDATION
...
Purpose:
  Test omnivory where top predator eats both mesopredators AND prey.

Research Question:
  Does omnivory (intraguild predation) stabilize or destabilize the system?

Background:
  C304: Stable tri-trophic with specialists
  C308: Add omnivory - top predator also eats prey

Ecological Theory:
  Intraguild predation (IGP) often destabilizes by competing with mesopredator.

Design:
  - Top predator eats meso (primary) and prey (secondary)
  - Compare pure specialist vs omnivore
...
TOP_ATTACK_PREY = 0.001  # Secondary prey consumption (omnivory)
TOP_H = 0.03
TOP_E_CONSUME = 0.4
TOP_E_PER_MESO = 0.6
TOP_E_PER_PREY = 0.3  # Less energy from prey than from meso
TOP_CONVERSION = 0.25

# Test conditions
CONDITIONS = [
    {"name": "specialist", "omnivory": False},
    {"name": "omnivore", "omnivory": True}
]
SEEDS = [1500, 1501, 1502]


def run_experiment(condition: dict, seed: int, cycles: int) -> dict:
    """Run omnivory experiment."""

    label = condition["name"]
    omnivory = condition["omnivory"]

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c308_{label}_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 308: OMNIVORY / INTRAGUILD PREDATION")
    ...
    print("=" * 80)
    print()
    print("Research Question: Does omnivory stabilize or destabilize?")
    print(f"Top attack on meso: {TOP_ATTACK_MESO}")
    print(f"Top attack on prey: {TOP_ATTACK_PREY} (omnivory)")
    print()
    ...
    print("\nOmnivory Results:")
    print("-" * 80)
    print(f"{'Condition':>12} {'Prey':>10} {'Meso':>10} {'Top':>10} {'Coexist':>10}")
    print("-" * 80)
    ...
    for cond in CONDITIONS:
        ...
        if exp_list:
            ...
            print(f"{name:>12} {prey_f:>10.1f} {meso_f:>10.1f} {top_f:>10.1f} {coexist:>10.0%}")
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c308_omnivory_results.json"
