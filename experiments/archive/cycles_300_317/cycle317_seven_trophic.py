...
Purpose:
  Test seven-level food chain (septenary consumer).

Research Question:
  Where is the actual stability limit? Can seven levels coexist?

Background:
  C316: Six-trophic stable (300-30-10-5-3-2)
  C317: Extend to seven levels (L6 = 1?)

Design:
  - 7 populations
  - Compare six- vs seven-trophic
...
CYCLE_ID = "C317"
CYCLE_NAME = "Seven-Trophic Chain"
MODE = "SEVEN_TROPHIC"

CYCLES = 30000
INITIAL = [300, 30, 10, 5, 3, 2, 1]  # prey, meso, top, apex, hyper, ultra, supreme

# Parameters for each level
PARAMS = [
    {"f": 0.1, "K": 600, "e_con": 0.2, "e_rech": 0.4},  # L0 prey
    ...
    {"attack": 0.018, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.10}   # L6 supreme
]

CONDITIONS = [
    {"name": "six_trophic", "n_levels": 6},
    {"name": "seven_trophic", "n_levels": 7}
]
SEEDS = [2400, 2401, 2402]


def run_experiment(condition: dict, seed: int, cycles: int) -> dict:
    """Run seven-trophic experiment."""

    label = condition["name"]
    n_levels = condition["n_levels"]

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c317_{label}_seed{seed}.db"
    ...
    # Coexistence: all levels > 0 (very lenient for top levels)
    coexist = all(finals[i] >= (0.5 if i >= 5 else 1 if i >= 4 else 3) for i in range(n_levels))
...
def main():
    print("=" * 80)
    print(f"CYCLE 317: SEVEN-TROPHIC CHAIN")
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c317_seven_trophic_results.json"
