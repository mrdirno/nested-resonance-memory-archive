#!/usr/bin/env python3
"""
CYCLE 387: MEDIUM-LOW ATTACK + 3.0× CONVERSION - 40 CYCLES
...
Purpose:
  Test if 0.375× attack achieves 100% at 3.0× conversion.

Background:
  C384: 0.375× attack + 2.5× conv - 100%
  C386: 0.25× attack + 3.0× conv - 65%
  C387: 0.375× attack + 3.0× conv - expect 100%?

Design:
  - K declines from 600 to 200 over 40 cycles
  - Attack: 0.375×, Conversion: 3.0×
  - 20 seeds
"""
...
CYCLE_ID = "C387"
CYCLE_NAME = "Med-Low Attack 3x Conv"
MODE = "MLOW_ATK_3XCONV"
...
# 0.375× attack + 3.0× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.001125, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.9},
    {"attack": 0.001875, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.75},
    {"attack": 0.003, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.6},
    {"attack": 0.0045, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.45},
    {"attack": 0.005625, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.36},
    {"attack": 0.00675, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.3}
]

SEEDS = list(range(5720, 5740))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run medium-low attack + 3.0× conversion experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c387_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 387: MEDIUM-LOW ATTACK + 3.0× CONVERSION - 40 CYCLES")
    ...
    print(f"Attack: 0.375×, Conversion: 3.0×")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c387_med_low_attack_3x_conv_results.json"
