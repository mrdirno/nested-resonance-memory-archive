#!/usr/bin/env python3
"""
CYCLE 443: 0.95× ATTACK AT 2.5× CONVERSION
...
SEEDS = list(range(6840, 6860))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c443_seed{seed}.db", n_populations=n_levels, mode="095X_25XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 443: 0.95× + 2.5× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c443_095x_at_25x_conv_results.json", 'w') as f:
