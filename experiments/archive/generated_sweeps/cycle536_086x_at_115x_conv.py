#!/usr/bin/env python3
"""
CYCLE 536: 0.86× ATTACK AT 1.15× CONVERSION
...
Purpose:
  Find where 0.86× rise from 30% begins.
"""
...
SEEDS = list(range(8701, 8721))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c536_seed{seed}.db", n_populations=n_levels, mode="086X_115XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 536: 0.86× + 1.15× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c536_086x_at_115x_conv_results.json", 'w') as f:
