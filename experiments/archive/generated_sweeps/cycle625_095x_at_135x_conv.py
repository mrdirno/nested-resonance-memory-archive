#!/usr/bin/env python3
"""
CYCLE 625: 0.95× ATTACK AT 1.35× CONVERSION
...
SEEDS = list(range(10481, 10501))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c625_seed{seed}.db", n_populations=n_levels, mode="095X_135XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 625: 0.95× + 1.35× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c625_095x_at_135x_conv_results.json", 'w') as f:
