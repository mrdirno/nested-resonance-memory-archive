#!/usr/bin/env python3
"""
CYCLE 461: 0.97× ATTACK AT 2.5× CONVERSION
...
# 0.97× attack + 2.5× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.00291, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.75},
    {"attack": 0.00485, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.625},
    {"attack": 0.00776, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.5},
    {"attack": 0.01164, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.375},
    {"attack": 0.01455, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.3},
    {"attack": 0.01746, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.25}
]
SEEDS = list(range(7200, 7220))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c461_seed{seed}.db", n_populations=n_levels, mode="097X_25XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 461: 0.97× + 2.5× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c461_097x_at_25x_conv_results.json", 'w') as f:
        json.dump({"cycle_id": CYCLE_ID, "attack_multiplier": 0.97, "conversion_multiplier": 2.5, "coexistence_rate": coexist_count/len(results), "results": results}, f, indent=2)
