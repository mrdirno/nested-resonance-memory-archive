#!/usr/bin/env python3
"""
CYCLE 419: 0.75× ATTACK AT 2.25× CONVERSION
...
Purpose:
  Map conversion profile at 0.75× attack.

Background:
  At 2.0× conversion: 0.75× = 95%
  At 2.5× conversion: 0.75× = 80%

  C419: 0.75× + 2.25× - intermediate value?

Design:
  - K declines from 600 to 200 over 40 cycles
  - Attack: 0.75×, Conversion: 2.25×
  - 20 seeds
"""
...
CYCLE_ID = "C419"
CYCLE_NAME = "0.75x at 2.25x Conv"
MODE = "075X_225XCONV"
...
# 0.75× attack + 2.25× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.00225, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.675},
    {"attack": 0.00375, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.5625},
    {"attack": 0.006, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.45},
    {"attack": 0.009, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.3375},
    {"attack": 0.01125, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.27},
    {"attack": 0.0135, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.225}
]

SEEDS = list(range(6360, 6380))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run 0.75× attack at 2.25× conversion experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c419_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 419: 0.75× ATTACK AT 2.25× CONVERSION")
    ...
    print(f"Attack: 0.75×, Conversion: 2.25×")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c419_075x_at_225x_conv_results.json"
