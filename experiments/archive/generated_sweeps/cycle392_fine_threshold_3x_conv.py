#!/usr/bin/env python3
"""
CYCLE 392: FINE THRESHOLD SEARCH - 3.0× CONVERSION
...
Purpose:
  Find exact threshold for 100% at 3.0× conversion.

Background:
  C386: 0.25× attack + 3.0× conv - 65%
  C387: 0.375× attack + 3.0× conv - 100%
  C392: 0.3125× attack + 3.0× conv - expect 80-90%?

Design:
  - K declines from 600 to 200 over 40 cycles
  - Attack: 0.3125×, Conversion: 3.0×
  - 20 seeds
"""
...
CYCLE_ID = "C392"
CYCLE_NAME = "Fine Threshold 3x Conv"
MODE = "FINE_THRESH_3XCONV"
...
# 0.3125× attack + 3.0× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.0009375, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.9},
    {"attack": 0.0015625, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.75},
    {"attack": 0.0025, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.6},
    {"attack": 0.00375, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.45},
    {"attack": 0.0046875, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.36},
    {"attack": 0.005625, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.3}
]

SEEDS = list(range(5820, 5840))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run fine threshold search at 3.0× conversion."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c392_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 392: FINE THRESHOLD SEARCH - 3.0× CONVERSION")
    ...
    print(f"Attack: 0.3125×, Conversion: 3.0×")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c392_fine_threshold_3x_conv_results.json"
