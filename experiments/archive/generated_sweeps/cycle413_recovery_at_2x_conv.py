#!/usr/bin/env python3
"""
CYCLE 413: RECOVERY POINT AT 2.0× CONVERSION
...
Purpose:
  Test if recovery point (0.71875×) reaches 100% at 2.0× conversion.

Background:
  At 2.5× conversion: 0.71875× = 100% (recovery)
  At 2.0× conversion: 0.6875× = 90% (notch persists)

  C413: 0.71875× + 2.0× - recovery at lower conv?
...
CYCLE_ID = "C413"
CYCLE_NAME = "Recovery at 2x Conv"
MODE = "RECOVERY_2XCONV"
...
# 0.71875× attack + 2.0× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.00215625, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.6},
    {"attack": 0.00359375, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.5},
    {"attack": 0.00575, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.4},
    {"attack": 0.008625, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.3},
    {"attack": 0.01078125, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.24},
    {"attack": 0.0129375, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.2}
]

SEEDS = list(range(6240, 6260))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run recovery point at 2.0× conversion experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c413_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 413: RECOVERY POINT AT 2.0× CONVERSION")
    ...
    print(f"Attack: 0.71875×, Conversion: 2.0×")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c413_recovery_at_2x_conv_results.json"
