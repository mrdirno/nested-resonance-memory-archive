#!/usr/bin/env python3
"""
CYCLE 538: 0.95× ATTACK AT 1.15× CONVERSION
...
Purpose:
  Complete 0.95× profile in pre-suppression region.
"""
...
SEEDS = list(range(8741, 8761))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c538_seed{seed}.db", n_populations=n_levels, mode="095X_115XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 538: 0.95× + 1.15× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c538_095x_at_115x_conv_results.json", 'w') as f:
