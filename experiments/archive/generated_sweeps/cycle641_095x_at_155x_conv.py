#!/usr/bin/env python3
"""
CYCLE 641: 0.95× ATTACK AT 1.55× CONVERSION
...
SEEDS = list(range(10801, 10821))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c641_seed{seed}.db", n_populations=n_levels, mode="095X_155XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 641: 0.95× + 1.55× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c641_095x_at_155x_conv_results.json", 'w') as f:
