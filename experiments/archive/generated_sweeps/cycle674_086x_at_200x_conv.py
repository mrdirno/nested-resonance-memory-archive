#!/usr/bin/env python3
"""
CYCLE 674: 0.86× ATTACK AT 2.0× CONVERSION
...
SEEDS = list(range(11461, 11481))

def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c674_seed{seed}.db", n_populations=n_levels, mode="086X_200XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 674: 0.86× + 2.0× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c674_086x_at_200x_conv_results.json", 'w') as f:
