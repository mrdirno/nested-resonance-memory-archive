#!/usr/bin/env python3
"""
CYCLE 752: 0.92× ATTACK AT 3.0× CONVERSION - CRITICAL BOUNDARY TEST
...
SEEDS = list(range(13021, 13041))

def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c752_seed{seed}.db", n_populations=n_levels, mode="092X_300XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 752: 0.92× + 3.0× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c752_092x_at_300x_conv_results.json", 'w') as f:
