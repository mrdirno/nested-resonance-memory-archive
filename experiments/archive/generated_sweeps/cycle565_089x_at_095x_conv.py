#!/usr/bin/env python3
"""
CYCLE 565: 0.89× ATTACK AT 0.95× CONVERSION
...
Purpose:
  Map 0.89× response at 0.95× local minimum zone.
"""
...
# 0.89× attack + 0.95× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.00267, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.285},
    {"attack": 0.00445, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.2375},
    {"attack": 0.00712, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.19},
    {"attack": 0.01068, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.1425},
    {"attack": 0.01335, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.114},
    {"attack": 0.01602, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.095}
]
SEEDS = list(range(9281, 9301))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c565_seed{seed}.db", n_populations=n_levels, mode="089X_095XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 565: 0.89× + 0.95× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c565_089x_at_095x_conv_results.json", 'w') as f:
        json.dump({"cycle_id": CYCLE_ID, "attack_multiplier": 0.89, "conversion_multiplier": 0.95, "coexistence_rate": coexist_count/len(results), "results": results}, f, indent=2)
