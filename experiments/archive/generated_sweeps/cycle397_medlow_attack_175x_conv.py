#!/usr/bin/env python3
"""
CYCLE 397: MEDIUM-LOW ATTACK + 1.75× CONVERSION - 40 CYCLES
...
Purpose:
  Refine sigmoid transition at 0.375× attack.
...
Background:
  ...
  C397: 1.75× conv - expect ~50-60%?

Design:
  - K declines from 600 to 200 over 40 cycles
  - Attack: 0.375×, Conversion: 1.75×
  - 20 seeds
"""
...
CYCLE_ID = "C397"
CYCLE_NAME = "Med-Low Attack 1.75x Conv"
MODE = "MEDLOW_ATK_175XCONV"
...
# 0.375× attack + 1.75× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.001125, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.525},
    {"attack": 0.001875, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.4375},
    {"attack": 0.003, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.35},
    {"attack": 0.0045, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.2625},
    {"attack": 0.005625, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.21},
    {"attack": 0.00675, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.175}
]

SEEDS = list(range(5920, 5940))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run medium-low attack + 1.75× conversion experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c397_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 397: MEDIUM-LOW ATTACK + 1.75× CONVERSION - 40 CYCLES")
    ...
    print(f"Attack: 0.375×, Conversion: 1.75×")
    print("=" * 80)
    print()
    ...
    output = {
        ...
        "conversion_multiplier": 1.75,
        "n_seeds": len(SEEDS),
        "coexistence_rate": coexist_count / len(results) if results else 0,
        "results": results
    }

    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c397_medlow_attack_175x_conv_results.json"
