#!/usr/bin/env python3
"""
CYCLE 645: 0.95× ATTACK AT 1.6× CONVERSION
...
Purpose:
  Test if 0.95× recovers from 40% collapse.
"""
...
SEEDS = list(range(10881, 10901))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c645_seed{seed}.db", n_populations=n_levels, mode="095X_160XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 645: 0.95× + 1.6× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c645_095x_at_160x_conv_results.json", 'w') as f:
