#!/usr/bin/env python3
"""
CYCLE 432: 0.65× ATTACK AT 3.0× CONVERSION
...
Purpose:
  Complete 0.65× column in phase diagram.
"""
...
# 0.65× attack + 3.0× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.00195, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.9},
    {"attack": 0.00325, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.75},
    {"attack": 0.0052, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.6},
    {"attack": 0.0078, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.45},
    {"attack": 0.00975, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.36},
    {"attack": 0.0117, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.3}
]
SEEDS = list(range(6620, 6640))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c432_seed{seed}.db", n_populations=n_levels, mode="065X_3XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 432: 0.65× + 3.0× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c432_065x_at_3x_conv_results.json", 'w') as f:
        json.dump({"cycle_id": CYCLE_ID, "attack_multiplier": 0.65, "conversion_multiplier": 3.0, "coexistence_rate": coexist_count/len(results), "results": results}, f, indent=2)
