#!/usr/bin/env python3
"""
CYCLE 989: 0.95× ATTACK AT 5.95× CONVERSION - FINAL PRE-6.0× MILESTONE
"""
...
BASE_PARAMS = [
    ...
    {"attack": 0.00285, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 1.785},
    {"attack": 0.00475, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 1.4875},
    {"attack": 0.0076, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 1.19},
    {"attack": 0.0114, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.8925},
    {"attack": 0.01425, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.714},
    {"attack": 0.0171, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.595}
]
SEEDS = list(range(17761, 17781))

def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c989_seed{seed}.db", n_populations=n_levels, mode="095X_595XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 989: 0.95× + 5.95× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c989_095x_at_595x_conv_results.json", 'w') as f:
        json.dump({"cycle_id": CYCLE_ID, "attack_multiplier": 0.95, "conversion_multiplier": 5.95, "coexistence_rate": coexist_count/len(results), "results": results}, f, indent=2)
