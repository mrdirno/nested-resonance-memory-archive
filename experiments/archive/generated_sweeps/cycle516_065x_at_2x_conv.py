#!/usr/bin/env python3
"""
CYCLE 516: 0.65× ATTACK AT 2.0× CONVERSION
...
Purpose:
  Characterize 0.64× micro-dip boundary.
"""
...
SEEDS = list(range(8301, 8321))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c516_seed{seed}.db", n_populations=n_levels, mode="065X_2XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 516: 0.65× + 2.0× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c516_065x_at_2x_conv_results.json", 'w') as f:
        json.dump({"cycle_id": CYCLE_ID, "attack_multiplier": 0.65, "conversion_multiplier": 2.0, "coexistence_rate": coexist_count/len(results), "results": results}, f, indent=2)
