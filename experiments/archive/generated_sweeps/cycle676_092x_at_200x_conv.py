#!/usr/bin/env python3
"""
CYCLE 676: 0.92× ATTACK AT 2.0× CONVERSION
...
SEEDS = list(range(11501, 11521))

def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c676_seed{seed}.db", n_populations=n_levels, mode="092X_200XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 676: 0.92× + 2.0× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c676_092x_at_200x_conv_results.json", 'w') as f:
