#!/usr/bin/env python3
"""
CYCLE 550: 1.0× ATTACK AT 2.0× CONVERSION
...
Purpose:
  Test if 1.0× collapse persists at high conversion.
"""
...
SEEDS = list(range(8981, 9001))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c550_seed{seed}.db", n_populations=n_levels, mode="10X_20XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 550: 1.0× + 2.0× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c550_10x_at_20x_conv_results.json", 'w') as f:
