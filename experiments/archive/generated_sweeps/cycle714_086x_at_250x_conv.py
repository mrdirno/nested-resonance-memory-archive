#!/usr/bin/env python3
"""
CYCLE 714: 0.86× ATTACK AT 2.5× CONVERSION
...
SEEDS = list(range(12261, 12281))

def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c714_seed{seed}.db", n_populations=n_levels, mode="086X_250XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 714: 0.86× + 2.5× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c714_086x_at_250x_conv_results.json", 'w') as f:
