#!/usr/bin/env python3
"""
CYCLE 444: 0.95× ATTACK AT 3.0× CONVERSION
...
SEEDS = list(range(6860, 6880))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c444_seed{seed}.db", n_populations=n_levels, mode="095X_3XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 444: 0.95× + 3.0× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c444_095x_at_3x_conv_results.json", 'w') as f:
