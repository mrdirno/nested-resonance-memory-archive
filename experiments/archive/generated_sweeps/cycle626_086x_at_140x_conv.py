#!/usr/bin/env python3
"""
CYCLE 626: 0.86× ATTACK AT 1.4× CONVERSION
...
Purpose:
  Test 0.86× recovery continuation at 1.4×.
"""
...
SEEDS = list(range(10501, 10521))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c626_seed{seed}.db", n_populations=n_levels, mode="086X_140XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 626: 0.86× + 1.4× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c626_086x_at_140x_conv_results.json", 'w') as f:
