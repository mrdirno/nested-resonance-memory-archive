#!/usr/bin/env python3
"""
CYCLE 659: 0.89× ATTACK AT 1.8× CONVERSION
...
BASE_PARAMS = [
    ...
    {"attack": 0.00267, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.54},
    {"attack": 0.00445, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.45},
    {"attack": 0.00712, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.36},
    {"attack": 0.01068, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.27},
    {"attack": 0.01335, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.216},
    {"attack": 0.01602, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.18}
]
SEEDS = list(range(11161, 11181))

def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c659_seed{seed}.db", n_populations=n_levels, mode="089X_180XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 659: 0.89× + 1.8× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c659_089x_at_180x_conv_results.json", 'w') as f:
        json.dump({"cycle_id": CYCLE_ID, "attack_multiplier": 0.89, "conversion_multiplier": 1.8, "coexistence_rate": coexist_count/len(results), "results": results}, f, indent=2)
