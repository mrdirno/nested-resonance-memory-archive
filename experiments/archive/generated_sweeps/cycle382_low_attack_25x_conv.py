#!/usr/bin/env python3
"""
CYCLE 382: LOW ATTACK + 2.5× CONVERSION - 40 CYCLES
...
Purpose:
  Test if 0.5× attack maintains 100% at even higher conversion.

Background:
  C380: 0.5× attack + 2.0× conv - 100%
  C382: 0.5× attack + 2.5× conv - still 100%?

Design:
  - K declines from 600 to 200 over 40 cycles
  - Attack: 0.5×, Conversion: 2.5×
  - 20 seeds
"""
...
CYCLE_ID = "C382"
CYCLE_NAME = "Low Attack 2.5x Conv"
MODE = "LOW_ATK_25XCONV"
...
# 0.5× attack + 2.5× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.0015, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.75},
    {"attack": 0.0025, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.625},
    {"attack": 0.004, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.5},
    {"attack": 0.006, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.375},
    {"attack": 0.0075, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.3},
    {"attack": 0.009, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.25}
]

SEEDS = list(range(5620, 5640))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run low attack + 2.5× conversion experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c382_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 382: LOW ATTACK + 2.5× CONVERSION - 40 CYCLES")
    ...
    print(f"Attack: 0.5×, Conversion: 2.5×")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c382_low_attack_25x_conv_results.json"
