#!/usr/bin/env python3
"""
CYCLE 487: 0.89× ATTACK AT 1.5× CONVERSION
...
Purpose:
  Fill inter-peak region at 1.5× conversion.
"""
...
SEEDS = list(range(7720, 7740))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c487_seed{seed}.db", n_populations=n_levels, mode="089X_15XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 487: 0.89× + 1.5× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c487_089x_at_15x_conv_results.json", 'w') as f:
