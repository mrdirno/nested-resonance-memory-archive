#!/usr/bin/env python3
"""
CYCLE 395: ULTRA-LOW ATTACK + 4.5× CONVERSION - 40 CYCLES
...
Purpose:
  Find threshold for stable 100% survival (not population explosion).

Background:
  C393: 0.25× attack + 4.0× conv - 90% (stable)
  C394: 0.25× attack + 5.0× conv - 100% (population explosion)
  C395: 0.25× attack + 4.5× conv - expect stable ~95-100%?

Design:
  - K declines from 600 to 200 over 40 cycles
  - Attack: 0.25×, Conversion: 4.5×
  - 20 seeds
"""
...
CYCLE_ID = "C395"
CYCLE_NAME = "Ultra-Low Attack 4.5x Conv"
MODE = "ULOW_ATK_45XCONV"
...
# 0.25× attack + 4.5× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.00075, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 1.35},
    {"attack": 0.00125, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 1.125},
    {"attack": 0.002, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.9},
    {"attack": 0.003, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.675},
    {"attack": 0.00375, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.54},
    {"attack": 0.0045, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.45}
]

SEEDS = list(range(5880, 5900))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run ultra-low attack + 4.5× conversion experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c395_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 395: ULTRA-LOW ATTACK + 4.5× CONVERSION - 40 CYCLES")
    ...
    print(f"Attack: 0.25×, Conversion: 4.5×")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c395_ultralow_attack_45x_conv_results.json"
