#!/usr/bin/env python3
"""
CYCLE 297: REFUGE AT INTERMEDIATE ATTACK RATE
...
Purpose:
  Test spatial refuge at intermediate attack rate where predators are viable.

Research Question:
  Does refuge stabilize coexistence when predation is not already marginal?

Background:
  C295: Attack rate 0.002 gave 67% coexistence
  C296: Refuge at high attack rate (0.01) reduced coexistence to 0%
  C297: Test refuge at intermediate rate (0.003) where predators are viable

Design:
  - Attack rate 0.003 (between optimal 0.002 and marginal 0.005)
  - Test refuge fractions
...
# Configuration
CYCLE_ID = "C297"
CYCLE_NAME = "Refuge at Intermediate Attack Rate"
MODE = "REFUGE_INTERMEDIATE"
...
# Intermediate attack rate (between 0.002 optimal and 0.005 marginal)
ATTACK_RATE = 0.003
...
def run_experiment(refuge_fraction: float, seed: int, cycles: int) -> dict:
    """Run refuge experiment at intermediate attack rate."""

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c297_refuge{refuge_fraction}_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 297: REFUGE AT INTERMEDIATE ATTACK RATE")
    ...
    print("=" * 80)
    print()
    print("Research Question: Does refuge stabilize when predation is not marginal?")
    print()
    print(f"Attack rate: {ATTACK_RATE} (intermediate)")
    ...
    print("\nRefuge Effects at Intermediate Attack Rate:")
    print("-" * 80)
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c297_refuge_intermediate_results.json"
