...
def run_experiment(seed, e_consume):
    ...
    reality = RealityInterface(
        db_path=f"experiments/results/c281_METABOLIC_{e_consume:.4f}_seed{seed}.db",
        n_populations=1
    )
...
def main():
    print("CYCLE 281: ULTRA-FINE METABOLIC SWEEP (Precision Criticality)")
    print("===============================================================")
    
    # Parameter Sweep: E_consume from 0.100 to 0.110
    # We expect a transition somewhere in this range.
    # 0.100, 0.101, ..., 0.110
    e_consume_values = [round(0.100 + i * 0.001, 3) for i in range(11)]
    seeds = range(800, 820) # 20 seeds per condition for higher confidence
    ...
    for e_val in e_consume_values:
        print(f"\nTesting E_consume = {e_val:.3f} (20 seeds)")
        print("----------------------------------------")
    ...
    output_file = "experiments/results/c281_ultra_fine_metabolic_results.json"
