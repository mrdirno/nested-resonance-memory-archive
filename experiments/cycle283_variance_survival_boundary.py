...
def main():
    print("CYCLE 283: FINE-GRAINED VARIANCE-SURVIVAL BOUNDARY")
    print("===================================================")
    print("Objective: Quantify the 'rescue effect' of variance on population survival across the critical metabolic cost.")
    
    # Test Conditions
    e_consume_values = [round(0.100 + i * 0.001, 3) for i in range(11)] # 0.100 to 0.110
    fixed_sigma = 0.05 # Small fixed variance
    seeds = range(1000, 1020) # 20 seeds per condition
    
    results = []
    
    for e_val in e_consume_values:
        print(f"\nTesting E_consume = {e_val:.3f} (Sigma={fixed_sigma})")
        batch_results = []
        for seed in seeds:
            res = run_experiment(seed, e_val, fixed_sigma)
    ...
    with open("experiments/results/c283_variance_survival_boundary.json", "w") as f:
