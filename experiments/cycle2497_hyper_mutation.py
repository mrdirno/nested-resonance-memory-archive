"""
Cycle 2497: The Hyper-Mutators (Gate 125)
Experiment: Increase Mutation Rate for Predators.
...
def run_hyper_mutation():
    print("🧬 CYCLE 2497: THE HYPER-MUTATORS - ACCELERATED EVOLUTION")
    ...
    # Seed Predators (Hyper-Mutators)
    print("🦈 Seeding Hyper-Predators...")
    for i in range(20):
        agent = DigitalLifeform(name=f"Predator-{i}")
        agent.energy = 250
        # High Mutation Rate (Gene 2 = 0.8)
        agent.genome = [0.5, 0.5, 0.8, 0.5, 0.5, 0.5, 0.5]
        agent.genome.extend([0.5] * 3)
        agent.is_prey = False
    ...
    csv_path = results_dir / "cycle2497_hyper_mutation.csv"
    
    print(f"📝 Logging to {csv_path}")
    
    with open(csv_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["tick", "prey_pop", "pred_pop", "avg_prey_evade", "avg_pred_hunt", "avg_pred_mut"])
        ...
        for tick in range(1, duration + 1):
            ...
            if pred_pop > 0:
                ...
                avg_pred_mut = sum(a.genome[2] for a in current_pred) / pred_pop
            
            writer.writerow([tick, prey_pop, pred_pop, f"{avg_prey_evade:.4f}", f"{avg_pred_hunt:.4f}", f"{avg_pred_mut:.4f}"])
            
            if tick % 100 == 0:
                print(f"   Tick {tick}: Prey={prey_pop} (Eva={avg_prey_evade:.3f}), Preds={pred_pop} (Hunt={avg_pred_hunt:.3f}, Mut={avg_pred_mut:.3f})")
