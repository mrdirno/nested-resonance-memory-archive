#!/usr/bin/env python3
"""
CYCLE 634: 0.86× ATTACK AT 1.5× CONVERSION
...
Purpose:
  Test if 0.86× maintains 90% or declines.
"""
...
SEEDS = list(range(10661, 10681))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c634_seed{seed}.db", n_populations=n_levels, mode="086X_150XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 634: 0.86× + 1.5× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c634_086x_at_150x_conv_results.json", 'w') as f:
