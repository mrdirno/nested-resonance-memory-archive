...
Purpose:
  Add metabolic cost to attack rate to create selection pressure.

Research Question:
  Does directional selection emerge when high attack is costly?

Background:
  C328-C331: Neutral evolution (0.5-1.3 all viable)
  C332: Add cost → create fitness gradient

Design:
  - 7 populations
  - Energy cost = base_cost * trait_multiplier
  - Higher attack = higher metabolic cost
  - Test from optimum (1.0) - should evolve DOWN
...
CYCLE_ID = "C332"
CYCLE_NAME = "Attack Cost Test"
MODE = "ATTACK_COST"

CYCLES = 30000
K = 600
INITIAL = [300, 30, 10, 5, 3, 2, 2]
INITIAL_TRAIT = 1.0  # Start at supposed optimum
...
SEEDS = list(range(3900, 3920))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run attack cost experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c332_seed{seed}.db"
    ...
    for cycle in range(cycles):
        ...
        for lvl in range(1, n_levels):
            for a in reality.get_population_agents(lvl):
                ...
                # ATTACK COST: energy consumption scaled by trait
                effective_cost = BASE_PARAMS[lvl]["e_con"] * trait_mult
...
def main():
    print("=" * 80)
    print(f"CYCLE 332: ATTACK COST TEST")
    ...
    print(f"Cost model: e_con * trait_multiplier")
    print("=" * 80)
    print()
    ...
    output = {
        ...
        "cost_model": "e_con * trait_multiplier",
        "n_seeds": len(SEEDS),
        "coexistence_rate": coexist_count / len(results) if results else 0,
        "results": results
    }

    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c332_attack_cost_results.json"
