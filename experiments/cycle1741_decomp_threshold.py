...
Test if changing the decomposition threshold (1.3) affects dead zones.
"""
...
def find_minimum(seeds, n_range, decomp_threshold):
    results = []
    for n in n_range:
        outcomes = [run_test(seed, n, decomp_threshold) for seed in seeds]
...
def main():
    print(f"CYCLE 1741: Decomp Threshold | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 70)
    print("Goal: Test if decomposition threshold affects dead zones")
    print("=" * 70)

    seeds = list(range(1741001, 1741031))
