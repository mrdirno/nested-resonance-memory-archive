"""
Cycle 424: The Perpetual Engine (Long-Duration Run)
Role: Perpetual Engine
Responsibility: Execute the Architect loop for an extended duration to observe emergent behavior and stability.
"""
import asyncio
import websockets
import json
...
class KnowledgeGraphInterface:
    ...
    def _init_db(self):
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS perpetual_log (
    ...
    def log_event(self, cycle, phase, details):
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO perpetual_log (cycle_num, phase, details) VALUES (?, ?, ?)",
...
class PerpetualEngine:
    ...
    async def run_cycle(self, cycle_num):
        try:
            ...
            return True, self.strategist.mood, target_shape['params']['mode'], actual_fitness
            
        except Exception as e:
            print(f"[PERPETUAL] Cycle {cycle_num} Failed: {e}")
            traceback.print_exc()
            self.memory.log_event(cycle_num, "ERROR", {"error": str(e)})
...
async def main():
    ...
    print("Starting Perpetual Engine (100 Cycles)...")
    
    stats = {"cycles": 0, "successes": 0, "moods": {}}
    
    start_time = time.time()
    
    for i in range(1, 101): # 100 Cycles
        success, mood, shape, fitness = await engine.run_cycle(i)
        ...
        if i % 10 == 0:
            ...
            print(f"\n--- Status Report (Cycle {i}) ---")
            print(f"Elapsed: {elapsed:.2f}s")
            print(f"Success Rate: {stats['successes']}/{stats['cycles']}")
            print(f"Current Mood: {mood}")
            print(f"Recent Shape: {shape} (Fitness: {fitness:.2f})")
            print(f"Mood Distribution: {stats['moods']}")
            
        await asyncio.sleep(0.01) # Fast forward for simulation
        
    print("\nPerpetual Run Complete.")
