#!/usr/bin/env python3
"""
CYCLE 502: 0.98× ATTACK AT 1.5× CONVERSION
...
Purpose:
  Test right side of notch region at 1.5× conversion.
"""
...
SEEDS = list(range(8020, 8040))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c502_seed{seed}.db", n_populations=n_levels, mode="098X_15XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 502: 0.98× + 1.5× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c502_098x_at_15x_conv_results.json", 'w') as f:
