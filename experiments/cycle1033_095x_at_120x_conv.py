#!/usr/bin/env python3
"""
CYCLE 1033: 0.95× ATTACK AT 12.0× CONVERSION - ULTRA-HIGH CONVERSION TEST
"""
...
BASE_PARAMS = [
    ...
    {"attack": 0.00285, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 3.6},
    {"attack": 0.00475, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 3.0},
    {"attack": 0.0076, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 2.4},
    {"attack": 0.0114, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 1.8},
    {"attack": 0.01425, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 1.44},
    {"attack": 0.0171, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 1.2}
]
SEEDS = list(range(18641, 18661))

def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c1033_seed{seed}.db", n_populations=n_levels, mode="095X_120XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 1033: 0.95× + 12.0× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c1033_095x_at_120x_conv_results.json", 'w') as f:
        json.dump({"cycle_id": CYCLE_ID, "attack_multiplier": 0.95, "conversion_multiplier": 12.0, "coexistence_rate": coexist_count/len(results), "results": results}, f, indent=2)
