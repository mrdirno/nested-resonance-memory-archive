#!/usr/bin/env python3
"""
CYCLE 255 HIGH CAPACITY TEST: H1 × H2 Factorial with MAX_AGENTS=1000

**CAPACITY CEILING HYPOTHESIS TEST (Cycle 570):**

Previous C255 results (MAX_AGENTS=100):
- OFF-OFF: 13.97 mean (stable baseline)
- OFF-ON: 99.72 mean (H2 only, hits ceiling)
- ON-OFF: 99.69 mean (H1 only, hits ceiling)
- ON-ON: 99.75 mean (both, ceiling)
- Synergy: -85.68 (ANTAGONISTIC due to ceiling effect)

**Hypothesis:**
Antagonistic interaction was artifact of capacity ceiling. With higher capacity,
true synergy should emerge.

**Predictions with MAX_AGENTS=1000:**
- OFF-OFF: ~14 (unchanged - baseline equilibrium)
- OFF-ON: ~500-700 (H2 exponential growth continues)
- ON-OFF: ~500-700 (H1 exponential growth continues)
- ON-ON: ~900-1000 (SYNERGISTIC - both mechanisms amplify)
- Expected synergy: >200 (strongly synergistic)
...
Date: 2025-10-29
Researcher: Claude (DUALITY-ZERO-V2), Cycle 570
...
CYCLES = 3000
MAX_AGENTS = 1000  # 10× capacity increase to test ceiling hypothesis
RESULTS_FILE = Path(__file__).parent / "results" / "cycle255_h1h2_high_capacity_results.json"
...
def main():
    """Run factorial validation experiment with high capacity."""
    print("=" * 70)
    print("CYCLE 255 HIGH CAPACITY TEST: MAX_AGENTS=1000")
    print("=" * 70)
    print(f"Start time: {datetime.now().isoformat()}")
    print(f"Cycles per experiment: {CYCLES}")
    print(f"Max agents: {MAX_AGENTS} (10× vs baseline)")
    print(f"Hypothesis: Synergy emerges when ceiling removed")
    print()
    ...
    output = {
        'metadata': {
            ...
            'n_per_condition': 1,
            'deterministic': True,
            'hypothesis': 'synergy_emerges_without_ceiling',
            'baseline_max_agents': 100,
