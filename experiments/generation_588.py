#!/usr/bin/env python3
"""
BCP Evolution: Generation 588 (Robust Anchoring Sentry)
Evaluates the selection of a "Robust Anchoring Sentry" which pays an upfront 
metabolic cost to shield the Memory Sentry against substrate degradation, 
preventing gate collapse in deep famines.
"""
...
def evaluate_lineage(name, starvation_durations, psi=2.0):
    ...
    for T in starvation_durations:
        ...
        for t, b in enumerate(budgets):
            ...
            if name in ["decaying_sentry", "robust_anchoring_sentry"] and not has_seed and b <= seed_trigger_budget and current_N > 1:
                has_seed = True
                seed_template_N = current_N
                paid_cost = seed_construction_cost
                if name == "robust_anchoring_sentry":
                    paid_cost += 0.20 # Additional metabolic cost for anchoring
                seed_integrity = 1.0
                sentry_integrity = 1.0
            elif has_seed and is_starving:
                seed_integrity *= 0.86
                if name == "decaying_sentry":
                    sentry_integrity *= 0.923 # ~exp(-0.08)
                elif name == "robust_anchoring_sentry":
                    sentry_integrity *= 0.98  # Anchored sentry decays much slower
...
def run_generation():
    ...
    starvation_durations = [2, 6, 12] # Mixed famines, including an ultra-deep one
    ...
    selected_name = "robust_anchoring_sentry"
    ...
    print(f"\nSelected Lineage: {selected_name.upper()} (Robust Anchoring Mutation)")
