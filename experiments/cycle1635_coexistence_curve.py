...
Maps the entire sub-saturation regime from collapse to saturation.
"""
...
# Full curve from collapse to saturation
MAGNITUDES = [0.05, 0.10, 0.15, 0.20, 0.30, 0.40, 0.50]
...
def run_experiment(seed, magnitude):
    ...
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c1635_mag{magnitude}_seed{seed}.db", n_populations=n_levels, mode=f"CURVE_{magnitude}")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 1635: Full Coexistence Curve | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 70)

    seeds = list(range(60001, 60031))  # 30 seeds per magnitude
    all_results = []

    for mag in MAGNITUDES:
        max_prob = 3.0 * mag * 1.4
        print(f"\nMagnitude {mag} (max prob = {max_prob:.2f})")
        ...
        for i, seed in enumerate(seeds, 1):
            ...
            if i % 10 == 0:
                ...
                print(f"  [{i}/30] {current}/{i} survive")

        coexist_count = sum(1 for r in mag_results if r["coexist"])
        print(f"  → {coexist_count}/30 ({coexist_count/30*100:.0f}%)")
        all_results.extend(mag_results)

    # Generate curve
    print("\n" + "=" * 70)
    print("COEXISTENCE CURVE")
    print("=" * 70)
    ...
    for i, (mag, rate) in enumerate(curve_data):
        if rate >= 0.5 and (i == 0 or curve_data[i-1][1] < 0.5):
            print(f"  50% threshold crossed between {curve_data[i-1][0] if i > 0 else 0:.2f} and {mag:.2f}")
            break

    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c1635_coexistence_curve_results.json", 'w') as f:
        json.dump({"cycle_id": CYCLE_ID, "magnitudes": MAGNITUDES, "seeds_per_mag": len(seeds), "curve": curve_data, "results": all_results}, f, indent=2)
