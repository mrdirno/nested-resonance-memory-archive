#!/usr/bin/env python3
"""
CYCLE 722: 0.86× ATTACK AT 2.6× CONVERSION
...
BASE_PARAMS = [
    ...
    {"attack": 0.00258, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.78},
    {"attack": 0.0043, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.65},
    {"attack": 0.00688, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.52},
    {"attack": 0.01032, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.39},
    {"attack": 0.0129, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.312},
    {"attack": 0.01548, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.26}
]
SEEDS = list(range(12421, 12441))

def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c722_seed{seed}.db", n_populations=n_levels, mode="086X_260XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 722: 0.86× + 2.6× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c722_086x_at_260x_conv_results.json", 'w') as f:
        json.dump({"cycle_id": CYCLE_ID, "attack_multiplier": 0.86, "conversion_multiplier": 2.6, "coexistence_rate": coexist_count/len(results), "results": results}, f, indent=2)
