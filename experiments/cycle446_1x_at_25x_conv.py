#!/usr/bin/env python3
"""
CYCLE 446: 1.0× ATTACK AT 2.5× CONVERSION
...
SEEDS = list(range(6900, 6920))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c446_seed{seed}.db", n_populations=n_levels, mode="1X_25XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 446: 1.0× + 2.5× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c446_1x_at_25x_conv_results.json", 'w') as f:
        json.dump({"cycle_id": CYCLE_ID, "attack_multiplier": 1.0, "conversion_multiplier": 2.5, "coexistence_rate": coexist_count/len(results), "results": results}, f, indent=2)
