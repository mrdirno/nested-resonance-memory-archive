...
# -----------------------------------------------------------------------------
# CHILD V39 (Catalog #104): THE MOORE CURVE (Closed Loop Hilbert)
# -----------------------------------------------------------------------------
# Concept: A continuous loop version of the Hilbert space-filling curve.
#          Unlike the Hilbert curve which has endpoints, the Moore curve is cyclic.
#          Projected onto a sphere or used to fill a toroidal volume.
# Parents: 97_hilbert_cube (Pathfinding), 50_torus_knot (Loop).
# Math: Recursive Lindenmayer System for Moore Curve.
# -----------------------------------------------------------------------------

def generate_moore_curve(iterations):
    ...
    current_dir = np.array([0.0, 1.0]) # North
    step_size = 1.0
...
def generate_child_v39(output_path, diameter=200.0, height=140.0, resolution=120, hole_diameter=14.0):
    print(f"Generating CHILD V39 (The Moore Curve): {output_path}")
    ...
    # ---------------------------------------------------------
    # MOORE CURVE GENERATION
    # ---------------------------------------------------------
    
    # Iteration 5 provides good density for a sphere mapping
    # The curve fills a square area.
    points_2d = generate_moore_curve(5)
    ...
    for p in norm_points:
        ...
        # Map to Sphere
        # To preserve the closed loop property without a seam, we need a seamless map.
        # A simple equirectangular map (theta, phi) has poles.
        # The Moore curve starts and ends at the same place (bottom center of square).
        
        # Let's map to phi (0 to 2pi) and theta (epsilon to pi-epsilon).
        # Moore curve is a loop.
        
        theta = u * math.pi
        phi = v * 2.0 * math.pi
        
        # Twist for aesthetics
        phi += theta * 0.5
            ...
            for bx in range(-brush, brush+1):
                ...
                for by in range(-brush, brush+1):
                    ...
                    for bz in range(-brush, brush+1):
                        ...
                        # Box brush for "Digital" look (Moore curve is square)
                        d_box = max(abs(vx-p_surf[0]), abs(vy-p_surf[1]), abs(vz-p_surf[2]))
                        
                        # Mixed brush? Or simple round for printability.
                        # Round is safer.
                        if d < tube_radius:
                            grid[ix, iy, iz] = True
    ...
    for z_idx in range(res_z):
        ...
        for x_idx in range(res_x):
            ...
            for y_idx in range(res_y):
                ...
                # Support Pillar
                # Necessary for the projected loop
...
if __name__ == "__main__":
    ...
    output_file = os.path.join(output_dir, "child_104_moore_curve.stl")
    
    if len(sys.argv) > 1: output_file = sys.argv[1]
    generate_child_v39(output_file)
