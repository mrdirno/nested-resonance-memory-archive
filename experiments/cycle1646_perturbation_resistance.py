#!/usr/bin/env python3
"""
CYCLE 1646: PERTURBATION RESISTANCE
Tests if optimal parameters allow recovery from mid-simulation population shocks.
Applies 50% population reduction at cycle 5000 to different levels.
"""
...
PERTURBATION_CYCLE = 5000  # When to apply shock
REDUCTION = 0.5  # Kill 50% of population

# Test perturbations at different levels
PERTURBATION_LEVELS = [0, 1, 3, 5, 6, "all"]
...
def run_experiment(seed, perturb_level):
    params = get_params()
    n_levels = 7
    mode = f"PERTURB_{perturb_level}" if perturb_level != "all" else "PERTURB_ALL"
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c1646_perturb{perturb_level}_seed{seed}.db", n_populations=n_levels, mode=mode)
    np.random.seed(seed)
    ...
    for cycle in range(CYCLES):
        ...
        # Apply perturbation at designated cycle
        if cycle == PERTURBATION_CYCLE and not perturbation_applied:
            if perturb_level == "all":
                for lvl in range(n_levels):
                    agents = reality.get_population_agents(lvl)
                    n_kill = int(len(agents) * REDUCTION)
                    for a in agents[:n_kill]:
                        reality.remove_agent(a.agent_id, lvl)
            else:
                agents = reality.get_population_agents(perturb_level)
                n_kill = int(len(agents) * REDUCTION)
                for a in agents[:n_kill]:
                    reality.remove_agent(a.agent_id, perturb_level)
            perturbation_applied = True
            # Re-fetch populations after perturbation
            pops = [reality.get_population_agents(i) for i in range(n_levels)]
            ns = [len(p) for p in pops]
...
def main():
    print(f"CYCLE 1646: Perturbation Resistance | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 70)
    print(f"Testing 50% population shock at cycle {PERTURBATION_CYCLE} (30 seeds)")
    print("=" * 70)

    seeds = list(range(170001, 170031))  # 30 seeds per condition
    all_results = []
    rates = {}

    for perturb_level in PERTURBATION_LEVELS:
        label = f"L{perturb_level}" if perturb_level != "all" else "ALL"
        print(f"\nPerturbation target: {label}")
        results = []
        for seed in seeds:
            r = run_experiment(seed, perturb_level)
            results.append(r)

        rate = sum(1 for r in results if r["coexist"]) / len(results)
        rates[str(perturb_level)] = rate
        print(f"  → Recovery: {rate*100:.0f}%")
        all_results.extend(results)

    # Summary
    print("\n" + "=" * 70)
    print("PERTURBATION RESISTANCE")
    print("=" * 70)

    for perturb_level in PERTURBATION_LEVELS:
        rate = rates[str(perturb_level)]
        bar = "█" * int(rate * 20) + "░" * (20 - int(rate * 20))
        label = f"L{perturb_level}" if perturb_level != "all" else "ALL"
        print(f"{label:6s}: {bar} {rate*100:.0f}%")
    ...
    all_resilient = all(r >= 0.9 for r in rates.values())
    min_rate = min(rates.values())
    max_rate = max(rates.values())

    if all_resilient:
        print(f"\n✅ FULLY RESILIENT: All shocks recoverable ({min_rate*100:.0f}%-{max_rate*100:.0f}%)")
    else:
        vulnerable = [k for k, v in rates.items() if v < 0.9]
        print(f"\n⚠️ VULNERABLE to shocks at: {vulnerable}")

    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c1646_perturbation_resistance_results.json", 'w') as f:
        json.dump({
            "cycle_id": CYCLE_ID,
            "attack_mult": ATTACK_MULT,
            "magnitude": MAGNITUDE,
            "perturbation_cycle": PERTURBATION_CYCLE,
            "reduction": REDUCTION,
