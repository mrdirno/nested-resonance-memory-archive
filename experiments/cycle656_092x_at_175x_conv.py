#!/usr/bin/env python3
"""
CYCLE 656: 0.92× ATTACK AT 1.75× CONVERSION
...
Purpose:
  Test if 0.92× recovers from 65% collapse.
"""
...
SEEDS = list(range(11101, 11121))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c656_seed{seed}.db", n_populations=n_levels, mode="092X_175XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 656: 0.92× + 1.75× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c656_092x_at_175x_conv_results.json", 'w') as f:
