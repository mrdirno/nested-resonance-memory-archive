#!/usr/bin/env python3
"""
CYCLE 1766: FIXED 50% RESONANCE
Test if dead zones emerge with completely random resonance (50% match rate).
This removes all structure from resonance calculation.
"""
    ...
    for cycle in range(CYCLES):
        ...
        for d in range(N_DEPTHS - 1):
            ...
            while i < len(agents) - 1:
                # Fixed match rate - no phase calculation
...
def main():
    print(f"CYCLE 1766: Fixed Resonance | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 70)
    print("Testing fixed match rates (no phase calculation)")
    print("=" * 70)

    seeds = list(range(1766001, 1766021))
    zone1_range = range(25, 34)

    print(f"\n{'Match Rate':<12} | {'Zone 1 Min':>15}")
    print("-" * 32)
    ...
    for rate in [0.3, 0.5, 0.7]:
        ...
        print(f"{rate*100:.0f}%{'':8} | {min_n} ({min_c:.0f}%)")
