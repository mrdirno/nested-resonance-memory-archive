#!/usr/bin/env python3
"""
CYCLE 908: 0.92× ATTACK AT 4.95× CONVERSION - PRE-5.0× MILESTONE
"""
...
BASE_PARAMS = [
    ...
    {"attack": 0.00276, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 1.485},
    {"attack": 0.0046, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 1.2375},
    {"attack": 0.00736, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.99},
    {"attack": 0.01104, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.7425},
    {"attack": 0.0138, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.594},
    {"attack": 0.01656, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.495}
]
SEEDS = list(range(16141, 16161))

def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c908_seed{seed}.db", n_populations=n_levels, mode="092X_495XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 908: 0.92× + 4.95× conv (PRE-5.0×) | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c908_092x_at_495x_conv_results.json", 'w') as f:
        json.dump({"cycle_id": CYCLE_ID, "attack_multiplier": 0.92, "conversion_multiplier": 4.95, "coexistence_rate": coexist_count/len(results), "results": results}, f, indent=2)
