#!/usr/bin/env python3
"""
CYCLE 635: 0.89× ATTACK AT 1.5× CONVERSION
...
SEEDS = list(range(10681, 10701))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c635_seed{seed}.db", n_populations=n_levels, mode="089X_150XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 635: 0.89× + 1.5× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c635_089x_at_150x_conv_results.json", 'w') as f:
