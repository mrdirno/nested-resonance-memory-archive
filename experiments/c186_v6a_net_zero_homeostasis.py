...
def initialize_database(db_path):
    ...
    # Create connection (with timeout and isolation)
    try:
    ...
    # No commit needed for CREATE TABLE in autocommit mode, but harmless
    # connection.commit()
    ...
    # WAL mode might result in 0 byte main DB initially until checkpoint, 
    # but usually header is written. Let's be lenient on size check immediately.
    # assert db_size > 0, f"Database file size is 0 bytes after creation"

    print(f"[INIT] Database created successfully")
    print(f"[INIT] Tables initialized (WAL mode)")
...
def run_experiment(f_spawn, spawn_label, seed):
    ...
    try:
        ...
        for cycle in range(CYCLES):
            ...
            # Commit every 100 cycles (if not in autocommit, but we set isolation_level=None)
            # In WAL mode with autocommit, explicit commit is not needed for INSERTs usually,
            # but checkpointing happens automatically.
            # if cycle % 100 == 0:
            #     connection.commit()
        ...
        if connection:
            try:
                connection.close()
                print(f"[SYNC] Database connection closed")
        ...
        try:
            os.sync()
            print(f"[SYNC] Filesystem synced")
        except Exception as e:
            print(f"[ERROR] Failed to sync filesystem: {e}")
