#!/usr/bin/env python3
"""
CYCLE 393: ULTRA-LOW ATTACK + 4.0× CONVERSION - 40 CYCLES
...
Purpose:
  Test if 4.0× conversion enables 0.25× attack to achieve high survival.

Background:
  C386: 0.25× attack + 3.0× conv - 65%
  C393: 0.25× attack + 4.0× conv - expect >65%?

Design:
  - K declines from 600 to 200 over 40 cycles
  - Attack: 0.25×, Conversion: 4.0×
  - 20 seeds
"""
...
CYCLE_ID = "C393"
CYCLE_NAME = "Ultra-Low Attack 4x Conv"
MODE = "ULOW_ATK_4XCONV"
...
# 0.25× attack + 4.0× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.00075, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 1.2},
    {"attack": 0.00125, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 1.0},
    {"attack": 0.002, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.8},
    {"attack": 0.003, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.6},
    {"attack": 0.00375, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.48},
    {"attack": 0.0045, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.4}
]

SEEDS = list(range(5840, 5860))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run ultra-low attack + 4.0× conversion experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c393_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 393: ULTRA-LOW ATTACK + 4.0× CONVERSION - 40 CYCLES")
    ...
    print(f"Attack: 0.25×, Conversion: 4.0×")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c393_ultralow_attack_4x_conv_results.json"
