#!/usr/bin/env python3
"""
CYCLE 389: OPTIMAL CONFIGURATION - SLOW DECLINE (60 CYCLES)
...
Purpose:
  Test if optimal configuration maintains 100% at slower K decline.

Background:
  C388: 0.5× attack + 2.0× conv at 20 cycles - 85%
  C380: 0.5× attack + 2.0× conv at 40 cycles - 100%
  C389: Same configuration at 60 cycles - expect 100%?

Design:
  - K declines from 600 to 200 over 60 cycles (slow)
  - Attack: 0.5×, Conversion: 2.0×
  - 20 seeds
"""
...
CYCLE_ID = "C389"
CYCLE_NAME = "Optimal Slow Decline"
MODE = "OPT_SLOW_DECLINE"

CYCLES = 30000
K_START = 600
K_END = 200
DECLINE_CYCLES = 60  # Slow decline
INITIAL = [300, 30, 10, 5, 3, 2, 2]

# 0.5× attack + 2.0× conversion (optimal from C380)
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.0015, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.6},
    {"attack": 0.0025, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.5},
    {"attack": 0.004, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.4},
    {"attack": 0.006, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.3},
    {"attack": 0.0075, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.24},
    {"attack": 0.009, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.2}
]

SEEDS = list(range(5760, 5780))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run optimal config at slow decline."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c389_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 389: OPTIMAL CONFIGURATION - SLOW DECLINE (60 CYCLES)")
    ...
    print(f"K: {K_START} → {K_END} over {DECLINE_CYCLES} cycles (slow)")
    print(f"Attack: 0.5×, Conversion: 2.0×")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c389_optimal_slow_decline_results.json"
