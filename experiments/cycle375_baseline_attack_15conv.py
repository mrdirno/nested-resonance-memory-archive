#!/usr/bin/env python3
"""
CYCLE 375: BASELINE ATTACK + 1.5× CONVERSION - 40 CYCLES
...
Purpose:
  Confirm that baseline attack + high conversion is better than combined high.

Background:
  C371: 1.5× conv alone - 80%
  C374: 1.25× attack + 1.5× conv - 55%
  C375: Replicate C371 to confirm

Design:
  - K declines from 600 to 200 over 40 cycles
  - Attack: 1.0×, Conversion: 1.5×
  - 20 seeds
"""
...
CYCLE_ID = "C375"
CYCLE_NAME = "Baseline Attack 1.5x Conv"
MODE = "BASELINE_ATK_15CONV"
...
# Baseline attack + 1.5× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.003, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.45},
    {"attack": 0.005, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.375},
    {"attack": 0.008, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.3},
    {"attack": 0.012, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.225},
    {"attack": 0.015, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.18},
    {"attack": 0.018, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.15}
]

SEEDS = list(range(5480, 5500))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run baseline attack + 1.5× conversion experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c375_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 375: BASELINE ATTACK + 1.5× CONVERSION - 40 CYCLES")
    ...
    print(f"Attack: 1.0×, Conversion: 1.5×")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c375_baseline_attack_15conv_results.json"
