#!/usr/bin/env python3
"""
CYCLE 182: H2+H5 FACTORIAL EXPERIMENT - External Sources × Multi-Generational Recovery

Purpose: Test synergistic effects of combining H2 (External Sources) with H5 (Multi-Generational Recovery)
...
Background:
  ...
  - C177: External sources (H1) tested independently
...
Synergy Prediction:
  Combined H2+H5 should produce super-additive effects:
  ...
  - Predicted: Effect(H2+H5) > Effect(H1) + Effect(H5)

Experimental Design (2×2 Factorial):
  4 Conditions:
    1. BASELINE: Neither H1 nor H5 (no pooling, no multi-generational coordination)
    2. H2-only: External sources enabled (α=0.10), no temporal coordination
    3. H5-only: No external sources, multi-generational coordination (offset=300)
    4. H2+H5: Both mechanisms active (pooling α=0.10, offset=300)
...
Statistical Analysis:
  ...
  - Main effect of H1, Main effect of H5, Interaction (H2×H5)
  - Post-hoc: Tukey HSD pairwise comparisons (Bonferroni α=0.0083)
  - Effect sizes: Cohen's d for all pairwise comparisons
  - Synergy Index: SI = [Effect(H2+H5) - (Effect(H1) + Effect(H5))] / Effect(BASELINE)
...
Cycle: 180 (Paper 3 experimental battery, combination 5 of 6)
...
# Experimental parameters (C182: H2+H5 factorial design)
...
class FactorialCondition:
    ...
    def __init__(self, name: str, external_sources_enabled: bool, multigenerational_enabled: bool):
        """
        Initialize factorial condition.

        Args:
            name: Condition name (BASELINE, H2-only, H5-only, H2+H5)
            external_sources_enabled: Whether H1 (external sources) is active
    ...
    def __repr__(self):
        return f"FactorialCondition({self.name}, H1={self.external_sources_enabled}, H5={self.multigenerational_enabled})"
...
def run_single_experiment(
    ...
    """
    Run single factorial experiment (H2×H5).

    Args:
        condition: Factorial condition (BASELINE, H2-only, H5-only, H2+H5)
    ...
    for cycle_idx in range(cycles):
        ...
        # 3. Composition detection (death through clustering)
...
def main():
    """Run C182 H2×H5 factorial experiment."""
    print("=" * 80)
    print("CYCLE 182: H2×H5 FACTORIAL EXPERIMENT - External Sources × Multi-Generational Recovery")
    print("=" * 80)
    print()
    print("Purpose: Test synergistic effects of H2+H5 combination")
    ...
    print("  2. H2-only: External sources (α=0.10), no multi-generational coordination")
    print("  3. H5-only: Multi-generational coordination (offset=300), no pooling")
    print("  4. H2+H5: Both mechanisms active")
    print()
    ...
    conditions = [
        FactorialCondition("BASELINE", external_sources_enabled=False, multigenerational_enabled=False),
        FactorialCondition("H2-only", external_sources_enabled=True, multigenerational_enabled=False),
        FactorialCondition("H5-only", external_sources_enabled=False, multigenerational_enabled=True),
        FactorialCondition("H2+H5", external_sources_enabled=True, multigenerational_enabled=True)
    ]
    ...
    for condition in conditions:
        ...
        desc_parts = []
        if condition.external_sources_enabled:
            desc_parts.append(f"External sources (α={EXTERNAL_REWARD})")
        if condition.multigenerational_enabled:
        ...
        if condition.external_sources_enabled:
            pooling_pools = [r['external_sources_stats']['total_rewards_given'] for r in condition_results]
            pooling_energy = [r['external_sources_stats']['total_energy_rewarded'] for r in condition_results]
    ...
    print(f"  POOLING (α={EXTERNAL_REWARD:.2f}) | {pooling_mean:8.2f} | {pooling_std:7.2f} | {pooling_cv:6.1f}% | "
    ...
    if pooling_mean > baseline_mean * 2:
        print("⚠️  SIGNIFICANT IMPROVEMENT: External sources substantially increases population")
        ...
        print("⚠️  MODERATE IMPROVEMENT: External sources partially overcomes bottleneck")
        print("    → May require synergistic combination with H2/H3/H4/H5")
    else:
        print("⚠️  NO SIGNIFICANT EFFECT: External sources insufficient")
    ...
    results_file = output_dir / "cycle182_h2h5_factorial_results.json"

    output = {
        'metadata': {
            'cycle': '182',
            'experiment': 'H2×H5 Factorial - External Sources × Multi-Generational Recovery',
