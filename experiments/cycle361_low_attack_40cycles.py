#!/usr/bin/env python3
"""
CYCLE 361: LOW ATTACK RATE - 40 CYCLES
...
Purpose:
  Test parameter sensitivity - low attack rates.

Background:
  C359: 40 cycles with base attack - 50% survival
  C360: 40 cycles with 1.5× attack - 20% survival
  C361: 40 cycles with 0.5× attack - expect higher survival

Hypothesis:
  Lower attack rates → need faster K decline (lower critical rate)

Design:
  - K declines from 600 to 200 over 40 cycles
  - Rate: 10 K/cycle
  - Attack rates: 0.5× baseline
  - 20 seeds
"""
...
CYCLE_ID = "C361"
CYCLE_NAME = "Low Attack Rate 40 Cycles"
MODE = "LOW_ATTACK_40"
...
BASE_PARAMS = [
    ...
    {"attack": 0.0015, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.3},
    {"attack": 0.0025, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.25},
    {"attack": 0.004, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.2},
    {"attack": 0.006, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.15},
    {"attack": 0.0075, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.12},
    {"attack": 0.009, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.10}
]

SEEDS = list(range(5200, 5220))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run low attack rate experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c361_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 361: LOW ATTACK RATE - 40 CYCLES")
    ...
    print(f"Attack rates: 0.5× baseline")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c361_low_attack_40cycles_results.json"
