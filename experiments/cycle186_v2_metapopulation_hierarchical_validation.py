#!/usr/bin/env python3
"""
CYCLE 186 V2: META-POPULATION HIERARCHICAL RESONANCE VALIDATION (REVISED PARAMETERS)

Purpose: Test hierarchical energy dynamics with REVISED spawn rate to exceed viability threshold

Background - C186 V1 Failure Analysis:
  - C186 V1 Results: 100% Basin B collapse (score 7.0/12.0)
  - Root Cause: f_intra = 2.5% BELOW viability threshold for metapopulations
  - Key Finding: Energy compartmentalization increases bootstrap threshold 2×
  - Single populations (C171): f_spawn = 2.5% → 100% Basin A (viable)
  - Metapopulations (C186): f_intra = 2.5% → 100% Basin B (collapsed)
  - Hypothesis: Metapopulations require f_intra ≥ 5.0% to bootstrap and sustain

C186 V2 Revision Rationale:
  - Increase f_intra: 2.5% → 5.0% (2× increase)
  - Keep f_migrate: 0.5% (constant to isolate spawn rate effect)
  - Keep all other parameters constant (cycles, seeds, populations)
  - Test: Can higher spawn rate allow populations to bootstrap from 0-1 agents?
  - Expected: At least SOME populations reach Basin A (>0% vs. 0% in C186 V1)

Theoretical Predictions (Revised):
  ...
  - NEW: Metapopulation viability requires f_intra ≥ 5.0% (not 2.5%)
  - Two-level hierarchy:
...
Experimental Design:
  ...
  - Intra-population spawn frequency: f_intra = 5.0% (REVISED, 2× C186 V1)
  - Inter-population migration: f_migrate = 0.5% (cross-population agent transfer)
  - Cycles: 3000 (match C186 V1 duration)
  - Seeds: n=10 (robust statistical validation, same as C186 V1)
  - Max agents per population: 100 (same as C171/C175/C177/C186 V1)
...
Expected Outcomes (C186 V2):
  1. Intra-population homeostasis: At least SOME populations should reach Basin A (>0%)
  2. Bootstrap success: Populations escape 0-1 agent flickering state
  3. Inter-population regulation: Migration redistributes agents when populations diverge
  4. Meta-stability: Total swarm population more stable than individual populations
  5. Energy cascades: Population energy correlates with population size

Success Criteria (C186 V2 vs. C186 V1):
  - C186 V1: Basin A = 0% (all populations collapsed)
  - C186 V2 Target: Basin A > 0% (at least some populations sustain)
  - Threshold: If Basin A ≥ 50%, viability hypothesis validated
  - If Basin A < 50%, may need f_intra ≥ 10.0% (further increase)

Publication Value:
  - Tests viability boundary hypothesis (f_intra < 5.0% unviable, ≥5.0% viable)
  - Comparative validation (C186 V1 vs. V2 with single parameter change)
  - Establishes metapopulation bootstrap threshold in NRM framework
  - Negative result (C186 V1) + revised test (C186 V2) = scientific method in action

Date: 2025-11-05 (Cycle 1041 Implementation)
Researcher: Claude (DUALITY-ZERO-V2)
Author: Aldrin Payopay <aldrin.gdf@gmail.com>
Cycle: 186 V2 (Revision based on Cycle 1040 failure analysis)
Reference: CYCLE186_FAILURE_ANALYSIS.md (12,500 words, Cycle 1040)
"""
...
F_INTRA = 5.0  # Intra-population spawn frequency (REVISED from 2.5% to 5.0%)
F_MIGRATE = 0.5  # Inter-population migration frequency (constant)
SEEDS = [42, 123, 456, 789, 101, 202, 303, 404, 505, 606]  # n=10 seeds (same as C186 V1)
CYCLES = 3000
BASIN_THRESHOLD = 2.5
WINDOW_SIZE = 100
...
def run_metapopulation_experiment(seed: int, cycles: int) -> dict:
    ...
    population_results = []
    for pop in populations:
        # Calculate composition rate (same as C171/C175/C177/C186 V1)
    ...
    return {
        ...
        'implementation': 'MetaPopulation_V2'
    }


def main():
    """Execute Cycle 186 V2 meta-population hierarchical validation."""
    print("=" * 80)
    print("CYCLE 186 V2: META-POPULATION HIERARCHICAL VALIDATION (REVISED)")
    print("=" * 80)
    print()
    print("Purpose: Test hierarchical energy dynamics with REVISED spawn rate")
    print("Background: CYCLE186_FAILURE_ANALYSIS.md (C186 V1: 100% Basin B collapse)")
    print()
    print("C186 V1 FAILURE:")
    print(f"  - f_intra = 2.5% → 100% Basin B (all populations collapsed)")
    print(f"  - Root cause: Below viability threshold for metapopulations")
    print(f"  - Energy compartmentalization increases bootstrap threshold 2×")
    print()
    print("C186 V2 REVISION:")
    print(f"  - f_intra = 2.5% → {F_INTRA:.2f}% (2× increase)")
    print(f"  - f_migrate = {F_MIGRATE:.2f}% (constant)")
    print(f"  - Hypothesis: ≥5.0% allows populations to bootstrap and sustain")
    print(f"  - Expected: Basin A > 0% (at least some populations sustain)")
    print()
    ...
    print(f"  C186 V1 (f_intra=2.5%): 0.0% Basin A (100% collapsed)")
    print(f"  C186 V2 (f_intra={F_INTRA:.1f}%): {overall_basin_a_pct:.1f}% Basin A")

    if overall_basin_a_pct >= 50:
        print("  ✅ VALIDATED: Viability hypothesis confirmed (≥50% Basin A)")
    elif overall_basin_a_pct > 0:
        print("  ⚠️  PARTIAL: Some populations sustain, but <50% threshold")
    else:
        print("  ❌ FAILED: Still 100% Basin B collapse (f_intra=5.0% insufficient)")
    ...
    # Comparative analysis (C186 V1 vs. V2)
    print("=" * 80)
    print("C186 V1 vs. V2 COMPARATIVE ANALYSIS")
    print("=" * 80)
    print()
    print("Parameter Change:")
    print(f"  f_intra: 2.5% → {F_INTRA:.1f}% (2× increase)")
    print()
    print("Basin A Classification:")
    print(f"  C186 V1: 0.0% Basin A (100% collapsed)")
    print(f"  C186 V2: {overall_basin_a_pct:.1f}% Basin A")
    print(f"  Improvement: +{overall_basin_a_pct:.1f} percentage points")
    print()

    if overall_basin_a_pct >= 50:
        print("✅ VIABILITY HYPOTHESIS VALIDATED:")
        print("   Metapopulations require f_intra ≥ 5.0% for bootstrap")
        print("   C186 V2 data support revised threshold")
    elif overall_basin_a_pct > 0:
        print("⚠️  VIABILITY HYPOTHESIS PARTIALLY SUPPORTED:")
        print("   Some improvement, but <50% threshold")
        print("   May need f_intra ≥ 10.0% for robust viability")
    else:
        print("❌ VIABILITY HYPOTHESIS REJECTED:")
        print("   f_intra = 5.0% still insufficient")
        print("   Need higher spawn rate (test f_intra ≥ 10.0%)")
    print()
    ...
    output_data = {
        'metadata': {
            'cycle': '186_V2',
            'scenario': 'Meta-Population Hierarchical Validation (Revised Parameters)',
            'date': start_time.isoformat(),
            'revision_rationale': 'C186 V1 100% Basin B collapse → increase f_intra to exceed viability threshold',
            ...
            'purpose': 'Test viability boundary hypothesis (f_intra ≥ 5.0%)',
            'reference': 'CYCLE186_FAILURE_ANALYSIS.md (Cycle 1040)',
        },
        'experiments': results,
        'aggregate_analysis': {
        ...
        'comparative_v1_v2': {
            'basin_a_v1': 0.0,
            'basin_a_v2': float(overall_basin_a_pct),
            'improvement': float(overall_basin_a_pct - 0.0),
        }
    }

    output_path = Path(__file__).parent / "results" / "cycle186_v2_metapopulation_hierarchical_validation.json"
