...
# -----------------------------------------------------------------------------
# HELIOS LAMP SERIES V32: THE BRIDGE (SHADE)
# -----------------------------------------------------------------------------
# Concept: Keystone Arch (Vaulted Ceiling).
# Math: Catenary Arches rotated.
# -----------------------------------------------------------------------------

def generate_shade(output_path, diameter=200.0, height=140.0, resolution=100, hole_diameter=14.0):
    print(f"Generating V32 SHADE (Keystone Arch): {output_path}")
        ...
        for x_idx in range(res_x):
            ...
            for y_idx in range(res_y):
                ...
                if in_outer and not in_inner and not in_hand:
                    ...
                    # We simulate this with a Gyroid that has "Arched" cells
                    # Distort Z coordinate to bend the grid downwards
                    
                    sag = 10.0 * math.cos(dist_xy * 0.1)
                    
                    val = math.sin(x_mm * base_scale) * math.cos(y_mm * base_scale) + \
                          math.sin(y_mm * base_scale) * math.cos((z_mm + sag) * base_scale) + \
                          math.sin((z_mm + sag) * base_scale) * math.cos(x_mm * base_scale)
                    
                    # Add structural ribs
                    # Ribs radiating from center?
                    angle = math.atan2(y_mm, x_mm)
                    ribs = math.sin(angle * 12.0)
                    
                    if abs(val) < 0.5 or ribs > 0.9:
                        grid[x_idx,y_idx,z_idx] = True
                    else:
