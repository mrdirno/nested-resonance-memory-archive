...
# -----------------------------------------------------------------------------
# HELIOS LAMP SERIES V19: THE SINGULAR (SHAFT)
# -----------------------------------------------------------------------------
# Concept: Monolith Riser.
# Math: Vertical Gyroid Flow.
# -----------------------------------------------------------------------------

def generate_shaft(output_path, height=160.0, resolution=120):
    print(f"Generating V19 SHAFT (Monolith Riser): {output_path}")
    ...
    for z_idx in range(res_z):
        ...
        for x_idx in range(res_xy):
            ...
            for y_idx in range(res_xy):
                ...
                if dist <= base_radius:
                    # Vertically aligned gyroid
                    val = math.sin(x_mm * base_scale) * math.cos(y_mm * base_scale) + \
                          math.sin(y_mm * base_scale) * math.cos(z_mm * base_scale * 0.1) + \
                          math.sin(z_mm * base_scale * 0.1) * math.cos(x_mm * base_scale)
