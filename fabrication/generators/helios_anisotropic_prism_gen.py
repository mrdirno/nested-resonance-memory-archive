...
def generate_anisotropic_gyroid_prism(output_path, plate_width=200.0, plate_depth=200.0, margin_xy=25.4, size_z=120.0, resolution=120, k_mod=0.01, robust_base_height=25.4, top_extend_height=0.0, k_expansion_shape=0.0, expand_outward=True):
    """
    RESTORED & OPTIMIZED VERSION
    Generates a 3D mesh representing an Anisotropic Gyroid Prism (Pyramid/Frustum).
    Uses Binary STL for speed.
    Uses simple scaling factor k_expansion_shape.
    """
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python helios_anisotropic_prism_gen.py <output.stl> ...")
    else:
        output_file = sys.argv[1]
        generate_anisotropic_gyroid_prism(
            output_file, 
            plate_width=float(sys.argv[2]), 
            plate_depth=float(sys.argv[3]),
            margin_xy=float(sys.argv[4]),
            size_z=float(sys.argv[5]),
            resolution=int(sys.argv[6]),
            k_mod=float(sys.argv[7]),
            robust_base_height=float(sys.argv[8]),
            top_extend_height=float(sys.argv[9]),
            k_expansion_shape=float(sys.argv[10])
        )