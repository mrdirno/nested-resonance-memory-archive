...
# -----------------------------------------------------------------------------
# HELIOS LAMP SERIES V23: THE GLITCH (SHADE)
# -----------------------------------------------------------------------------
# Concept: Compression Artifact (JPEG Blocks).
# Math: Quantized Domain Warping.
# -----------------------------------------------------------------------------

def generate_shade(output_path, diameter=200.0, height=140.0, resolution=100, hole_diameter=14.0):
    print(f"Generating V23 SHADE (Compression Artifact): {output_path}")
        ...
        for x_idx in range(res_x):
            ...
            for y_idx in range(res_y):
                ...
                if in_outer and not in_inner and not in_hand:
                    # Compression Logic
                    # Divide space into blocks (e.g. 8x8mm)
                    # Apply random offset per block
                    
                    block_size = 10.0
                    bx = int(x_mm / block_size)
                    by = int(y_mm / block_size)
                    bz = int(z_mm / block_size)
                    
                    # Hash function for random offset
                    h = (bx * 73856093) ^ (by * 19349663) ^ (bz * 83492791)
                    offset = (h % 100) / 10.0 # 0.0 to 10.0
                    
                    # Only shift X
                    ox = x_mm + offset
                    
                    val = math.sin(ox * base_scale) * math.cos(y_mm * base_scale) + \
                          math.sin(y_mm * base_scale) * math.cos(z_mm * base_scale) + \
                          math.sin(z_mm * base_scale) * math.cos(ox * base_scale)
