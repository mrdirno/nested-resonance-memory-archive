...
# -----------------------------------------------------------------------------
# HELIOS LAMP SERIES V17: THE SIMULATION (SHADE)
# -----------------------------------------------------------------------------
# Concept: Wireframe Shell (Polygon Edges).
# Math: Lattice with very thin struts, derived from voronoi/gyroid mix.
# -----------------------------------------------------------------------------

def generate_shade(output_path, diameter=200.0, height=140.0, resolution=100, hole_diameter=14.0):
    print(f"Generating V17 SHADE (Wireframe): {output_path}")
        ...
        for x_idx in range(res_x):
            ...
            for y_idx in range(res_y):
                ...
                if in_outer and not in_inner and not in_hand:
                    # Wireframe Look
                    # Use `abs(val) < thickness` but make thickness very small
                    # And use a sharper function like `cos(x)*cos(y)*cos(z)`?
                    # Or Schwartz P with low threshold.
                    
                    val = math.cos(x_mm * base_scale) + math.cos(y_mm * base_scale) + math.cos(z_mm * base_scale)
                    
                    if abs(val) < 0.6: # Very thin
                        grid[x_idx,y_idx,z_idx] = True
                    else:
