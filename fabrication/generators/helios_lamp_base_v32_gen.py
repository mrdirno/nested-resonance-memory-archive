...
# -----------------------------------------------------------------------------
# HELIOS LAMP SERIES V32: THE BRIDGE (BASE)
# -----------------------------------------------------------------------------
# Concept: Foundation Pier (Heavy Engineering).
# Math: Reinforced Grid / Buttress.
# -----------------------------------------------------------------------------

def generate_base(output_path, diameter=140.0, height=35.0, resolution=100):
    print(f"Generating V32 BASE (Foundation Pier): {output_path}")
        ...
        for x_idx in range(res_xy):
            ...
            for y_idx in range(res_xy):
                ...
                if dist <= radius:
                    ...
                    # Bridge Pier Logic
                    # Vertical stress lines (Z-aligned)
                    # Cross-bracing (X/Y aligned)
                    
                    # We use a Gyroid but stretched to look like bracing
                    
                    val = math.sin(x_mm * base_scale) * math.cos(y_mm * base_scale) + \
                          math.sin(y_mm * base_scale) * math.cos(z_mm * base_scale * 0.5) + \
                          math.sin(z_mm * base_scale * 0.5) * math.cos(x_mm * base_scale)
                    
                    # Add "Rebar" lines
                    # Grid of vertical cylinders
                    rebar = (math.sin(x_mm * 0.5) > 0.9) and (math.sin(y_mm * 0.5) > 0.9)
                    
                    if abs(val) < 0.6 or rebar:
                        grid[x_idx,y_idx,z_idx] = True
                    else:
                        grid[x_idx,y_idx,z_idx] = False
                else:
