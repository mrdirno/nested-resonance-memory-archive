...
# -----------------------------------------------------------------------------
# HELIOS LAMP SERIES V31: THE END (BASE)
# -----------------------------------------------------------------------------
# Concept: Impact Crater.
# Math: Crater function + Voronoi cracks.
# -----------------------------------------------------------------------------

def generate_base(output_path, diameter=140.0, height=35.0, resolution=100):
    print(f"Generating V31 BASE (Impact Crater): {output_path}")
        ...
        for x_idx in range(res_xy):
            ...
            for y_idx in range(res_xy):
                ...
                if dist <= radius:
                    ...
                    # Crater Logic
                    # Crater depth modulation
                    # Deep in center, rising to a rim, then falling off
                    
                    crater_shape = -10.0 * math.exp(-(dist*dist)/800.0) + 5.0 * math.exp(-(dist-30)**2/200.0)
                    
                    z_rel = z_mm - (height/2.0 + crater_shape)
                    
                    # Cracks (Voronoi-ish)
                    # Simple sin interference acting as cracks
                    c1 = math.sin(x_mm * 0.2)
                    c2 = math.sin(y_mm * 0.2)
                    c3 = math.sin((x_mm+y_mm)*0.15)
