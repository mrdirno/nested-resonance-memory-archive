...
# -----------------------------------------------------------------------------
# HELIOS LAMP SERIES V16: THE TESSERACT (SHAFT)
# -----------------------------------------------------------------------------
# Concept: Unfolding (Nested Cubes).
# Math: Quantized Rotation.
# -----------------------------------------------------------------------------

def generate_shaft(output_path, height=160.0, resolution=120):
    print(f"Generating V16 SHAFT (Unfolding): {output_path}")
    ...
    for z_idx in range(res_z):
        ...
        # Discrete Rotation steps
        # Rotate 90 degrees every 40mm
        block_h = 40.0
        block_idx = int(z_mm / block_h)
        angle = block_idx * (math.pi / 4.0) # 45 degree steps
        ...
        for x_idx in range(res_xy):
            ...
            for y_idx in range(res_xy):
                ...
                if dist <= base_radius:
                    # Rotate coords by discrete angle
                    rx = x_mm * math.cos(angle) - y_mm * math.sin(angle)
                    ry = x_mm * math.sin(angle) + y_mm * math.cos(angle)
                    
                    # Cubic P-Surface
                    val = math.cos(rx * base_scale) + math.cos(ry * base_scale) + math.cos(z_mm * base_scale)
