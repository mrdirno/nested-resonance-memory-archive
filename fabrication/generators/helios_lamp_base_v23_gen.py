...
# -----------------------------------------------------------------------------
# HELIOS LAMP SERIES V23: THE GLITCH (BASE)
# -----------------------------------------------------------------------------
# Concept: Pixel Sort (Sorted Voxels).
# Math: Gyroid with Vertical Sorting bias.
# -----------------------------------------------------------------------------

def generate_base(output_path, diameter=140.0, height=35.0, resolution=100):
    print(f"Generating V23 BASE (Pixel Sort): {output_path}")
        ...
        for x_idx in range(res_xy):
            ...
            for y_idx in range(res_xy):
                ...
                if dist <= radius:
                    ...
                    # Pixel Sort Logic
                    # We simulate sorting by stretching high-value regions vertically
                    # and compressing low-value regions
                    
                    # Noise determines "Sort Value"
                    sort_val = math.sin(x_mm*0.1) * math.cos(y_mm*0.1)
                    
                    # If sort_val is high, we stretch Z
                    # If low, we compress Z or leave it
                    
                    z_warp = z_mm
                    if sort_val > 0.5:
                        z_warp = z_mm * 0.2 # Streak vertically
                    
                    val = math.sin(x_mm * base_scale) * math.cos(y_mm * base_scale) + \
                          math.sin(y_mm * base_scale) * math.cos(z_warp * base_scale) + \
                          math.sin(z_warp * base_scale) * math.cos(x_mm * base_scale)
