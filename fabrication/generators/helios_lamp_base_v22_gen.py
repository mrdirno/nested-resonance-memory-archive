...
# -----------------------------------------------------------------------------
# HELIOS LAMP SERIES V22: THE NOISE (BASE)
# -----------------------------------------------------------------------------
# Concept: Static Field (High-Freq Coherent Noise).
# Math: High-Frequency Perlin-ish noise modulating a Gyroid.
# -----------------------------------------------------------------------------

def generate_base(output_path, diameter=140.0, height=35.0, resolution=100):
    print(f"Generating V22 BASE (Static Field): {output_path}")
    ...
    noise_scale = 2.0 * math.pi / 5.0 # High frequency (5mm wavelength)
        ...
        for x_idx in range(res_xy):
            ...
            for y_idx in range(res_xy):
                ...
                if dist <= radius:
                    ...
                    # High Freq Noise
                    n_val = math.sin(x_mm * noise_scale) * math.sin(y_mm * noise_scale) * math.sin(z_mm * noise_scale)
                    
                    # Modulate Gyroid with Noise
                    # This breaks the smooth surface into "static"
                    total = g_val + (n_val * 0.8)
