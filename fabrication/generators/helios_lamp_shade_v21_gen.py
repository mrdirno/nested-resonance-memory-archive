...
# -----------------------------------------------------------------------------
# HELIOS LAMP SERIES V21: THE FLUID (SHADE)
# -----------------------------------------------------------------------------
# Concept: Surface Tension (Metaballs).
# Math: Blobby Isosurface.
# -----------------------------------------------------------------------------

def generate_shade(output_path, diameter=200.0, height=140.0, resolution=100, hole_diameter=14.0):
    print(f"Generating V21 SHADE (Surface Tension): {output_path}")
    ...
    base_scale = 2.0 * math.pi / 35.0 # Large blobs
        ...
        for x_idx in range(res_x):
            ...
            for y_idx in range(res_y):
                ...
                if in_outer and not in_inner and not in_hand:
                    # Blobby Logic
                    # Simplex-ish or just smooth Gyroid
                    ...
                    # Threshold creates large, rounded holes
                    if abs(val) < 0.5:
                        grid[x_idx,y_idx,z_idx] = True
                    else:
