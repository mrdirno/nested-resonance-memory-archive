...
# -----------------------------------------------------------------------------
# HELIOS LAMP SERIES V29: THE AIR (SHADE)
# -----------------------------------------------------------------------------
# Concept: Cloud Form (Cumulus).
# Math: Volumetric Noise (FBM).
# -----------------------------------------------------------------------------

def generate_shade(output_path, diameter=200.0, height=140.0, resolution=100, hole_diameter=14.0):
    print(f"Generating V29 SHADE (Cloud Form): {output_path}")
        ...
        for x_idx in range(res_x):
            ...
            for y_idx in range(res_y):
                ...
                if in_outer and not in_inner and not in_hand:
                    # Cloud Logic
                    # Low frequency Gyroid + Medium Frequency Noise
                    ...
                    # Turbulent Noise
                    n_scale = base_scale * 2.5
                    noise = math.sin(x_mm * n_scale) * math.sin(y_mm * n_scale) * math.sin(z_mm * n_scale)
                    
                    total = g_val + (noise * 0.5)
                    
                    # Soft, fluffy threshold
                    if abs(total) < 0.6:
                        grid[x_idx,y_idx,z_idx] = True
                    else:
