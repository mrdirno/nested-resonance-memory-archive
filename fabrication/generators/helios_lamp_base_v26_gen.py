...
# -----------------------------------------------------------------------------
# HELIOS LAMP SERIES V26: THE IMPOSSIBLE (BASE)
# -----------------------------------------------------------------------------
# Concept: Penrose Steps (Infinite Staircase).
# Math: Spiral Step function with height modulus.
# -----------------------------------------------------------------------------

def generate_base(output_path, diameter=140.0, height=35.0, resolution=100):
    print(f"Generating V26 BASE (Penrose Steps): {output_path}")
        ...
        for x_idx in range(res_xy):
            ...
            for y_idx in range(res_xy):
                ...
                if dist <= radius:
                    ...
                    # Ideal height of the step at this angle
                    step_height = norm_angle * 10.0 # 10mm rise per loop
                    
                    # Modulo distance from current Z
                    # (z - step_height) % block_height
                    
                    block_h = 10.0
                    
                    # Warp Z based on angle
                    z_warp = (z_mm - step_height) % block_h
                    
                    # Create steps using Gyroid on warped Z
