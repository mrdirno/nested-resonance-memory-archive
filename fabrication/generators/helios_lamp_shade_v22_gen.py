...
# -----------------------------------------------------------------------------
# HELIOS LAMP SERIES V22: THE NOISE (SHADE)
# -----------------------------------------------------------------------------
# Concept: White Noise ( grainy shell).
# Math: High-Frequency Perlin Noise.
# -----------------------------------------------------------------------------

def generate_shade(output_path, diameter=200.0, height=140.0, resolution=100, hole_diameter=14.0):
    print(f"Generating V22 SHADE (White Noise): {output_path}")
        ...
        for x_idx in range(res_x):
            ...
            for y_idx in range(res_y):
                ...
                if in_outer and not in_inner and not in_hand:
                    # Noise Logic
                    # Simple coherent noise (product of sines)
                    n1 = math.sin(x_mm * noise_scale)
                    n2 = math.sin(y_mm * noise_scale)
                    n3 = math.sin(z_mm * noise_scale)
                    
                    val = n1 * n2 * n3
                    
                    # Thresholding random noise creates disconnected dust.
                    # But product of sines is connected if threshold is low enough.
                    # Or we can use Gyroid + Noise.
                    # Let's use Gyroid + Noise to ensure structure.
                    
                    g_val = math.sin(x_mm * 0.2) * math.cos(y_mm * 0.2) + \
                            math.sin(y_mm * 0.2) * math.cos(z_mm * 0.2) + \
                            math.sin(z_mm * 0.2) * math.cos(x_mm * 0.2)
