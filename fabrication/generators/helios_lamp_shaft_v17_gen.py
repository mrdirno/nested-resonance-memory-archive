...
# -----------------------------------------------------------------------------
# HELIOS LAMP SERIES V17: THE SIMULATION (SHAFT)
# -----------------------------------------------------------------------------
# Concept: Z-Fighting (Intersecting Planes).
# Math: Stacked Plateaus.
# -----------------------------------------------------------------------------

def generate_shaft(output_path, height=160.0, resolution=120):
    print(f"Generating V17 SHAFT (Z-Fighting): {output_path}")
    ...
    for z_idx in range(res_z):
        ...
        # Quantize Z to create "Plates"
        plate_h = 5.0
        z_quant = round(z_mm / plate_h) * plate_h
        ...
        for x_idx in range(res_xy):
            ...
            for y_idx in range(res_xy):
                ...
                if dist <= base_radius:
                    # Use Quantized Z for Gyroid calc -> Creates horizontal steps
                    val = math.sin(x_mm * base_scale) * math.cos(y_mm * base_scale) + \
                          math.sin(y_mm * base_scale) * math.cos(z_quant * base_scale) + \
                          math.sin(z_quant * base_scale) * math.cos(x_mm * base_scale)
