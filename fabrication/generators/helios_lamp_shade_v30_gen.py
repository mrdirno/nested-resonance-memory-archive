...
# -----------------------------------------------------------------------------
# HELIOS LAMP SERIES V30: THE AETHER (SHADE)
# -----------------------------------------------------------------------------
# Concept: Aether Wind (Triangular Waves).
# Math: Sharpened Gyroid.
# -----------------------------------------------------------------------------
...
def generate_shade(output_path, diameter=200.0, height=140.0, resolution=100, hole_diameter=14.0):
    print(f"Generating V30 SHADE (Aether Wind): {output_path}")
    ...
    for z_idx in range(res_z):
        ...
        for x_idx in range(res_x):
            ...
            for y_idx in range(res_y):
                ...
                # NO SOLID SKIN! Only pattern.
                if in_outer and not in_inner and not in_hand:
                    # Triangular Gyroid with Twist
                    ...
                    tri_val = tri_gyroid(rx, ry, z_mm, base_scale)
                    
                    # Standard Gyroid Backbone (for connectivity)
                    std_val = math.sin(rx * base_scale) * math.cos(ry * base_scale) + \
                              math.sin(ry * base_scale) * math.cos(z_mm * base_scale) + \
                              math.sin(z_mm * base_scale) * math.cos(rx * base_scale)
                              
                    val = (tri_val * 0.8) + (std_val * 0.2)
