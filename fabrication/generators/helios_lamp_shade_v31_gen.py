...
# -----------------------------------------------------------------------------
# HELIOS LAMP SERIES V31: THE END (SHADE)
# -----------------------------------------------------------------------------
# Concept: Broken Sky (Fissures).
# Math: Large scale cracks.
# -----------------------------------------------------------------------------

def generate_shade(output_path, diameter=200.0, height=140.0, resolution=100, hole_diameter=14.0):
    print(f"Generating V31 SHADE (Broken Sky): {output_path}")
        ...
        for x_idx in range(res_x):
            ...
            for y_idx in range(res_y):
                ...
                if in_outer and not in_inner and not in_hand:
                    # Crack Logic
                    # Low frequency domain warp + high threshold
                    
                    c1 = math.sin(x_mm * 0.05)
                    c2 = math.sin(y_mm * 0.05)
                    c3 = math.sin(z_mm * 0.05)
                    
                    crack_field = c1*c2*c3
                    ...
                    # If crack field is high, remove material
                    if abs(crack_field) > 0.8:
                        val = 10.0
