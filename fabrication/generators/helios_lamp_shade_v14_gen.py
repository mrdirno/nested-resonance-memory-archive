...
# -----------------------------------------------------------------------------
# HELIOS LAMP SERIES V14: THE GRAVITY WELL (SHADE)
# -----------------------------------------------------------------------------
# Concept: Spacetime Fabric (Geodesic Sag).
# Math: Warped Grid (Z-bias).
# -----------------------------------------------------------------------------

def generate_shade(output_path, diameter=200.0, height=140.0, resolution=100, hole_diameter=14.0):
    print(f"Generating V14 SHADE (Spacetime Fabric): {output_path}")
        ...
        for x_idx in range(res_x):
            ...
            for y_idx in range(res_y):
                ...
                if in_outer and not in_inner and not in_hand:
                    # Gravity Sag Logic
                    # Warp Z coordinate based on distance from center (Sagging fabric)
                    # z' = z + k/(r+1)
                    
                    sag = 20.0 / ( (dist_xy/10.0) + 1.0 )
