...
# -----------------------------------------------------------------------------
# HELIOS LAMP SERIES V16: THE TESSERACT (SHADE)
# -----------------------------------------------------------------------------
# Concept: The Dali Cross (Unfolded Hypercube).
# Math: Cubic P-Surface with Cross Bias.
# -----------------------------------------------------------------------------

def generate_shade(output_path, diameter=200.0, height=140.0, resolution=100, hole_diameter=14.0):
    print(f"Generating V16 SHADE (Dali Cross): {output_path}")
        ...
        for x_idx in range(res_x):
            ...
            for y_idx in range(res_y):
                ...
                if in_outer and not in_inner and not in_hand:
                    # Cubic Logic
                    # Force alignment to axes to create "Cross" look
                    
                    # Bias coordinates towards axes
                    # This "pulls" the shape into a cube
                    bx = x_mm * (1.0 + 0.3 * abs(math.sin(x_mm*0.1)))
                    by = y_mm * (1.0 + 0.3 * abs(math.sin(y_mm*0.1)))
                    
                    val = math.cos(bx * base_scale) + math.cos(by * base_scale) + math.cos(z_mm * base_scale)
