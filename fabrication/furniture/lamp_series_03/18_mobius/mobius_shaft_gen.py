...
# -----------------------------------------------------------------------------
# HELIOS LAMP SERIES 03: THE MOBIUS (SHAFT) v2.0
# -----------------------------------------------------------------------------
# REFINEMENT v2.0: Triple Propeller Twist, Library Integration.
# Logic: Single Surface (Twisted Column).
# Features: V4 QA (Solid Core, End Caps).
# -----------------------------------------------------------------------------

def generate_shaft(output_path, height=180.0, resolution=100):
    print(f"Generating MOBIUS SHAFT (v2.0): {output_path}")
    ...
    for z_idx in range(res_z):
        ...
        for x_idx in range(res_x):
            ...
            for y_idx in range(res_y):
                ...
                # Propeller Twist (v2.0 - Anisotropic)
                
                local_angle = angle + angle_offset
                
                # Anisotropy: Modulate thickness with Z
                # Stretches the ribbon contact patch
                thickness = 0.0 + 0.5 * math.sin(z_norm * math.pi)
                
                val = math.cos(num_ribbons * local_angle)
                
                if val > thickness: # Solid lobes
                    if dist < 25.0:
                        grid[x_idx,y_idx,z_idx] = True
                    else:
                        grid[x_idx,y_idx,z_idx] = False
