...
# -----------------------------------------------------------------------------
# HELIOS LAMP SERIES 06: THE BEGINNING (BASE)
# -----------------------------------------------------------------------------
# Logic: Primordial Soup.
# Features: Wire Channel, Feet Recesses (V4 Std).
# Pattern: Boiling/Bubbling chaos.
# -----------------------------------------------------------------------------

def generate_base(output_path, diameter=140.0, height=30.0, resolution=100):
    print(f"Generating BEGINNING BASE: {output_path}")
    ...
    for z_idx in range(res_z):
        ...
        for x_idx in range(res_xy):
            ...
            for y_idx in range(res_xy):
                ...
                if dist <= radius:
                    # Soup Relief
                    # High frequency bubbling
                    val = math.sin(x_mm*scale) * math.sin(y_mm*scale)
                    val2 = math.sin((x_mm+y_mm)*scale*1.5)
                    
                    # Normalize
                    h_mod = (val + val2 + 2.0) / 4.0
...
if __name__ == "__main__":
    output_file = "beginning_base.stl"
