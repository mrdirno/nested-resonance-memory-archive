...
# -----------------------------------------------------------------------------
# HELIOS LAMP SERIES 06: THE ARCHITECT (BASE)
# -----------------------------------------------------------------------------
# Logic: Foundation Plan (Grid Layout).
# Features: Wire Channel, Feet Recesses (V4 Std).
# Pattern: Floorplan relief.
# -----------------------------------------------------------------------------

def generate_base(output_path, diameter=140.0, height=30.0, resolution=100):
    print(f"Generating ARCHITECT BASE: {output_path}")
    ...
    for z_idx in range(res_z):
        ...
        for x_idx in range(res_xy):
            ...
            for y_idx in range(res_xy):
                ...
                if dist <= radius:
                    # Floorplan Relief
                    # Random rooms? Or structured grid?
                    
                    # Structured grid layout
                    grid_size = 15.0
                    wall_thick = 3.0
                    
                    gx = abs(x_mm) % grid_size
                    gy = abs(y_mm) % grid_size
                    
                    is_wall = False
                    if gx < wall_thick or gy < wall_thick:
                        is_wall = True
                        
                    # Height modulation
                    # Walls are high, rooms are low
                    z_surf = height
                    if not is_wall:
                        z_surf = height - 3.0
...
if __name__ == "__main__":
    output_file = "architect_base.stl"
