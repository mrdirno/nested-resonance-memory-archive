...
# -----------------------------------------------------------------------------
# HELIOS LAMP SERIES 04: THE ARTIFACT (BASE)
# -----------------------------------------------------------------------------
# Logic: Pixel Grid (Quantized Relief).
# Features: Wire Channel, Feet Recesses (V4 Std).
# Pattern: Blocky/Stepped surface.
# -----------------------------------------------------------------------------

def generate_base(output_path, diameter=140.0, height=30.0, resolution=100):
    print(f"Generating ARTIFACT BASE: {output_path}")
        ...
        for x_idx in range(res_xy):
            ...
            for y_idx in range(res_xy):
                ...
                if dist <= radius:
                    ...
                    # Pattern: Random height or structured?
                    # Structured: Radial ripple but quantized
                    q_dist = math.sqrt(qx**2 + qy**2)
                    
                    # Height function
                    val = math.cos(q_dist * 0.1)
                    ...
                    # Quantize Z
                    z_surf = math.floor(z_surf / 2.0) * 2.0
