...
# -----------------------------------------------------------------------------
# HELIOS LAMP SERIES 04: THE LAG (BASE)
# -----------------------------------------------------------------------------
# Logic: Motion Blur Relief.
# Features: Wire Channel, Feet Recesses (V4 Std).
# Pattern: Smeared/Sheared relief.
# -----------------------------------------------------------------------------

def generate_base(output_path, diameter=140.0, height=30.0, resolution=100):
    print(f"Generating LAG BASE: {output_path}")
    ...
    for z_idx in range(res_z):
        ...
        for x_idx in range(res_xy):
            ...
            for y_idx in range(res_xy):
                ...
                if dist <= radius:
                    # Smear Pattern
                    # Base pattern: Sine waves
                    # Smeared along vector
                    
                    # Rotate to align with blur direction
                    u = x_mm * math.cos(blur_angle) + y_mm * math.sin(blur_angle)
                    v = -x_mm * math.sin(blur_angle) + y_mm * math.cos(blur_angle)
                    
                    # Stretch V, Compress U
                    val = math.sin(u * 0.5) * math.sin(v * 0.05)
