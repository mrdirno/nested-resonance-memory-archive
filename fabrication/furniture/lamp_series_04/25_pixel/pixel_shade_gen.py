...
# -----------------------------------------------------------------------------
# HELIOS LAMP SERIES 04: THE PIXEL (SHADE)
# -----------------------------------------------------------------------------
# Logic: Resolution Drop / Low-Poly.
# Method: Minecraft-style voxels (Explicit cubes).
...
def generate_shade(output_path, diameter=200.0, height=180.0, resolution=120, hole_diameter=14.0):
    print(f"Generating THE PIXEL SHADE (LOW-POLY): {output_path}")
    ...
    # Pixel Logic
    # Similar to Artifact, but strict geometric primitives.
    # "8-Bit" Look.
    
    block_size = 15.0
    
    print("Dropping Resolution...")
    ...
    for z_idx in range(res_z):
        ...
        for x_idx in range(res_x):
            ...
            for y_idx in range(res_y):
                ...
                # Make it look like a low-res sphere
                is_solid = False
                ...
                if q_dist <= radius and q_dist >= (radius - wall_thickness):
                    ...
                    if (ix + iy + iz) % 2 == 0:
                        is_solid = True
                    elif q_dist > (radius - 10.0): # Keep outer shell more solid
                        is_solid = True
