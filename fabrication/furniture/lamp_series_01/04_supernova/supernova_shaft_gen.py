...
# -----------------------------------------------------------------------------
# HELIOS LAMP SERIES 01: THE SUPERNOVA (SHAFT)
# -----------------------------------------------------------------------------
# Logic:
# 1. Shape: Shockwave Ripple (Cylinder with Ring Pulses).
...
def generate_shaft(output_path, height=160.0, resolution=120):
    print(f"Generating SUPERNOVA SHAFT: {output_path}")
    ...
    # Ripple Params
    freq = 2.0 * math.pi / 20.0 # 20mm wavelength
    
    for z_idx in range(res_z):
        z_mm = z_idx * step
        
        # Ripple Function
        # Amplitude increases then decreases? No, consistent shockwave.
        # Sharp sine wave?
        ripple = math.sin(z_mm * freq)
        current_radius = base_radius + (ripple * max_ripple)
        
        # Add some noise/chaos to the rings?
        # Let's keep it clean shockwave for contrast with chaos shade.
    ...
    for z in range(res_z):
        ...
        for x in range(res_x):
            x_mm = (x * step) - max_r_bound
            for y in range(res_y):
                y_mm = (y * step) - max_r_bound
...
if __name__ == "__main__":
    output_file = "fabrication/furniture/lamp_series_01/04_supernova/supernova_shaft.stl"
