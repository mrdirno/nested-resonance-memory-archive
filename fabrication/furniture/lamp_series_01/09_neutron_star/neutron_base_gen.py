...
# -----------------------------------------------------------------------------
# HELIOS LAMP SERIES 01: THE NEUTRON STAR (BASE)
# -----------------------------------------------------------------------------
# Logic:
# 1. Shape: Accretion Disk (Swirling spiralled base).
...
def generate_base(output_path, diameter=140.0, height=35.0, resolution=100):
    print(f"Generating NEUTRON BASE: {output_path}")
    ...
    for z_idx in range(res_z):
        ...
        for x_idx in range(res_xy):
            ...
            for y_idx in range(res_xy):
                ...
                # Spiral Log
                # r = a * exp(b * theta)
                # Simplified: sin(arms * angle + twist(r))
                
                twist_r = (dist/radius) * twist
                mod = math.sin(arms * angle + twist_r)
                ...
                if dist <= r_layer:
                    # Modulate height?
                    # No, modulate radius?
                    # Let's make the surface bumpy
                    
                    # Top surface modulation
                    # z_surf = height - depth * (1 - mod)
                    z_surf = height - 5.0 * (1.0 - mod)
                    if z_mm <= z_surf:
                        is_solid = True
...
if __name__ == "__main__":
    output_file = "fabrication/furniture/lamp_series_01/09_neutron_star/neutron_base.stl"
