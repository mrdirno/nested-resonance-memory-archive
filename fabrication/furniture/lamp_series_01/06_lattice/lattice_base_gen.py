...
# -----------------------------------------------------------------------------
# HELIOS LAMP SERIES 01: THE LATTICE (BASE) - V4 QA
# -----------------------------------------------------------------------------
...
def generate_base(output_path, diameter=160.0, height=50.0, resolution=100):
    print(f"Generating LATTICE BASE (V4 QA): {output_path}")
    ...
    for z_idx in range(res_z):
        ...
        for x_idx in range(res_xy):
            ...
            for y_idx in range(res_xy):
                ...
                if z_mm < foot_depth:
                    foot_dist_center = radius - foot_offset
                    # 4 feet
                    if math.sqrt((x_mm-foot_dist_center)**2 + y_mm**2) < foot_radius: 
                        grid[x_idx,y_idx,z_idx] = False
                        continue
                    if math.sqrt((x_mm+foot_dist_center)**2 + y_mm**2) < foot_radius:
                        grid[x_idx,y_idx,z_idx] = False
                        continue
                    if math.sqrt(x_mm**2 + (y_mm-foot_dist_center)**2) < foot_radius:
                        grid[x_idx,y_idx,z_idx] = False
                        continue
                    if math.sqrt(x_mm**2 + (y_mm+foot_dist_center)**2) < foot_radius:
                        grid[x_idx,y_idx,z_idx] = False
