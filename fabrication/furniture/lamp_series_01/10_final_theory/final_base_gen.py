...
# -----------------------------------------------------------------------------
# HELIOS LAMP SERIES 01: THE FINAL THEORY (BASE) - V4 QA
# -----------------------------------------------------------------------------
# Logic:
# 1. Shape: Unity Ring (Torus or Complex Knot?).
# 2. Features: Wire Channel, Feet Recesses (V4 Std).
# 3. Form: Solid cylindrical base with 5-fold symmetry relief.
# -----------------------------------------------------------------------------
...
def generate_base(output_path, diameter=140.0, height=35.0, resolution=100):
    print(f"Generating FINAL BASE (V4 QA): {output_path}")
    ...
    # 5-fold symmetry relief
    vectors = []
    for i in range(5):
        angle = i * 2*math.pi/5
        vectors.append((math.cos(angle), math.sin(angle)))
        
    scale = 2.0 * math.pi / 20.0 # 20mm features
        ...
        for x_idx in range(res_xy):
            ...
            for y_idx in range(res_xy):
                ...
                if dist <= radius:
                    ...
                    for v in vectors:
                        vx, vy = v
                        val += math.cos( (x_mm*vx + y_mm*vy) * scale )
                    
                    # Top height modulation
                    z_surf = height - 2.0 * (1.0 - (val/5.0))
                    if z_mm <= z_surf:
                        is_solid = True
...
if __name__ == "__main__":
    output_file = "fabrication/furniture/lamp_series_01/10_final_theory/final_base.stl"
