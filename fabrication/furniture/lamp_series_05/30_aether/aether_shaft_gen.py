...
# -----------------------------------------------------------------------------
# HELIOS LAMP SERIES 05: THE AETHER (SHAFT)
# -----------------------------------------------------------------------------
# Logic: Disconnected Segments (Tensegrity Illusion).
# Core: 14mm Central Channel (V4 Std).
# Ends: Solid End Caps (V4 Std).
# -----------------------------------------------------------------------------

def generate_shaft(output_path, height=180.0, resolution=100):
    print(f"Generating AETHER SHAFT: {output_path}")
    ...
    # Segment Logic
    # Floating blocks held by thin core
    segment_h = 20.0
    gap_h = 5.0
    ...
    for z_idx in range(res_z):
        ...
        for x_idx in range(res_x):
            ...
            for y_idx in range(res_y):
                ...
                # Segments
                # Modulo Z
                z_mod = z_mm % (segment_h + gap_h)
                
                if z_mod < segment_h:
                    # Solid block
                    # Rotating?
                    # Layer index
                    layer = int(z_mm / (segment_h + gap_h))
                    angle = layer * (math.pi / 4.0)
                    
                    # Rotated square
                    rx = x_mm * math.cos(angle) - y_mm * math.sin(angle)
                    ry = x_mm * math.sin(angle) + y_mm * math.cos(angle)
                    
                    if max(abs(rx), abs(ry)) < base_radius:
                        grid[x_idx,y_idx,z_idx] = True
                else:
                    # Gap -> Only core exists (handled above)
                    grid[x_idx,y_idx,z_idx] = False
