...
# -----------------------------------------------------------------------------
# HELIOS LAMP SERIES 05: THE FIRE (BASE)
# -----------------------------------------------------------------------------
# Logic: Embers (Charred/Glowing Coals).
# Features: Wire Channel, Feet Recesses (V4 Std).
# Pattern: Rocky/Cracked noise relief.
# -----------------------------------------------------------------------------

def generate_base(output_path, diameter=140.0, height=30.0, resolution=100):
    print(f"Generating FIRE BASE: {output_path}")
        ...
        for x_idx in range(res_xy):
            ...
            for y_idx in range(res_xy):
                ...
                if dist <= radius:
                    ...
                    val2 = math.sin(x_mm * scale * 2.3 + 1.0) * math.sin(y_mm * scale * 2.3 + 2.0)
                    
                    combined = val + 0.5 * val2
                    
                    # Height map: Rough peaks
                    h_mod = (combined + 1.5) / 3.0
                    
                    z_surf = height - 8.0 * (1.0 - h_mod)
                    
                    # Flatten top for shaft mount (visual stability)
                    if dist < 25.0:
                        z_surf = max(z_surf, height - 2.0)
