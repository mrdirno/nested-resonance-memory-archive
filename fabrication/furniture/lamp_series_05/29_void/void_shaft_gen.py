...
def generate_shaft(output_path, height=180.0, resolution=100):
    ...
    # Exoskeleton Pattern
    # Large holes revealing the core (which is just the rod channel)
    ...
    for z_idx in range(res_z):
        ...
        for x_idx in range(res_x):
            ...
            for y_idx in range(res_y):
                ...
                # Outer Shell (Anisotropic Exoskeleton)
                if dist <= current_radius:
                    # Large Voronoi-like holes (Stretched Z)
                    
                    scale = 2.0 * math.pi / 40.0
                    sz = scale * 0.5 # Stretched
                    
                    val = math.sin(x_mm*scale)*math.cos(z_mm*sz) + math.sin(y_mm*scale)
