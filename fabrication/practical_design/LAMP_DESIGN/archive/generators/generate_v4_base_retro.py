...
# ==========================================
# CONFIGURATION: V4 "Industrial Geometric"
# ==========================================
OUTPUT_FILE = "fabrication/practical_design/LAMP_DESIGN/base_v4_retrofit.stl"
RESOLUTION = 120 
SIZE_Z = 45.0     
MAX_DIAMETER = 150.0
HOLE_DIAMETER = 14.0
WIRE_CHANNEL_W = 12.0 # Updated to V6 Safety
WIRE_CHANNEL_H = 12.0 
FOOT_DEPTH = 2.5
FOOT_RADIUS = 12.0
FOOT_OFFSET_R = 55.0
RECESS_DIAMETER = 40.5 # Updated to V6 Mating
RECESS_DEPTH = 3.0
TWIST_RATE = 0.0 # V4 is "Clean/Predictable" - No Twist
...
# GENERATOR
# ==========================================
def generate_base_v4_retro():
    print(f"Initializing Base V4 (Industrial Geometric Retrofit)...")
    
    # V4 Aesthetic: Uniform Gyroid, No Distortion
    agph = AGPHCore(
        scale_x=0.25, scale_y=0.25, scale_z=0.25,
        twist_rate=0.0, # No Twist
        anisotropy=(1.0, 1.0, 1.0), # Isotropic
        gyroid_thickness=0.5 # Balanced
    )
    ...
    for z in range(res_z):
        ...
        # V4 Shape: Simple Chamfered Cylinder
        # r = MAX if z < 35, then taper to 40
        if pz < (SIZE_Z - 10.0):
            current_radius_limit = MAX_DIAMETER / 2.0
        else:
            # Chamfer top 10mm
            t = (pz - (SIZE_Z - 10.0)) / 10.0 # 0 to 1
            r_start = MAX_DIAMETER / 2.0
            r_end = 45.0 / 2.0
            current_radius_limit = r_start + (r_end - r_start) * t
            
        macro_scale = 1.0 # Uniform scale
        ...
        for x in range(res_x):
            ...
            for y in range(res_y):
                ...
                # 4. Wire Channel (Updated to V6 Spec: Arch)
                if px > 0:
                ...
                # Core solidity
                if r < ((HOLE_DIAMETER/2.0) + 5.0):
                    grid[x,y,z] = True
                
                # Rim solidity (Industrial Look)
                if r > (current_radius_limit - 2.0):
                    grid[x,y,z] = True
