...
# CONFIGURATION
# ==========================================
OUTPUT_FILE = "fabrication/practical_design/LAMP_DESIGN/base_qa_v5.stl"
RESOLUTION = 150 
SIZE_Z = 45.0     
MAX_DIAMETER = 150.0
HOLE_DIAMETER = 14.0
...
# GENERATOR
# ==========================================
def generate_base_v5():
    print(f"Initializing QA Base V5 (AGPH Dome)...")
    
    agph = AGPHCore(
        scale_x=0.2, scale_y=0.2, scale_z=0.2, # Low freq for "Roots"
        twist_rate=TWIST_RATE,
        anisotropy=(1.0, 1.0, 0.5), # Squashed Z
        gyroid_thickness=0.8 # Very thick (mostly solid)
    )
    ...
    for z in range(res_z):
        ...
        for x in range(res_x):
            ...
            for y in range(res_y):
                ...
                # 6. AGPH Solid Field
                # Use field to determine solidity.
                # We want a "solid object with gyroid texture".
                # So if field > -threshold?
                
                val = agph.get_field_value(px, py, pz, macro_scale_factor=macro_scale)
                
                # Gyroid range -1.5 to 1.5 approx.
                # To make it mostly solid with voids:
                # val > -0.5 ?
                
                if val > -0.8: # mostly solid
                    grid[x,y,z] = True
