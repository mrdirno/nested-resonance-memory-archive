...
OUTPUT_FILE = "fabrication/practical_design/LAMP_DESIGN/shaft_qa_v5_solid_core.stl"
RESOLUTION = 120 
SIZE_Z = 180.0   
MAX_DIAMETER = 40.0 
CROWN_DIAMETER = 55.0 
...
def generate_shaft_v5_solid_core():
    print(f"Initializing QA Shaft V5 (Solid Core Repair)...")
    
    # Denser lattice to ensure connectivity
    agph = AGPHCore(
        scale_x=0.5, scale_y=0.5, scale_z=0.15, 
        twist_rate=TWIST_RATE,
        anisotropy=(1.0, 1.0, 1.0),
        gyroid_thickness=0.9 # Very thick
    )
    ...
    for z in range(res_z):
        ...
        for x in range(res_x):
            ...
            for y in range(res_y):
                ...
                # SOLID CORE REPAIR
                # Force everything within r < 22mm to be solid?
                # Base radius at waist is ~20mm. 
                # If we force solid up to 18mm, the lattice is just a surface texture.
                # That defeats AGPH.
                # But connectivity is priority.
                
                # Let's force solid up to `current_radius * 0.7`
                # This guarantees a solid pipe that modulates in thickness.
                
                solid_core_r = current_radius * 0.7
                if r < solid_core_r:
                    grid[x,y,z] = True
                    continue
                
                # 4. AGPH Structure (Outer Layer)
                val = agph.get_field_value(px, py, pz, macro_scale_factor=macro_scale)
                
                if agph.is_solid(val):
                    grid[x,y,z] = True
                
                if r > (current_radius - 1.0): grid[x,y,z] = True
