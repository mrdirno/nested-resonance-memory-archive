...
OUTPUT_FILE = "fabrication/practical_design/LAMP_DESIGN/shaft_qa_v5_robust.stl"
RESOLUTION = 120 
SIZE_Z = 180.0   
MAX_DIAMETER = 40.0 
CROWN_DIAMETER = 55.0 
...
PLUG_DIAMETER = 40.0
PLUG_HEIGHT = 3.0
TWIST_RATE = 0.02 # REDUCED from 0.05
...
def generate_shaft_v5_robust():
    print(f"Initializing QA Shaft V5 (Robust)...")
    
    # Very thick lattice, lower twist
    agph = AGPHCore(
        scale_x=0.4, scale_y=0.4, scale_z=0.2, 
        twist_rate=TWIST_RATE,
        anisotropy=(1.0, 1.0, 1.0),
        gyroid_thickness=1.2 # Very thick
    )
    ...
    for z in range(res_z):
        ...
        for x in range(res_x):
            ...
            for y in range(res_y):
                ...
                # Skin
                if r > (current_radius - 1.5): grid[x,y,z] = True
                # Core
                if r < ((CABLE_CLEARANCE/2.0) + 3.0):
                    grid[x,y,z] = True
