...
def generate_shade_fixed(output_path, resolution=150):
    print(f"Generating FIXED Manifold Shade: {output_path}")
    ...
    # Physical Bounds
    max_width = size_x_top * (1.0 + k_expansion) # Base width
    ...
    base_scale_xy = 2.0 * math.pi / (size_x_top / 3.0)
    
    for z_idx in range(res_z):
        pz = z_idx * step
        z_norm = pz / size_z
        
        # Expansion at this height
        # Top (z=size_z, norm=1) -> factor 1.0
        # Bottom (z=0, norm=0) -> factor 1+k
        exp_factor = 1.0 + k_expansion * (1.0 - z_norm)
        
        current_width = size_x_top * exp_factor
        
        # Scale Modulation
        base_wavelength_z = size_z / 4.0
        scale_z = (2.0 * math.pi / base_wavelength_z) / (1 + 0.01 * z_norm)
        ...
        for x_idx in range(res_x):
            ...
            for y_idx in range(res_y):
                ...
                # 5. Pattern (Inverse Transform)
                # We want the pattern to scale with the object
    ...
    # Meshing (Standard Marching Cubes-ish / Voxel Quads)
    vertices = []
    faces = []
...
if __name__ == "__main__":
    generate_shade_fixed("fabrication/output/shade_v4_fixed.stl")
