...
from sklearn.gaussian_process import GaussianProcessRegressor # For GPR model
from sklearn.gaussian_process.kernels import RBF, ConstantKernel # For GPR kernel
...
# --- TSF-8 CONFIGURATION ---
EXPERIMENT_ID = "TSF-8"
TITLE = "Advanced Model-Based Agency with GPR"
AGENTS_PER_CELL = 20
DURATION = 300 # Longer duration for adaptation scenarios
MODEL_UPDATE_INTERVAL = 10 # Steps between model updates
EXPLORATION_STEPS = 10 # Initial random exploration steps to build model
NUM_CANDIDATE_PERTS = 20 # Number of candidate perturbations to evaluate with model

# GPR specific parameters
GPR_ALPHA = 1e-5 # Noise level for GPR
GPR_KAPPA = 2.0 # Exploration-exploitation trade-off for UCB

# Target Regime (same as TSF-5/6/7)
C_TARGET_INIT = 0.2
R_TARGET_INIT = 0.6
...
class AdvancedModelBasedAgencyTSF8:
    def __init__(self):
    ...
    def update_model(self):
        """Build or update the internal GPR models."""
        if len(self.model_data_X) > GPR_ALPHA: # Need more than just noise level for training
        ...
        # else: print("Not enough data to train model yet.")
    ...
    def run(self):
        ...
        print("Initial exploration to build phase space model with GPR...")
        ...
        for step in range(DURATION):
            ...
            if step == CHANGE_STEP_1:
                ...
                self.model_data_yC = []
                self.model_data_yR = []
                for _ in range(EXPLORATION_STEPS): # Re-explore to build new model
            ...
            # 2. Update Model Periodically (or on major change events)
            if (step > EXPLORATION_STEPS and len(self.model_data_X) > GPR_ALPHA and step % MODEL_UPDATE_INTERVAL == 0) or \
               (step == CHANGE_STEP_1 + 1) or (step == CHANGE_STEP_2 + 1):
                self.update_model()
                # print(f"Model updated at step {step}.")

            # 3. Predict & Select Best Action using GPR (with Uncertainty - UCB)
            if len(self.model_data_X) > GPR_ALPHA: # Only use model if trained
                best_ucb_score = -np.inf
                best_E_action = self.energy_influx
                best_S_action = self.stability_coupling

                # Generate candidate (E,S) points around current best
                candidate_points = []
                for _ in range(NUM_CANDIDATE_PERTS):
                    candidate_E = np.clip(self.energy_influx + np.random.normal(0, 0.05), 0.01, 0.3)
                    candidate_S = np.clip(self.stability_coupling + np.random.normal(0, 0.05), 0.0, 0.8)
                    candidate_points.append([candidate_E, candidate_S])
                
                # Also include current point as a candidate (to stay put if best)
                candidate_points.append([self.energy_influx, self.stability_coupling])
                ...
                for i, (cand_E, cand_S) in enumerate(candidate_points):
                    ...
                    predicted_C_uncertainty = predicted_C_std[i]
                    predicted_R_uncertainty = predicted_R_std[i]
                    
                    # Calculate performance from predicted values
                    predicted_performance = self.calculate_performance(predicted_C, predicted_R)
                    
                    # Combine performance with uncertainty (UCB - Upper Confidence Bound)
                    # We want to maximize performance (less negative distance) and explore uncertainty
                    ucb_score = predicted_performance + GPR_KAPPA * (predicted_C_uncertainty + predicted_R_uncertainty)
                ...
                # Apply the best predicted action (chosen by UCB)
                ...
                self.energy_influx = np.clip(self.energy_influx + np.random.normal(0, 0.05), 0.01, 0.3)
                self.stability_coupling = np.clip(self.stability_coupling + np.random.normal(0, 0.05), 0.0, 0.8)
    ...
    def _analyze(self):
        print("--- ANALYSIS: ADVANCED MODEL-BASED AGENCY ---")
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.results_dir, "gpr_model_based_control_trajectory.png"))
        plt.close()
        print(f"GPR Model-based control trajectory plot saved to {os.path.join(self.results_dir, 'gpr_model_based_control_trajectory.png')}")
...
if __name__ == "__main__":
    ...
    eng = AdvancedModelBasedAgencyTSF8()
    eng.run()
