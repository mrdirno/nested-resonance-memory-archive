...
# --- TSF-4 CONFIGURATION ---
EXPERIMENT_ID = "TSF-4"
TITLE = "Robust Agency: Convergent Navigation in Phase Space"
AGENTS_PER_CELL = 20
DURATION = 200 # Longer duration for convergence
PERTURB_MAGNITUDE = 0.01 # Magnitude of random perturbation
ADAPT_RATE = 0.1 # Rate at which to adapt E,S if perturbation fails

# Target Regime: Balanced Emergence (derived from TSF-2 analysis)
C_TARGET = 0.2
R_TARGET = 0.6

class RobustAgencyTSF4:
    def __init__(self):
        ...
        self.best_performance = -np.inf # Track best performance
        self.best_E = self.energy_influx
        self.best_S = self.stability_coupling

    def run_step_simulation(self, energy_influx, stability_coupling):
        """
        Run a single simulation chunk to measure C and R for given E and S.
        Similar to TSF-3's run_step.
        """
        ...
        internal_duration = 50 # Shorter internal duration for a "reading" to speed up search
    ...
    def calculate_performance(self, c_val, r_val):
        """
        Calculates a performance score based on proximity to target C and R.
        Lower is better (like an error function).
        """
        dist_c = abs(c_val - C_TARGET)
        dist_r = abs(r_val - R_TARGET)
        return -(dist_c + dist_r) # Negative sum of distances (maximize performance -> minimize distance)
    ...
    def run(self):
        ...
        for step in range(DURATION):
            ...
            current_c, current_r = self.run_step_simulation(self.energy_influx, self.stability_coupling)
            current_performance = self.calculate_performance(current_c, current_r)
            
            # 2. Agency: Probabilistic Hill Climbing
            # Propose a new (E,S) state
            trial_E = np.clip(self.energy_influx + np.random.normal(0, PERTURB_MAGNITUDE), 0.01, 0.3)
            trial_S = np.clip(self.stability_coupling + np.random.normal(0, PERTURB_MAGNITUDE), 0.0, 0.8)
            
            # Simulate the trial state
            trial_c, trial_r = self.run_step_simulation(trial_E, trial_S)
            ...
            if trial_performance > current_performance: # If new state is better
                ...
                current_c, current_r = trial_c, trial_r # Update current C,R to trial's for logging
            ...
            print(f"Step {step:03d} | E={self.energy_influx:.2f} S={self.stability_coupling:.2f} | C={current_c:.3f} R={current_r:.3f} | Perf={current_performance:.3f}")
    ...
    def _analyze(self):
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.results_dir, "robust_navigation_trajectory.png"))
        plt.close()
        print(f"Navigation trajectory plot saved to {os.path.join(self.results_dir, 'robust_navigation_trajectory.png')}")
