...
class AdaptiveControlTSF5:
    ...
    def run_step_simulation(self, energy_influx, stability_coupling, metabolic_cost):
        ...
        Metabolic cost is now a variable parameter.
        """
        ...
        for t in range(internal_duration):
            ...
            alive_agents = []
            for a in agents:
                a.update_energy(-metabolic_cost) # Use variable metabolic cost
                a.update_phase(1.0)
                if a.state.energy > 0:
    ...
    def run(self):
        ...
        for step in range(DURATION):
            ...
            # 2. Agency: Probabilistic Hill Climbing (same as TSF-4)
            ...
            print(f"Step {step:03d} | E={self.energy_influx:.2f} S={self.stability_coupling:.2f} | C={current_c:.3f} R={current_r:.3f} | Target C={self.c_target:.1f} R={self.r_target:.1f} | Cost={self.metabolic_cost:.2f} | Perf={current_performance:.3f}")
            
        self._analyze()

    def _analyze(self):
        print("--- ANALYSIS: ADAPTIVE CONTROL ---")
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.results_dir, "adaptive_control_trajectory.png"))
        plt.close()
        print(f"Adaptive control trajectory plot saved to {os.path.join(self.results_dir, 'adaptive_control_trajectory.png')}")
