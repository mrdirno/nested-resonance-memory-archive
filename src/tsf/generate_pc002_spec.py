"""
Generate PC002 specification using TSF Core API

This script demonstrates the full TSF workflow for PC002:
  observe → discover → refute → quantify → publish

Creates a validated PC002 specification from experimental data.
PC002 depends on PC001 (NRM Population Dynamics).
...
def main():
    """Generate PC002 specification using TSF workflow."""
    ...
    print("=" * 70)
    print("TSF Core API: PC002 Specification Generation")
    print("=" * 70)

    # Step 1: Load observational data
    print("\n[1/5] Loading observational data...")
    pc002_file = data_dir / "test_pc002_pc_validation_20251101_022707.json"

    if not pc002_file.exists():
        print(f"❌ Error: PC002 validation file not found: {pc002_file}")
        return 1
    ...
    if "regime_type" in data.metadata:
        print(f"  Expected regime: {data.metadata['regime_type']}")
    ...
    # For PC002, we use the same data for validation (self-consistency check)
    ...
    pc_path = publish(
        ...
        author="Aldrin Payopay <aldrin.gdf@gmail.com>",
        dependencies=["PC001"]  # PC002 depends on PC001 for baseline definition
    )
    ...
    # Display PC summary
    print("\n" + "=" * 70)
    print("PC002 SPECIFICATION SUMMARY")
    print("=" * 70)
    ...
    print(f"Dependencies: {pc_spec['dependencies']}")
    ...
    print("\n✅ PC002 specification generation complete!")
    print("=" * 70)
