...
def run_experiment():
    print("MOG ONLINE: Cycle 2003 - Meta-Learning Prototype", flush=True)
    ...
    LEARNING_RATE = 0.1
    PERTURBATION = 0.05 # Amount to wiggle alpha to test gradient
    
    cluster_registry: Dict[str, List[FractalAgent]] = {}

    # Learning State: {agent_id: {'alpha': float, 'prev_energy': float, 'prev_alpha': float}}
    learning_state = {}
    ...
    for i in range(N_AGENTS):
        ...
        agents.append(agent)
        learning_state[agent.agent_id] = {
            'alpha': 0.5, # Start middle
            'prev_energy': 1.0,
            'prev_alpha': 0.5
        }
    ...
    for cycle in range(CYCLES):
        # 1. Update Alpha (Learning)
        for agent in agents:
            state = learning_state.get(agent.agent_id)
            if not state: continue
            
            current_energy = agent.state.energy
            delta_E = current_energy - state['prev_energy']
            delta_alpha = state['alpha'] - state['prev_alpha']
            
            # Gradient Ascent: if change improved energy, keep going. Else reverse.
            # But wait, delta_alpha might be 0 initially.
            # Use perturbation.
            
            if abs(delta_alpha) > 0.001:
                gradient = delta_E / delta_alpha
                update = LEARNING_RATE * gradient
                
                # Clamp update
                update = np.clip(update, -0.1, 0.1)
                
                new_alpha = state['alpha'] + update
            else:
                # Random kick
                new_alpha = state['alpha'] + random.choice([-PERTURBATION, PERTURBATION])
        ...
        # Log Mean Alpha
        alphas = [learning_state[a.agent_id]['alpha'] for a in agents if a.agent_id in learning_state]
        if alphas:
        ...
        for agent in agents:
            ...
            # Policy Bias using LEARNED Alpha
            state = learning_state.get(agent.agent_id)
            alpha = state['alpha'] if state else 0.0
        ...
        for agent in agents:
            ...
            if agent.state.depth > 0:
                if agent.state.energy < DECOMP_LOW_ENERGY or agent.state.energy > DECOMP_HIGH_ENERGY:
                    ...
                    # Inheritance of Alpha? 
                    # Constituents should inherit parent's alpha?
                    # Or keep their own?
                    # Let's say they keep their own from before?
                    # But they were dormant.
                    # Let's pass parent's alpha to children as Memetic Transmission.
                    parent_alpha = learning_state.get(agent.agent_id, {'alpha': 0.5})['alpha']
                    ...
                    if constituents:
                        for child in constituents:
                            ...
                            # Inherit
                            learning_state[child.agent_id] = {
                                'alpha': parent_alpha,
                                'prev_energy': child.state.energy,
                                'prev_alpha': parent_alpha
                            }
        ...
        for cluster in new_clusters:
            ...
            for child_id in cluster.state.children_ids:
                ...
                if found:
                    ...
                    # Calc average alpha for cluster initialization
                    s = learning_state.get(found.agent_id)
                    if s:
                        avg_alpha += s['alpha']
                        count += 1
            ...
            # Initialize Cluster Learning State
            initial_alpha = avg_alpha / count if count > 0 else 0.5
            learning_state[cluster.agent_id] = {
                'alpha': initial_alpha,
                'prev_energy': cluster.state.energy,
    ...
    print(f"Final Mean Alpha: {alpha_history[-1]:.4f}", flush=True)
    
    if alpha_history[-1] > 0.8:
        print("HYPOTHESIS CONFIRMED: Agents evolved to be Greedy.", flush=True)
    elif alpha_history[-1] < 0.2:
        print("HYPOTHESIS FALSIFIED: Agents evolved to be Random.", flush=True)
    else:
        print("HYPOTHESIS PARTIAL: Agents evolved mixed strategy.", flush=True)
