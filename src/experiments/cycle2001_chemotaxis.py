...
def get_recharge_at_pos(pos, center=(50, 50)):
    # Gradient Field: Recharge depends on position
    # Max 0.10 at center, 0.00 at edge
    dist = np.linalg.norm(pos[:2] - np.array(center))
...
def calculate_cluster_gradient(cluster_agent: FractalAgent, constituents: List[FractalAgent]) -> np.ndarray:
    ...
    for child in constituents:
        # Hack: Assign random relative offsets if missing
...
def run_simulation(smart_mode: bool):
    print(f"\n--- Simulation: {'Smart' if smart_mode else 'Dumb'} ---", flush=True)
    ...
    COST_SINGLE = 0.05 # Standard metabolism
    COST_CLUSTER = 0.01 # Efficiency
    DECOMP_LOW_ENERGY = 0.2 
    DECOMP_HIGH_ENERGY = 4.0 
    
    cluster_registry: Dict[str, List[FractalAgent]] = {}

    # Initialize Population away from food
    agents = []
    ...
    for cycle in range(CYCLES):
        ...
        for agent in agents:
            ...
            # Smart Bias
            if smart_mode and agent.state.depth > 0:
                # Cluster Sensing
                ...
                if norm > 0:
                    bias = (grad / norm) * VELOCITY_MAGNITUDE * 0.5 # 50% bias
                    move_vec += bias
            
            agent.move(move_vec)
            # Wrap or Clamp? Let's Clamp to keep them in the arena
        ...
        # 3. Composition
        # Auto-compose willing agents (simplified for this test)
        # Just assume they all want to compose if close
        surviving_ids = set(a.agent_id for a in active_agents)
        
        # Filter: only compose if energy > 0.5 (Physics rule)
        # This is the filter we found earlier.
    ...
    print(f"Final System Energy: {final_energy:.2f}", flush=True)
    return final_energy

def run_experiment():
    print("MOG ONLINE: Cycle 2001 - Chemotaxis", flush=True)
    
    energy_dumb = run_simulation(smart_mode=False)
    energy_smart = run_simulation(smart_mode=True)
    
    print("\n--- Comparison ---", flush=True)
    print(f"Dumb Energy: {energy_dumb:.2f}", flush=True)
    print(f"Smart Energy: {energy_smart:.2f}", flush=True)
    
    if energy_smart > energy_dumb * 1.2:
        print("HYPOTHESIS CONFIRMED: Smart Clusters outperform Dumb Clusters.", flush=True)
    else:
