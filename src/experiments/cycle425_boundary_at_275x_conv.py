#!/usr/bin/env python3
"""
CYCLE 425: BOUNDARY AT 2.75× CONVERSION
...
Purpose:
  Map recovery profile at 0.75× attack.

Background:
  At 2.5× conversion: 0.75× = 80% (minimum)
  At 3.0× conversion: 0.75× = 90% (recovery)

  C425: 0.75× + 2.75× - between minimum and recovery
...
CYCLE_ID = "C425"
CYCLE_NAME = "Boundary at 2.75x Conv"
MODE = "BOUNDARY_275XCONV"
...
# 0.75× attack + 2.75× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.00225, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.825},
    {"attack": 0.00375, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.6875},
    {"attack": 0.006, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.55},
    {"attack": 0.009, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.4125},
    {"attack": 0.01125, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.33},
    {"attack": 0.0135, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.275}
]

SEEDS = list(range(6480, 6500))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run boundary at 2.75× conversion experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c425_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 425: BOUNDARY AT 2.75× CONVERSION")
    ...
    print(f"Attack: 0.75×, Conversion: 2.75×")
    print("=" * 80)
    print()
    ...
    output = {
        ...
        "conversion_multiplier": 2.75,
        "n_seeds": len(SEEDS),
        "coexistence_rate": coexist_count / len(results) if results else 0,
        "results": results
    }

    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c425_boundary_at_275x_conv_results.json"
