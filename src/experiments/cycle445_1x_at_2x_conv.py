#!/usr/bin/env python3
"""
CYCLE 445: 1.0× ATTACK AT 2.0× CONVERSION
...
Purpose:
  Test baseline (1.0×) attack rate with 2.0× conversion.
  Define upper viability boundary.
"""
...
# 1.0× attack + 2.0× conversion (baseline attack)
BASE_PARAMS = [
...
SEEDS = list(range(6880, 6900))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c445_seed{seed}.db", n_populations=n_levels, mode="1X_2XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 445: 1.0× + 2.0× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c445_1x_at_2x_conv_results.json", 'w') as f:
        json.dump({"cycle_id": CYCLE_ID, "attack_multiplier": 1.0, "conversion_multiplier": 2.0, "coexistence_rate": coexist_count/len(results), "results": results}, f, indent=2)
