#!/usr/bin/env python3
"""
CYCLE 415: RECOVERY POINT AT 2.125× CONVERSION
...
Purpose:
  Narrow down critical conversion threshold.

Background:
  At 2.0× conversion: 0.71875× = 70% (no recovery)
  At 2.25× conversion: 0.71875× = 100% (full recovery)

  C415: 0.71875× + 2.125× - locate threshold

Design:
  - K declines from 600 to 200 over 40 cycles
  - Attack: 0.71875×, Conversion: 2.125×
  - 20 seeds
"""
...
CYCLE_ID = "C415"
CYCLE_NAME = "Recovery at 2.125x Conv"
MODE = "RECOVERY_2125XCONV"
...
# 0.71875× attack + 2.125× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.00215625, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.6375},
    {"attack": 0.00359375, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.53125},
    {"attack": 0.00575, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.425},
    {"attack": 0.008625, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.31875},
    {"attack": 0.01078125, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.255},
    {"attack": 0.0129375, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.2125}
]

SEEDS = list(range(6280, 6300))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run recovery point at 2.125× conversion experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c415_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 415: RECOVERY POINT AT 2.125× CONVERSION")
    ...
    print(f"Attack: 0.71875×, Conversion: 2.125×")
    print("=" * 80)
    print()
    ...
    output = {
        ...
        "conversion_multiplier": 2.125,
        "n_seeds": len(SEEDS),
        "coexistence_rate": coexist_count / len(results) if results else 0,
        "results": results
    }

    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c415_recovery_at_2125x_conv_results.json"
