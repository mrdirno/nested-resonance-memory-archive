#!/usr/bin/env python3
"""
CYCLE 340: SUDDEN ENVIRONMENTAL CHANGE
...
Purpose:
  Test food web resilience to sudden environmental shock.

Research Question:
  Can the food web survive abrupt resource reduction?

Background:
  C337-C339: Gradual oscillations - buffered well
  C340: Sudden step change - shock response

Design:
  - K starts at 600
  - At cycle 15000, K drops to 200 (shock)
  - Tests recovery/collapse
  - 20 seeds
...
CYCLE_ID = "C340"
CYCLE_NAME = "Sudden Environmental Change"
MODE = "SUDDEN_CHANGE"

CYCLES = 30000
K_BEFORE = 600
K_AFTER = 200  # Shock reduction
SHOCK_CYCLE = 15000
...
SEEDS = list(range(4700, 4720))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run sudden change experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c340_seed{seed}.db"
    ...
    for cycle in range(cycles):
        # Step change at SHOCK_CYCLE
        K = K_BEFORE if cycle < SHOCK_CYCLE else K_AFTER
    ...
    for i in range(n_levels):
        ...
        # Pre-shock populations (around cycle 14000-15000)
        if len(histories[i]) > 150:
            pre_shock[i] = np.mean(histories[i][140:150])
        else:
            pre_shock[i] = INITIAL[i]

        # Post-shock populations (around cycle 16000-17000)
        if len(histories[i]) > 170:
            post_shock[i] = np.mean(histories[i][160:170])
        else:
            post_shock[i] = 0

    coexist = all(finals[i] >= 0.5 for i in range(n_levels))
    survived_shock = all(post_shock[i] >= 0.5 for i in range(n_levels))

    return {
        "seed": seed,
        "finals": {f"L{i}": float(finals[i]) for i in range(n_levels)},
        "pre_shock": {f"L{i}": float(pre_shock[i]) for i in range(n_levels)},
        "post_shock": {f"L{i}": float(post_shock[i]) for i in range(n_levels)},
        "coexist": bool(coexist),
        "survived_shock": bool(survived_shock),
        "runtime": runtime
    }
...
def main():
    print("=" * 80)
    print(f"CYCLE 340: SUDDEN ENVIRONMENTAL CHANGE")
    ...
    print(f"K: {K_BEFORE} → {K_AFTER} at cycle {SHOCK_CYCLE}")
    print("=" * 80)
    print()
    ...
    for exp_num, seed in enumerate(SEEDS, 1):
        ...
        try:
            ...
            shock = "OK" if result["survived_shock"] else "CRASH"
            l0_pre = result["pre_shock"]["L0"]
            l0_post = result["post_shock"]["L0"]
            print(f"→ {status}, Shock={shock}, L0: {l0_pre:.0f}→{l0_post:.0f}")
    ...
    shock_survive = sum(1 for r in results if r["survived_shock"])
    print(f"\nFinal coexistence: {coexist_count}/{len(results)} ({coexist_count/len(results)*100:.0f}%)")
    print(f"Shock survival: {shock_survive}/{len(results)} ({shock_survive/len(results)*100:.0f}%)")

    print("\nPopulation change from shock:")
    for lvl in range(7):
        coexist_results = [r for r in results if r["coexist"]]
        if coexist_results:
            pre = np.mean([r["pre_shock"][f"L{lvl}"] for r in coexist_results])
            post = np.mean([r["post_shock"][f"L{lvl}"] for r in coexist_results])
            change = ((post - pre) / pre * 100) if pre > 0 else 0
            print(f"  L{lvl}: {pre:.0f} → {post:.0f} ({change:+.0f}%)")
    ...
    output = {
        ...
        "shock_survival_rate": shock_survive / len(results) if results else 0,
        "results": results
    }

    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c340_sudden_change_results.json"
