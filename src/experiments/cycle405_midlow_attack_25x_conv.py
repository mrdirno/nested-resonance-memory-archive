#!/usr/bin/env python3
"""
CYCLE 405: MID-LOW ATTACK + 2.5× CONVERSION - 40 CYCLES
...
Purpose:
  Find exact overshoot boundary.

Background:
  Overshoot boundary is between 0.5× and 0.75× attack:
  - C404: 0.5× + 2.5× conv - 100% (no overshoot)
  - C400: 0.75× + 2.5× conv - 80% (overshoot)

  C405: 0.625× + 2.5× conv - where does overshoot start?

Design:
  - K declines from 600 to 200 over 40 cycles
  - Attack: 0.625×, Conversion: 2.5×
  - 20 seeds
"""
...
CYCLE_ID = "C405"
CYCLE_NAME = "Mid-Low Attack 2.5x Conv"
MODE = "MIDLOW_ATK_25XCONV"
...
# 0.625× attack + 2.5× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.001875, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.75},
    {"attack": 0.003125, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.625},
    {"attack": 0.005, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.5},
    {"attack": 0.0075, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.375},
    {"attack": 0.009375, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.3},
    {"attack": 0.01125, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.25}
]

SEEDS = list(range(6080, 6100))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run mid-low attack + 2.5× conversion experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c405_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 405: MID-LOW ATTACK + 2.5× CONVERSION - 40 CYCLES")
    ...
    print(f"Attack: 0.625×, Conversion: 2.5×")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c405_midlow_attack_25x_conv_results.json"
