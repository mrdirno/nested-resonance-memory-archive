#!/usr/bin/env python3
"""
CYCLE 398: LOW ATTACK + 1.75× CONVERSION - 40 CYCLES
...
Purpose:
  Complete scaling curve at 0.5× attack.
...
Background:
  ...
  C398: 1.75× conv - expect ~85-90%?

Design:
  - K declines from 600 to 200 over 40 cycles
  - Attack: 0.5×, Conversion: 1.75×
  - 20 seeds
"""
...
CYCLE_ID = "C398"
CYCLE_NAME = "Low Attack 1.75x Conv"
MODE = "LOW_ATK_175XCONV"
...
# 0.5× attack + 1.75× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.0015, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.525},
    {"attack": 0.0025, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.4375},
    {"attack": 0.004, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.35},
    {"attack": 0.006, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.2625},
    {"attack": 0.0075, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.21},
    {"attack": 0.009, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.175}
]

SEEDS = list(range(5940, 5960))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run low attack + 1.75× conversion experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c398_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 398: LOW ATTACK + 1.75× CONVERSION - 40 CYCLES")
    ...
    print(f"Attack: 0.5×, Conversion: 1.75×")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c398_low_attack_175x_conv_results.json"
