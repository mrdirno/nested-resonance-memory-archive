#!/usr/bin/env python3
"""
CYCLE 534: 0.86× ATTACK AT 1.3× CONVERSION
...
Purpose:
  Map 0.86× profile at 1.3× (suppression window start).
"""
...
SEEDS = list(range(8661, 8681))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c534_seed{seed}.db", n_populations=n_levels, mode="086X_13XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 534: 0.86× + 1.3× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c534_086x_at_13x_conv_results.json", 'w') as f:
