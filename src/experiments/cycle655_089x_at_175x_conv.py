#!/usr/bin/env python3
"""
CYCLE 655: 0.89× ATTACK AT 1.75× CONVERSION
...
Purpose:
  Test if 0.89× maintains 90% lead.
"""
...
SEEDS = list(range(11081, 11101))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c655_seed{seed}.db", n_populations=n_levels, mode="089X_175XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 655: 0.89× + 1.75× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c655_089x_at_175x_conv_results.json", 'w') as f:
