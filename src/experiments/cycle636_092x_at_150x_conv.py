#!/usr/bin/env python3
"""
CYCLE 636: 0.92× ATTACK AT 1.5× CONVERSION
...
SEEDS = list(range(10701, 10721))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c636_seed{seed}.db", n_populations=n_levels, mode="092X_150XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 636: 0.92× + 1.5× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c636_092x_at_150x_conv_results.json", 'w') as f:
