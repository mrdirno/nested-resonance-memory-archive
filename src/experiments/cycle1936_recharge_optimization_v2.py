...
Optimizing 'recharge_base' using the newly discovered high-stability configuration:
comp_thresh=0.99, decomp_thresh=0.80, repro_prob=0.17, effective_prob=1.05.

Goal: Bridge the gap from 56% to >90% stability for N=14 (Dead Zone).
"""
...
def run_simulation(seed, recharge_val):
    ...
    # New best parameters from C1934/C1935
    n_initial = 14
    repro_prob = 0.17
    comp_thresh = 0.99
    decomp_thresh = 0.80
...
def main():
    print(f"CYCLE 1936: Recharge Optimization V2 | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 80)
    
    seeds = list(range(1936000, 1936075)) # 75 seeds
    print(f"Validation Seeds: {len(seeds)}")
    print("Using new config: repro=0.17, comp=0.99, decomp=0.80, effective_prob=1.05")
    print("-" * 40)
    
    # Sweep recharge
    recharge_range = [0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50]
