...
Does optimal N depend on resonance threshold (currently 0.5)?
"""
...
def run_experiment(seed, n_initial=25, resonance_threshold=0.5):
    """Run with variable resonance threshold."""
...
def main():
    print(f"CYCLE 1693: Resonance Threshold | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 70)
    print("Goal: Does optimal N depend on resonance threshold?")
    print("=" * 70)

    seeds = list(range(1693001, 1693051))  # 50 seeds

    res_thresholds = [0.3, 0.4, 0.5, 0.6, 0.7]
    ...
    # Summary
    print("\n" + "=" * 70)
    print("SUMMARY: OPTIMAL N BY RESONANCE THRESHOLD")
    print("=" * 70)
    print(f"\n{'Res Thresh':>12} | {'Optimal N':>10} | {'Success':>10}")
    print("-" * 38)
    ...
    for thresh in res_thresholds:
        ...
        best_rate = 0
        for n_init in population_sizes:
            subset = [r for r in all_results if r["resonance_threshold"] == thresh and r["n_initial"] == n_init]
            success = sum(1 for r in subset if r["coexist"]) / len(subset)
            if success > best_rate:
                best_rate = success
                best_n = n_init
        print(f"{thresh:12.2f} | {best_n:10d} | {best_rate*100:9.0f}%")
