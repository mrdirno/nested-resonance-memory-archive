#!/usr/bin/env python3
"""
CYCLE 924: 0.92× ATTACK AT 5.15× CONVERSION - POST-5.0× EXPLORATION
"""
...
BASE_PARAMS = [
    ...
    {"attack": 0.00276, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 1.545},
    {"attack": 0.0046, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 1.2875},
    {"attack": 0.00736, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 1.03},
    {"attack": 0.01104, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.7725},
    {"attack": 0.0138, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.618},
    {"attack": 0.01656, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.515}
]
SEEDS = list(range(16461, 16481))

def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c924_seed{seed}.db", n_populations=n_levels, mode="092X_515XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 924: 0.92× + 5.15× conv (POST-5.0×) | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c924_092x_at_515x_conv_results.json", 'w') as f:
        json.dump({"cycle_id": CYCLE_ID, "attack_multiplier": 0.92, "conversion_multiplier": 5.15, "coexistence_rate": coexist_count/len(results), "results": results}, f, indent=2)
