#!/usr/bin/env python3
"""
CYCLE 520: 0.62× ATTACK AT 1.75× CONVERSION
...
Purpose:
  Map micro-structure at 1.75× conversion.
"""
...
# 0.62× attack + 1.75× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.00186, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.525},
    {"attack": 0.0031, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.4375},
    {"attack": 0.00496, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.35},
    {"attack": 0.00744, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.2625},
    {"attack": 0.0093, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.21},
    {"attack": 0.01116, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.175}
]
SEEDS = list(range(8381, 8401))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c520_seed{seed}.db", n_populations=n_levels, mode="062X_175XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 520: 0.62× + 1.75× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c520_062x_at_175x_conv_results.json", 'w') as f:
        json.dump({"cycle_id": CYCLE_ID, "attack_multiplier": 0.62, "conversion_multiplier": 1.75, "coexistence_rate": coexist_count/len(results), "results": results}, f, indent=2)
