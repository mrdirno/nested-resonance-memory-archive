#!/usr/bin/env python3
"""
CYCLE 581: 0.86× ATTACK AT 0.86× CONVERSION
...
Purpose:
  Locate exact phase boundary between 0.85× and 0.87×.
"""
...
# 0.86× attack + 0.86× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.00258, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.258},
    {"attack": 0.0043, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.215},
    {"attack": 0.00688, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.172},
    {"attack": 0.01032, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.129},
    {"attack": 0.0129, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.1032},
    {"attack": 0.01548, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.086}
]
SEEDS = list(range(9601, 9621))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c581_seed{seed}.db", n_populations=n_levels, mode="086X_086XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 581: 0.86× + 0.86× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c581_086x_at_086x_conv_results.json", 'w') as f:
        json.dump({"cycle_id": CYCLE_ID, "attack_multiplier": 0.86, "conversion_multiplier": 0.86, "coexistence_rate": coexist_count/len(results), "results": results}, f, indent=2)
