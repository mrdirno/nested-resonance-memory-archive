#!/usr/bin/env python3
"""
CYCLE 396: MEDIUM-LOW ATTACK + 1.5× CONVERSION - 40 CYCLES
...
Purpose:
  Test linear scaling at 0.375× attack (compare with 0.25×).
...
Background:
  ...
  C396: 0.375× attack + 1.5× conv - expect ~50%?

Design:
  - K declines from 600 to 200 over 40 cycles
  - Attack: 0.375×, Conversion: 1.5×
  - 20 seeds
"""
...
CYCLE_ID = "C396"
CYCLE_NAME = "Med-Low Attack 1.5x Conv"
MODE = "MEDLOW_ATK_15XCONV"
...
# 0.375× attack + 1.5× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.001125, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.45},
    {"attack": 0.001875, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.375},
    {"attack": 0.003, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.3},
    {"attack": 0.0045, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.225},
    {"attack": 0.005625, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.18},
    {"attack": 0.00675, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.15}
]

SEEDS = list(range(5900, 5920))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run medium-low attack + 1.5× conversion experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c396_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 396: MEDIUM-LOW ATTACK + 1.5× CONVERSION - 40 CYCLES")
    ...
    print(f"Attack: 0.375×, Conversion: 1.5×")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c396_medlow_attack_15x_conv_results.json"
