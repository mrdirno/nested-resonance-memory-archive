#!/usr/bin/env python3
"""
CYCLE 725: 0.95× ATTACK AT 2.6× CONVERSION
...
BASE_PARAMS = [
    ...
    {"attack": 0.00285, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.78},
    {"attack": 0.00475, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.65},
    {"attack": 0.0076, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.52},
    {"attack": 0.0114, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.39},
    {"attack": 0.01425, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.312},
    {"attack": 0.0171, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.26}
]
SEEDS = list(range(12481, 12501))

def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c725_seed{seed}.db", n_populations=n_levels, mode="095X_260XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 725: 0.95× + 2.6× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c725_095x_at_260x_conv_results.json", 'w') as f:
        json.dump({"cycle_id": CYCLE_ID, "attack_multiplier": 0.95, "conversion_multiplier": 2.6, "coexistence_rate": coexist_count/len(results), "results": results}, f, indent=2)
