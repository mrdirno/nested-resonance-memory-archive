#!/usr/bin/env python3
"""
CYCLE 558: 1.0× ATTACK AT 1.0× CONVERSION
...
Purpose:
  Complete baseline attack/conversion point.
"""
...
SEEDS = list(range(9141, 9161))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c558_seed{seed}.db", n_populations=n_levels, mode="10X_10XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 558: 1.0× + 1.0× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c558_10x_at_10x_conv_results.json", 'w') as f:
        json.dump({"cycle_id": CYCLE_ID, "attack_multiplier": 1.0, "conversion_multiplier": 1.0, "coexistence_rate": coexist_count/len(results), "results": results}, f, indent=2)
