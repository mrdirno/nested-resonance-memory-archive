#!/usr/bin/env python3
"""
CYCLE 399: LOW ATTACK + 1.875× CONVERSION - 40 CYCLES
...
Purpose:
  Confirm step-function at 0.5× attack.

Background:
  At 0.5× attack (step function pattern):
  - C378: 1.5× conv - 75%
  - C398: 1.75× conv - 75%
  - C380: 2.0× conv - 100%

  C399: 1.875× conv - expect 75% (before step) or ~85% (mid-transition)?

Design:
  - K declines from 600 to 200 over 40 cycles
  - Attack: 0.5×, Conversion: 1.875×
  - 20 seeds
"""
...
CYCLE_ID = "C399"
CYCLE_NAME = "Low Attack 1.875x Conv"
MODE = "LOW_ATK_1875XCONV"
...
# 0.5× attack + 1.875× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.0015, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.5625},
    {"attack": 0.0025, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.46875},
    {"attack": 0.004, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.375},
    {"attack": 0.006, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.28125},
    {"attack": 0.0075, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.225},
    {"attack": 0.009, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.1875}
]

SEEDS = list(range(5960, 5980))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run low attack + 1.875× conversion experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c399_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 399: LOW ATTACK + 1.875× CONVERSION - 40 CYCLES")
    ...
    print(f"Attack: 0.5×, Conversion: 1.875×")
    print("=" * 80)
    print()
    ...
    output = {
        ...
        "conversion_multiplier": 1.875,
        "n_seeds": len(SEEDS),
        "coexistence_rate": coexist_count / len(results) if results else 0,
        "results": results
    }

    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c399_low_attack_1875x_conv_results.json"
