#!/usr/bin/env python3
"""
CYCLE 1937: EFFECTIVE PROBABILITY V2 (FINAL OPTIMIZATION)

Optimizing 'effective_prob' (composition probability) using the
high-stability configuration:
comp_thresh=0.99, decomp_thresh=0.80, repro_prob=0.17, recharge_base=0.20.

Goal: Push N=14 stability from 62.7% to >90% by fine-tuning the
composition flux.
"""
...
def run_simulation(seed, effective_prob_val):
    ...
    # New best parameters from C1936
    n_initial = 14
    repro_prob = 0.17
    comp_thresh = 0.99
    decomp_thresh = 0.80
...
def main():
    print(f"CYCLE 1937: Effective Probability V2 | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 80)
    
    seeds = list(range(1937000, 1937075)) # 75 seeds
    print(f"Validation Seeds: {len(seeds)}")
    print("Using new config: repro=0.17, comp=0.99, decomp=0.80, recharge=0.20")
    print("-" * 40)
    
    # Sweep effective_prob
    prob_range = [1.00, 1.05, 1.10, 1.15, 1.20, 1.25, 1.30, 1.35, 1.40, 1.45, 1.50]
