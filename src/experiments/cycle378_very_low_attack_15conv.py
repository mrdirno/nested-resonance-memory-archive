#!/usr/bin/env python3
"""
CYCLE 378: VERY LOW ATTACK + 1.5× CONVERSION - 40 CYCLES
...
Purpose:
  Test if very low attack continues improving survival with high conversion.

Background:
  C361: 0.5× attack + 1.0× conv - 5%
  C376: 0.75× attack + 1.5× conv - 90%
  C378: 0.5× attack + 1.5× conv - expect >90% or drop?

Design:
  - K declines from 600 to 200 over 40 cycles
  - Attack: 0.5×, Conversion: 1.5×
  - 20 seeds
"""
...
CYCLE_ID = "C378"
CYCLE_NAME = "Very Low Attack 1.5x Conv"
MODE = "VLOW_ATK_15CONV"
...
# 0.5× attack + 1.5× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.0015, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.45},
    {"attack": 0.0025, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.375},
    {"attack": 0.004, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.3},
    {"attack": 0.006, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.225},
    {"attack": 0.0075, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.18},
    {"attack": 0.009, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.15}
]

SEEDS = list(range(5540, 5560))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run very low attack + 1.5× conversion experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c378_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 378: VERY LOW ATTACK + 1.5× CONVERSION - 40 CYCLES")
    ...
    print(f"Attack: 0.5×, Conversion: 1.5×")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c378_very_low_attack_15conv_results.json"
