...
Test how initial population size affects the 80% limit.
Theory predicts: More initial agents → more compositions → higher success.
"""
...
def run_experiment(seed, n_initial=100):
    """Run with different initial population size."""
...
def main():
    print(f"CYCLE 1679: Initial Population | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 70)

    seeds = list(range(1679001, 1679051))  # 50 seeds

    initial_sizes = [25, 50, 100, 200, 500]
    all_results = []
    ...
    for n_init in initial_sizes:
        ...
        print(f"n={n_init:4d}: {rate*100:.0f}% coexist, {avg_depths:.1f} depths")

    # Test theoretical prediction
    print("\n" + "=" * 70)
    print("THEORETICAL PREDICTION")
    print("=" * 70)
    print("\nFrom C1678: P(success) = P(≥6 D1 | n_comps at 16%)")
    print("More initial agents → more compositions → higher success")

    for n_init in initial_sizes:
        # Estimate compositions in first 10 cycles
        # ~n_init/2 pairs × 10 cycles × 0.5 compose rate = 2.5*n_init
        n_comps = int(2.5 * n_init * 0.1)  # Only 10% are low-energy
        p = 0.16
        from scipy.stats import binom
        p_success = 1 - binom.cdf(5, n_comps, p)
        p_success = min(p_success * 0.95, 0.95)  # Cap at 95%
        print(f"n={n_init:4d}: est_comps={n_comps:3d}, predicted={p_success*100:.0f}%")
