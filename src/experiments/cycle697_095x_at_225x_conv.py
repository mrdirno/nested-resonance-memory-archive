#!/usr/bin/env python3
"""
CYCLE 697: 0.95× ATTACK AT 2.25× CONVERSION
...
SEEDS = list(range(11921, 11941))

def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c697_seed{seed}.db", n_populations=n_levels, mode="095X_225XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 697: 0.95× + 2.25× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c697_095x_at_225x_conv_results.json", 'w') as f:
