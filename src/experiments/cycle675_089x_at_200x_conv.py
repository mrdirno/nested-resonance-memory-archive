#!/usr/bin/env python3
"""
CYCLE 675: 0.89× ATTACK AT 2.0× CONVERSION
...
SEEDS = list(range(11481, 11501))

def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c675_seed{seed}.db", n_populations=n_levels, mode="089X_200XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 675: 0.89× + 2.0× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c675_089x_at_200x_conv_results.json", 'w') as f:
