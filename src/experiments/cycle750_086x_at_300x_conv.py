#!/usr/bin/env python3
"""
CYCLE 750: 0.86× ATTACK AT 3.0× CONVERSION - CRITICAL BOUNDARY TEST
...
SEEDS = list(range(12981, 13001))

def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c750_seed{seed}.db", n_populations=n_levels, mode="086X_300XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 750: 0.86× + 3.0× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c750_086x_at_300x_conv_results.json", 'w') as f:
