...
export const UIOverlay: React.FC<UIProps> = (props) => {
  ...
  return (
    <>
      {/* App Header */}
      <div className="fixed top-0 left-0 w-full h-[60px] flex items-center justify-center z-20 pointer-events-none bg-gradient-to-b from-black/80 to-transparent">
        <h1 className="text-lg font-extrabold uppercase tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-primary via-secondary to-tertiary animate-shimmer">
          Transcendental Dynamics
        </h1>
      </div>

      {/* Status Bar */}
      <div className="fixed top-[70px] left-1/2 -translate-x-1/2 glass-panel rounded-full px-5 py-2 flex items-center gap-3 z-20 shadow-2xl">
        <div className="w-2 h-2 rounded-full bg-success animate-pulse-fast" />
        <span className="text-xs font-bold font-mono tracking-widest flex items-center gap-2">
           <span ref={digitRefs.pos}>0</span> DIGITS
        </span>
        <div className="h-3 w-px bg-white/20 mx-1" />
        <button onClick={() => setConfig(c => ({...c, particleCount: 238999, speed: 5, amplitude: 50, exposure: 0.4}))} className="hover:scale-110 transition" title="Fast Preset">⚡</button>
        <button onClick={() => window.location.reload()} className="hover:scale-110 transition" title="Reset">🔄</button>
      </div>
