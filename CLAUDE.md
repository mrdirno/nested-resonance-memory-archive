...
5.  **Link the Evidence:** Every claim of capability must link to a specific file (log, script, or paper) that substantiates it. This is now a gate: every number written in digits in `README.md` prose must sit inside a passage indexed in `data/claims.json`, with a status and a verbatim excerpt from the committed file that holds it. `python3 tools/archive/claims_index.py --check` fails otherwise, and the archive-health workflow runs it on every push to main and every pull request (`tests/archive/test_claims_index.py`). A README edit that adds or changes a number edits the index in the same commit.
...
HALO is the live browser instrument. The pre-registered memory grid has already run over 60 cells at 4.19 million particles. Its contrast is confounded by spatial sampling and static radial profiles, so it neither establishes nor retires NRM. Read `analysis/2026-09-02_cross_epoch_memory_preregistered.md` before proposing a successor experiment. The replacement estimator, frozen with five falsifiers on 2026-09-05, finds that grid unmeasurable (0 of 60 runs; `analysis/2026-09-06_memory_estimator_qualification.md`), so a successor grid must first be shown measurable in a pilot: a regime whose dynamics do not erase the seed, a relic footprint of at least 8 effective cells, no static runs (fields that do not move between epochs). That pilot ran on 2026-09-07 (`analysis/2026-09-07_measurability_pilot.md`): 21 runs over seven conditions drawn from four shipped field configurations, 0 measurable, and no branch of its own frozen rule fires. Do not register a confirmatory grid under the frozen protocol; and note that the recorded 60-run grid was produced by a page build replaced on 2026-09-02, so no per-run number from it describes today's chamber. Ring 18 (2026-09-09, `analysis/2026-09-09_measurability_noise_and_instrument_identity.md`) added the instrument pin (`experiments/halo/instrument_identity.py`; a whole-file page hash over-reports) and retired two claims: "F4 pass" (vacuous in both scored arms) and the pilot's "six independent draws" (three seeds under two instruments), so how much of "measurable" is chance is not identified by the recorded data. For the six recorded runs that clear the 12-epoch floor, the seed-balance gate E5b, not the floor, is what binds; in the generated regime it was the seed lock E3 (Ring 24).

Rings 21–29 then generated a support the frozen scorer can measure: `spinchladni` at gain/loss 0 with the drive cut to `fieldExp 1.7`, self-gravity 0.3 / 0.32 / 0.35, on one pinned instrument (`sim_digest 40bfdb68`). There it returned its first "qualified" (Ring 29, `analysis/2026-09-21_sg032_third_condition.md`). That word describes the nine recorded runs on that support, not the support itself: Ring 30 found that 48 of the 64 equally valid mirrored readings of the same data return it. It says nothing about memory. Ring 30 (`analysis/2026-09-22_relic_intervention.md`) ran the registered intervention on it, inverting the matter through the centre after every zoom-out. Every gate passed, and the pre-registered rule returned NOT DECIDABLE. Its open questions named the successor: a within-run carrier statistic qualified by its own synthetic receipt, and the positive control's power on fresh runs. Ring 32 (`analysis/2026-09-24_within_run_carrier_statistic.md`) built that statistic. Its design was pushed before its synthetic receipt existed, and the receipt passes on the twelve Ring 29 control runs. It registered eighteen fresh runs (nine new seeds; the identity arm and the positive control) with two reproduction checks and a fixed branch table, and launched them after the receipt was pushed. Ring 33 scored them once (the pre-registration's `## Results`), and the branch is CONFIRMED. The positive control calls Π in 5 of its 9 scored fresh runs and L in none; the identity arm calls L in 6 of 9; V1 passes. That says nothing about memory, nor about whether the matter or the solver's warm start carries anything. It licensed one thing, and Ring 34 did it (`analysis/2026-09-24_decided_arm_invert_matter.md`): the decided arm (`invert_matter`) was registered on those nine seeds and pushed (P `06e321c1`) before any of its runs, by a driver that refuses to launch otherwise, beside an untouched gate on the same seeds. Scored once, the branch is NO PREFERENCE: the decided arm calls neither frame in any of its 9 scored runs, while the gate calls L in 4 of the 9 scored. It names no carrier, it is not a null, and it says nothing about memory; a successor must separate "neither carries it", "both cancel" and "too few calls", on fresh seeds. The newest ring's open questions on the HALO page are the queue.
