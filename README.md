...
**Evidence index:** each number this README gives in digits outside code is listed in [claims.json](data/claims.json) with a status (established, qualified, open or retired) and an excerpt from the committed file that holds it, and [a test](tests/archive/test_claims_index.py) checks that on every push to main. Agents can start at [llms.txt](https://mrdirno.github.io/nested-resonance-memory-archive/llms.txt).
...
* **A replacement estimator the recorded grid cannot support:** An estimator that shrinks each previous snapshot about the chamber's centre onto the current one, compares only the region both snapshots cover and removes the spherically symmetric part, with five falsifiers declared in advance, was frozen on 2026-09-05 and scored once on the same 60 runs. It declares 0 of 60 runs measurable: 179 of 1,320 run-epochs (one epoch of one run) pass its eligibility gates, so its result is "insufficient support"; its recovery test could not be evaluated, and its sensitivity is stated on synthetic controls only. The three seeds of a condition are near-copies in 17 of 20 conditions, the shrunken copy of the previous snapshot it compares against has fewer than 8 effective cells (typically 4 to 7) in about half of the epochs where the matter has collapsed, and 12 runs are static fields that barely change between epochs. This is a result about the grid, not about NRM; the next grid has to be shown measurable first. [Protocol](docs/halo/2026-09-05_memory_estimator_qualification_protocol.md) · [Result and controls](analysis/2026-09-06_memory_estimator_qualification.md)
* **A pilot that found nothing measurable, and the recorded grid's instrument:** Before registering another grid, a pilot was frozen and run: one arm, 21 runs at 4,194,304 particles over seven conditions drawn from four of the page's shipped field configurations, scored once by the same frozen estimator. **0 of 21 runs are measurable** and 37 of 462 run-epochs eligible, so none of the seven conditions it ran gives that estimator something to measure. A reported but unscored variant that lowers the footprint floor from 8 to 4 admits 4 runs. The two conditions registered only to measure drift did not reproduce their recorded numbers; seven controlled re-runs on one run tag show that run comes back byte for byte under the page as it stood before one commit on 2026-09-02. The other 59 recorded runs share its provenance and were not re-run. That build was already pinned, by revision and checksum, in the [2026-09-02 audit](analysis/2026-09-02_cross_epoch_memory_preregistered.md) four days earlier; the re-runs confirmed the pin rather than establishing it. Nothing about NRM is decided in either direction. [Pre-registration](docs/halo/2026-09-06_measurability_pilot_preregistration.md) · [Result and controls](analysis/2026-09-07_measurability_pilot.md)
* **An instrument identity, and a question that turned out not to be answerable:** The recorded runs never named the build that produced them. A file hash is the wrong name for it — the page carries its own history as a comment, so publishing prose changes the hash while changing no simulation, and on the four page revisions with known behaviour a whole-file hash gives four answers where there are two. A digest over the GPU program, the named physics constants and the drive gives exactly two, and every future run now records the build that made it. Separately: the recorded records had been distinguishing those two builds by accident, through a header that froze at 22 names while the data grew to 28 columns; and the follow-up question the pilot raised — how much of "measurable" is chance — cannot be answered from these runs, because no condition was ever repeated without also changing the instrument. [Result and tools](analysis/2026-09-09_measurability_noise_and_instrument_identity.md)
* **A third condition, and the estimator's first "qualified" verdict:** Generating at the cut drive (`fieldExp 1.7`) instead of selecting from the record, four rings walked one axis — self-gravity — until three conditions (0.3 / 0.32 / 0.35) each had three measurable runs on one pinned instrument. The third condition was pre-registered with its seeds, directories and decision rule before any of its registered runs existed, then found measurable **3 of 3 on both pre-committed seed triples**; every falsifier passes (the false-positive one by its 2026-09-06 synthetic receipt) and the frozen scorer returns **"qualified"** for the first time. That word describes those nine recorded runs on exactly this support, and nothing more. A run mirrored through the chamber's centre is an equally likely run, so the same data can be read in 64 equally valid mirrored versions, and Ring 30 found that 48 of them return "qualified". Detection is sparse (4 of 9 and 5 of 9 runs), is not reproduced within a condition, and its cross-condition hits follow the seeds. No memory result is claimed; the fixed next step was a registered intervention on this support, which Ring 30 ran. [Pre-registration and result](analysis/2026-09-21_sg032_third_condition.md) · [Ring 28, the second condition](analysis/2026-09-19_sg035_second_triple.md)
* **An intervention that ran to its end without a decision:** Between ten-second epochs the chamber zooms out, and each run carries a lopsidedness (more matter in the north or the south half) across that moment; the estimator's score lives mostly in it. Ring 30 asked what carries it: the particles themselves, or the gravity solver's leftover potential, which the page does not rescale when it zooms out. Right after every zoom-out, it moved every particle to the opposite point through the chamber's centre. There were 28 runs on one pinned instrument, with their seeds, folders, checks and decision rule committed before any registered run existed. Every pre-set check passed, and the untouched control runs (the gate arm) reproduced the earlier runs byte for byte, with every score equal. Each flipped run was scored twice: as recorded, and with the flips undone. In the tested runs, where only the particles were flipped, the score did not clearly favour either reading, so the pre-registered rule returns **not decidable**; the rule does not say why. The check runs, which flip the leftover potential as well, did not clearly favour the undone reading either. No memory result is claimed either way. Before another tested arm runs, the next steps are a pre-registered measurement, on fresh runs, of how often the check runs give their expected answer, and a statistic read within each run, qualified first on its own synthetic test. That statistic now exists. Its design was pushed before its synthetic test, and it passed that test on the recorded control runs. The fresh runs that count how often the check runs give their expected answer were then scored, once. Read by the new statistic, the check runs gave their expected answer in enough runs for the rule written in advance, so that rule allows exactly one next step: registering the tested arm, on seeds already named, before any of its runs exist. Read by the original score, the same fresh check runs again did not clearly favour the undone reading; the two readings are not compared. That tested arm was then registered on the named seeds, with its rule written in advance, and pushed before any of its runs existed. Scored once by the same statistic, the runs where only the particles were flipped favoured neither reading; the untouched runs on the same seeds showed the carried lopsidedness often enough for the rule to decide the flipped ones. So the rule names neither carrier and does not say why; that is not evidence that neither carries it, and no memory result is claimed. [The tested arm's pre-registration and result](analysis/2026-09-24_decided_arm_invert_matter.md) · [The statistic's pre-registration and result](analysis/2026-09-24_within_run_carrier_statistic.md) · [Pre-registration and result](analysis/2026-09-22_relic_intervention.md) · [Ring 29, the first "qualified"](analysis/2026-09-21_sg032_third_condition.md)
...
The September 5 hardening pass runs 55 tests against allocation, real system monitoring, SQLite/files and plotted output. Whole-package statement coverage is 95.93%, up from the initial 49.55%. This clears the coverage threshold; a new release still requires the repository's evidence of usefulness and independence. [Audit record](archive/reports/2026-09-04_archive_stewardship.md) · [Coverage run](archive/reports/2026-09-05_bcp_hardening_gate.txt)
