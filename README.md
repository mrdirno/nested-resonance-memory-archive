...
* **A pilot that found nothing measurable, and the recorded grid's instrument:** Before registering another grid, a pilot was frozen and run: one arm, 21 runs at 4,194,304 particles over seven shipped field configurations, scored once by the same frozen estimator. **0 of 21 runs are measurable** and 37 of 462 run-epochs eligible, so no configuration the page ships gives that estimator something to measure. The two conditions registered only to measure drift did not reproduce their recorded numbers; seven controlled re-runs on one run tag show the recorded 60-run grid comes back byte for byte under the page as it stood before one commit on 2026-09-02. That build was already pinned, by revision and checksum, in the [2026-09-02 audit](analysis/2026-09-02_cross_epoch_memory_preregistered.md) four days earlier; the re-runs confirmed the pin rather than establishing it. Nothing about NRM is decided in either direction. [Pre-registration](docs/halo/2026-09-06_measurability_pilot_preregistration.md) · [Result and controls](analysis/2026-09-07_measurability_pilot.md)
...
* **A third condition, and the estimator's first "qualified" verdict:** Generating at the cut drive (`fieldExp 1.7`) instead of selecting from the record, five rings walked one axis — self-gravity — until three conditions (0.3 / 0.32 / 0.35) each had three measurable runs on one pinned instrument. The third condition was pre-registered with its seeds, directories and decision rule before any run existed, then found measurable **3 of 3 on both pre-committed seed triples**; every falsifier passes (the false-positive one by its 2026-09-06 synthetic receipt) and the frozen scorer returns **"qualified"** for the first time. That word means the estimator can measure on exactly this support, and nothing more: detection is sparse (4 of 9 and 5 of 9 runs), is not reproduced within a condition, and its cross-condition hits follow the seeds. No memory result is claimed; the fixed next step is a registered intervention on this support. [Pre-registration and result](analysis/2026-09-21_sg032_third_condition.md) · [Ring 28, the second condition](analysis/2026-09-19_sg035_second_triple.md)
* **An intervention that ran to its end without a decision:** Between ten-second epochs the chamber zooms out, and each run carries a lopsidedness (more matter in the north or the south half) across that moment; the estimator's score lives mostly in it. Ring 30 asked what carries it: the particles themselves, or the gravity solver's leftover potential, which the page does not rescale when it zooms out. Right after every zoom-out, it moved every particle to the opposite point through the chamber's centre. There were 28 runs on one pinned instrument, with their seeds, folders, checks and decision rule committed before any run existed. Every pre-set check passed, and the untouched comparison runs reproduced the earlier runs byte for byte, with every score equal. Each flipped run was scored twice: as recorded, and with the flips undone. In the tested runs, where only the particles were flipped, the score did not clearly favour either reading, so the pre-registered rule returns **not decidable**; the rule names no cause. The check runs, which flip the leftover potential as well, did not clearly favour the undone reading either. No memory result is claimed either way. Before another tested arm runs, the next step is a pre-registered measurement, on fresh runs, of how often the check runs give their expected answer. [Pre-registration and result](analysis/2026-09-22_relic_intervention.md) · [Ring 29, the qualified support](analysis/2026-09-21_sg032_third_condition.md)
