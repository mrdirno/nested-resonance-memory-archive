...
class PatternMemory:
    ...
    def compute_semantic_similarity(
        ...
        v_i = Vector(embedding_i)
        v_j = Vector(embedding_j)
        
        # Cosine similarity from Vector class returns -1 to 1
        sim = v_i.cosine_similarity(v_j)

        # Clamp to [0, 1] (convert from [-1, 1] if needed, though cosine_similarity usually is used directly)
        # The original implementation normalized it: (sim + 1) / 2
        # We will keep that behavior for compatibility if the embeddings are not non-negative.
