...
def create_cluster(N_AGENTS: int, WORLD_SIZE: float, comp_engine: CompositionEngine, initial_phase: float, cluster_id_prefix: str) -> Tuple[FractalAgent, List[FractalAgent]]:
    ...
    parent = clusters[0] # Assuming one large cluster
    constituents = [a for a in agents if a.state.parent_id == parent.state.agent_id]
    
    # Update parent's phase to represent the average
    sin_sum = sum(np.sin(c.state.phase) for c in constituents)
    cos_sum = sum(np.cos(c.state.phase) for c in constituents)
    parent.state.phase = np.arctan2(sin_sum, cos_sum) % (2 * np.pi)

    print(f"  Formed cluster '{cluster_id_prefix}' with {len(constituents)} constituents (Initial Phase: {parent.state.phase:.2f}).", flush=True)
...
def run_experiment():
    print("MOG ONLINE: Cycle 1986 - AND Gate Implementation (Bang-Bang Drive)", flush=True)
    print("Hypothesis: Using a Bang-Bang (Sign-based) driving force will robustly implement an AND gate.", flush=True)
    ...
    K_AND_DRIVE = 0.5 # Bang-Bang drive strength (similar to C1985's effective K)
    CYCLES = 200

    TARGET_PHASE_0 = 0.0
    TARGET_PHASE_PI = np.pi
    PHASE_THRESHOLD = 0.5 # For sensing if input is 'near 0' or 'near PI' (and success criteria)
    ...
    for input_a_init, input_b_init in INPUT_STATES:
        ...
        for t in range(CYCLES):
            ...
            for child in output_c_constituents:
                ...
                # External force (AND Gate Drive) - BANG-BANG (SIGN) CONTROL
                # err = target - current
                err = output_target_phase - child.state.phase
                f_and_drive = K_AND_DRIVE * np.sign(np.sin(err)) # Using np.sign(np.sin(err)) for Bang-Bang
