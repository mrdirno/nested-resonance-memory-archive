...
Test if strict semantic segregation allows Concepts to survive.
Mechanism:
- Concepts only bond with Concepts of the same tag.
- Concepts only reproduce if they have high energy.
- Noise agents interact normally.
"""
...
def run_assortative_test(seed, n_logos):
        ...
        for d in range(N_DEPTHS - 1):
            ...
            while i < len(agents) - 1:
                ...
                if sim >= COMP_THRESH:
                    ...
                    # ASSORTATIVE LOGIC:
                    # Bond ONLY if tags match (or both are Noise)
                    # LOGOS + Noise = REJECT (No Bond)
                    ...
                    if bond_allowed:
                        ...
                        i += 1 # Skip bonding, try next pair
                else:
                    i += 1
...
def main():
    print(f"CYCLE 1941: Assortative Mating | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 80)
    print("Testing Selective Semantic Bonding (Segregation).")
    print("Rule: LOGOS only bonds with LOGOS.")
    print("=" * 80)

    test_cases = [1, 3, 5, 10] # Retry lower numbers
    seeds = [1941, 1942, 1943]
    ...
    results = {}
    for n in test_cases:
        batch = [run_assortative_test(s, n) for s in seeds]
    ...
    success = any(results[n][0] > 50 for n in test_cases)
    
    if success:
        print("SUCCESS: Assortative Mating enables concept survival.")
        # Find min viable N
        min_n = next((n for n in test_cases if results[n][0] > 50), None)
        print(f"Minimum Viable Seed: N={min_n}")
    else:
        print("FAILURE: Even with segregation, concepts die out.")
        print("Possible cause: Noise reproduces faster or out-competes for energy.")
