> # ⚠ SUPERSEDED — RETRACTED 2026-09-02
>
> **This report's verdict of "CONFIRMED" is not supported by its own data, and the
> experiment it reports cannot answer the question it poses.** It is preserved here
> unaltered as a record; do not cite it.
>
> **1. The verdict never read the comparison it lost.** `generate_report()` computes
> `verdict_text = "CONFIRMED" if (hypothesis_1_confirmed or hypothesis_2_confirmed)`, and
> both variables read only `p_values["trans_vs_noise"]`. The `trans_vs_rational` comparison
> is computed, printed into the table, and never enters the verdict. Survival was
> not-significant (p=1.0, both conditions zero); coherence was significant;
> `False or True` → "CONFIRMED". The stated hypothesis explicitly claimed superiority over
> "incoherent pseudo-random noise **or simple commensurate cycles**" — it lost to
> commensurate cycles on all four metrics (t = −15.7, −33.3, −4.9).
>
> **2. But the underlying comparison is also invalid.** Survival here requires
> phase-locking to the driving field, which requires `min(driving frequency) < H`. The
> published run used H = 1.5. The rational triple's lowest frequency is *exactly* 1.5;
> the transcendental triple's lowest is φ ≈ 1.618. That 0.118 rad/s gap is the entire
> result. Sweeping H (2026-09-02): at H=1.5 rational 0.269 / transcendental 0.000; at
> H=1.7 they are 0.997 / 0.956; at H≥2.0 both are 1.000. **The winner flips with a
> parameter that has nothing to do with number theory.**
>
> **3. Neither control isolates the variable.** The "noise" condition resamples phase
> uniformly every step, so it has zero temporal correlation — it is a control for *no
> coherent driving*, not for *incommensurate driving*. The matched control that was
> missing — random incommensurate triples of the same magnitude — shows commensurate vs
> incommensurate is **not detectable** (p = 0.17 at H=1.7, p = 0.32 at H=1.9), and
> (π, e, φ) is **indistinguishable** from random incommensurate triples at H=1.9.
>
> **Superseded by:** `analysis/2026-09-02_phase_locking_governs_survival.md`, which
> identifies the actual governing variable and confirms it at 100% accuracy on 42 triples
