#!/usr/bin/env python3
"""
CYCLE 388: OPTIMAL CONFIGURATION - FAST DECLINE (20 CYCLES)
...
Purpose:
  Test if optimal configuration maintains 100% at faster K decline.

Background:
  C380: 0.5× attack + 2.0× conv at 40 cycles - 100%
  C388: Same configuration at 20 cycles - expect <100%?

Design:
  - K declines from 600 to 200 over 20 cycles (fast)
...
CYCLE_ID = "C388"
CYCLE_NAME = "Optimal Fast Decline"
MODE = "OPT_FAST_DECLINE"

CYCLES = 30000
K_START = 600
K_END = 200
DECLINE_CYCLES = 20  # Fast decline
...
SEEDS = list(range(5740, 5760))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run optimal config at fast decline."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c388_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 388: OPTIMAL CONFIGURATION - FAST DECLINE (20 CYCLES)")
    ...
    print(f"K: {K_START} → {K_END} over {DECLINE_CYCLES} cycles (fast)")
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c388_optimal_fast_decline_results.json"
