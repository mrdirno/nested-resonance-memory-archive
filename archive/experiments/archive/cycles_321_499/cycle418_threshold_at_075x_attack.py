#!/usr/bin/env python3
"""
CYCLE 418: CRITICAL THRESHOLD AT 0.75× ATTACK
...
Purpose:
  Map critical conversion threshold at higher attack.

Background:
  At 0.71875× attack: C_critical ≈ 2.0×
  Prediction: Higher attack needs higher conversion

  C418: 0.75× attack + 2.0× conv - below threshold?

Design:
  - K declines from 600 to 200 over 40 cycles
  - Attack: 0.75×, Conversion: 2.0×
  - 20 seeds
"""
...
CYCLE_ID = "C418"
CYCLE_NAME = "Threshold at 0.75x Attack"
MODE = "THRESHOLD_075X"
...
# 0.75× attack + 2.0× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.00225, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.6},
    {"attack": 0.00375, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.5},
    {"attack": 0.006, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.4},
    {"attack": 0.009, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.3},
    {"attack": 0.01125, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.24},
    {"attack": 0.0135, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.2}
]

SEEDS = list(range(6340, 6360))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run critical threshold at 0.75× attack experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c418_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 418: CRITICAL THRESHOLD AT 0.75× ATTACK")
    ...
    print(f"Attack: 0.75×, Conversion: 2.0×")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c418_threshold_at_075x_attack_results.json"
