#!/usr/bin/env python3
"""
CYCLE 360: HIGH ATTACK RATE - 40 CYCLES
...
Purpose:
  Test parameter sensitivity - how attack rate affects critical rate threshold.

Background:
  C359: 40 cycles with base attack - 50% survival
  C360: 40 cycles with 1.5× attack rates

Hypothesis:
  Higher attack rates → need slower K decline (higher critical rate)

Design:
  - K declines from 600 to 200 over 40 cycles
  - Rate: 10 K/cycle
  - Attack rates: 1.5× baseline
  - 20 seeds
"""
...
CYCLE_ID = "C360"
CYCLE_NAME = "High Attack Rate 40 Cycles"
MODE = "HIGH_ATTACK_40"
...
BASE_PARAMS = [
    ...
    {"attack": 0.0045, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.3},
    {"attack": 0.0075, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.25},
    {"attack": 0.012, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.2},
    {"attack": 0.018, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.15},
    {"attack": 0.0225, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.12},
    {"attack": 0.027, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.10}
]

SEEDS = list(range(5180, 5200))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run high attack rate experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c360_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 360: HIGH ATTACK RATE - 40 CYCLES")
    ...
    print(f"Attack rates: 1.5× baseline")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c360_high_attack_40cycles_results.json"
