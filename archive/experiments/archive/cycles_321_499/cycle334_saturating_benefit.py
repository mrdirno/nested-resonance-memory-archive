#!/usr/bin/env python3
"""
CYCLE 334: SATURATING BENEFIT TEST
...
Purpose:
  Test saturating benefit function to create true optimum.

Research Question:
  Does diminishing returns create directional selection?

Background:
  C328-C333: No evolution (flat benefit)
  C334: Saturating benefit + squared cost

Design:
  - Benefit = sqrt(trait) - diminishing returns
  - Cost = trait² - accelerating cost
  - Optimum should exist at intersection
  - 20 seeds
...
SEEDS = list(range(4100, 4120))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run saturating benefit experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c334_seed{seed}.db"
            ...
            if ns[prey_lvl] > 0:
                ...
                for pred in pops[lvl]:
                    ...
                    if avail > 0:
                        ...
                        # SATURATING BENEFIT: sqrt(trait) for attack
                        effective_trait = np.sqrt(trait_mult)
                        attack = p["attack"] * effective_trait
...
def main():
    print("=" * 80)
    print(f"CYCLE 334: SATURATING BENEFIT")
    ...
    print(f"Benefit: sqrt(trait), Cost: trait²")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c334_saturating_benefit_results.json"
