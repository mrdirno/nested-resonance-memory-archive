#!/usr/bin/env python3
"""
CYCLE 369: HIGH HANDLING TIME - 40 CYCLES
...
Purpose:
  Test handling time sensitivity at 40 cycles.

Background:
  C359: 40 cycles, baseline - 50% survival
  C369: 40 cycles, 1.5× handling time

Hypothesis:
  Higher handling time → less predation → different survival

Design:
  - K declines from 600 to 200 over 40 cycles
  - Handling times: 1.5× baseline
  - 20 seeds
"""
...
CYCLE_ID = "C369"
CYCLE_NAME = "High Handling Time 40 Cycles"
MODE = "HIGH_HANDLING_40"
...
BASE_PARAMS = [
    ...
    {"attack": 0.003, "h": 0.03, "e_con": 0.3, "e_gain": 0.5, "conv": 0.3},
    {"attack": 0.005, "h": 0.045, "e_con": 0.4, "e_gain": 0.6, "conv": 0.25},
    {"attack": 0.008, "h": 0.06, "e_con": 0.5, "e_gain": 0.8, "conv": 0.2},
    {"attack": 0.012, "h": 0.075, "e_con": 0.6, "e_gain": 1.0, "conv": 0.15},
    {"attack": 0.015, "h": 0.09, "e_con": 0.7, "e_gain": 1.2, "conv": 0.12},
    {"attack": 0.018, "h": 0.105, "e_con": 0.8, "e_gain": 1.4, "conv": 0.10}
]

SEEDS = list(range(5360, 5380))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run high handling time experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c369_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 369: HIGH HANDLING TIME - 40 CYCLES")
    ...
    print(f"Handling times: 1.5× baseline")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c369_high_handling_40cycles_results.json"
