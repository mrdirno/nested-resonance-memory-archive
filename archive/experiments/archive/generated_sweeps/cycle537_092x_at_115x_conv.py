#!/usr/bin/env python3
"""
CYCLE 537: 0.92× ATTACK AT 1.15× CONVERSION
...
Purpose:
  Test if 0.92× invariance extends to 1.15×.
"""
...
SEEDS = list(range(8721, 8741))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c537_seed{seed}.db", n_populations=n_levels, mode="092X_115XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 537: 0.92× + 1.15× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c537_092x_at_115x_conv_results.json", 'w') as f:
