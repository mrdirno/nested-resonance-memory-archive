#!/usr/bin/env python3
"""
CYCLE 411: DIP ONSET AT 2.0× CONVERSION
...
Purpose:
  Test if dip onset (0.65625×) persists at 2.0× conversion.

Background:
  At 2.5× conversion: 0.65625× = 90% (dip onset)
  At 2.0× conversion: 0.6875× = 90% (notch confirmed)

  C411: 0.65625× + 2.0× - dip onset at lower conv?
...
CYCLE_ID = "C411"
CYCLE_NAME = "Dip Onset at 2x Conv"
MODE = "DIP_ONSET_2XCONV"
...
# 0.65625× attack + 2.0× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.00196875, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.6},
    {"attack": 0.00328125, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.5},
    {"attack": 0.00525, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.4},
    {"attack": 0.007875, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.3},
    {"attack": 0.00984375, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.24},
    {"attack": 0.0118125, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.2}
]

SEEDS = list(range(6200, 6220))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run dip onset at 2.0× conversion experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c411_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 411: DIP ONSET AT 2.0× CONVERSION")
    ...
    print(f"Attack: 0.65625×, Conversion: 2.0×")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c411_dip_onset_at_2x_conv_results.json"
