#!/usr/bin/env python3
"""
CYCLE 629: 0.95× ATTACK AT 1.4× CONVERSION
...
SEEDS = list(range(10561, 10581))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c629_seed{seed}.db", n_populations=n_levels, mode="095X_140XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 629: 0.95× + 1.4× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c629_095x_at_140x_conv_results.json", 'w') as f:
