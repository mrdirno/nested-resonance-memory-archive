#!/usr/bin/env python3
"""
CYCLE 607: 0.89× ATTACK AT 1.15× CONVERSION
...
Purpose:
  Test if 0.89× continues recovering beyond 1.1× as anti-resonator.
"""
...
# 0.89× attack + 1.15× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.00267, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.345},
    {"attack": 0.00445, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.2875},
    {"attack": 0.00712, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.23},
    {"attack": 0.01068, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.1725},
    {"attack": 0.01335, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.138},
    {"attack": 0.01602, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.115}
]
SEEDS = list(range(10121, 10141))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c607_seed{seed}.db", n_populations=n_levels, mode="089X_115XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 607: 0.89× + 1.15× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c607_089x_at_115x_conv_results.json", 'w') as f:
        json.dump({"cycle_id": CYCLE_ID, "attack_multiplier": 0.89, "conversion_multiplier": 1.15, "coexistence_rate": coexist_count/len(results), "results": results}, f, indent=2)
