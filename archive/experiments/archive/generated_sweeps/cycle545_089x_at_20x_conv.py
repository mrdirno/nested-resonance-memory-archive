#!/usr/bin/env python3
"""
CYCLE 545: 0.89× ATTACK AT 2.0× CONVERSION
...
Purpose:
  Complete 0.89× profile at high conversion.
"""
...
SEEDS = list(range(8881, 8901))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c545_seed{seed}.db", n_populations=n_levels, mode="089X_20XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 545: 0.89× + 2.0× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c545_089x_at_20x_conv_results.json", 'w') as f:
