#!/usr/bin/env python3
"""
CYCLE 428: 0.7× ATTACK AT 3.0× CONVERSION
...
Purpose:
  Complete phase diagram grid.

Background:
  At 0.7× + 2.0× conv: 100%
  At 0.7× + 2.5× conv: 90%

  C428: 0.7× + 3.0× - complete row

Design:
  - K declines from 600 to 200 over 40 cycles
  - Attack: 0.7×, Conversion: 3.0×
  - 20 seeds
"""
...
# 0.7× attack + 3.0× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.0021, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.9},
    {"attack": 0.0035, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.75},
    {"attack": 0.0056, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.6},
    {"attack": 0.0084, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.45},
    {"attack": 0.0105, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.36},
    {"attack": 0.0126, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.3}
]

SEEDS = list(range(6540, 6560))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run 0.7× attack at 3.0× conversion experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c428_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 428: 0.7× ATTACK AT 3.0× CONVERSION")
    ...
    print(f"Attack: 0.7×, Conversion: 3.0×")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c428_07x_at_3x_conv_results.json"
