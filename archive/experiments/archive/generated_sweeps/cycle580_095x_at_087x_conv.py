#!/usr/bin/env python3
"""
CYCLE 580: 0.95× ATTACK AT 0.87× CONVERSION
...
Purpose:
  Complete 0.87× profile - test 0.95× at phase inversion zone.
"""
...
# 0.95× attack + 0.87× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.00285, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.261},
    {"attack": 0.00475, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.2175},
    {"attack": 0.0076, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.174},
    {"attack": 0.0114, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.1305},
    {"attack": 0.01425, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.1044},
    {"attack": 0.0171, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.087}
]
SEEDS = list(range(9581, 9601))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c580_seed{seed}.db", n_populations=n_levels, mode="095X_087XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 580: 0.95× + 0.87× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c580_095x_at_087x_conv_results.json", 'w') as f:
        json.dump({"cycle_id": CYCLE_ID, "attack_multiplier": 0.95, "conversion_multiplier": 0.87, "coexistence_rate": coexist_count/len(results), "results": results}, f, indent=2)
