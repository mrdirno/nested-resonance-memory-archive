#!/usr/bin/env python3
"""
CYCLE 531: 0.92× ATTACK AT 1.3× CONVERSION
...
Purpose:
  Test 0.92× invariance at suppression window start.
"""
...
def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c531_seed{seed}.db", n_populations=n_levels, mode="092X_13XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 531: 0.92× + 1.3× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c531_092x_at_13x_conv_results.json", 'w') as f:
