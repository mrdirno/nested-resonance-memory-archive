#!/usr/bin/env python3
"""
CYCLE 533: 0.86× ATTACK AT 1.5× CONVERSION
...
Purpose:
  Test 1.5× universal transition at 0.86× peak.
"""
...
SEEDS = list(range(8641, 8661))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c533_seed{seed}.db", n_populations=n_levels, mode="086X_15XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 533: 0.86× + 1.5× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c533_086x_at_15x_conv_results.json", 'w') as f:
