...
**C3722 shipped and verified live (2026-09-07):** browse a horizontal twelve-instrument library, audition one look over kept native layers, then explicitly keep or replace it. Five visible numbered slots provide quick layer access while preserving all eight saved layers. Four original projected 3D instruments add dimensional silhouettes and lighting through the existing deterministic Canvas renderer. This release refines C3713's browsing gesture; its historical evidence remains below. [Release and verification](C3722_RELEASE.md).
...
| Shipped C3722 | Temporary audition → explicit Keep/Replace, a horizontal twelve-template library and numbered kept layers; four original CPU-rendered 3D families. These are internal native-art layers, not global Studio photo/video overlays. |
| Shipped C3721 | Manual `.collage` now preserves the music original and its mute/level/trim/source-fade settings with content integrity. Original moving-video clips and complete take settings remain unbundled; SVG and incremental recovery omit music. A saved project is not a complete portable film source. |
...
C3722 adds Knot Foundry, Crystal Vault, Stellar Passage and Tidal Surface: an actual tube knot, faceted spire cluster, perspective tunnel and reflective heightfield. Their geometric depth, normals and lighting are computed from the seed and requested-time sample, with bounded face/point counts and no new engine dependency. Existing family output is protected by frozen pre-change Canvas-command hashes. Browser appearance, export and production evidence are in [C3722_RELEASE.md](C3722_RELEASE.md).

The browse → audition → keep flow separates an uncommitted look from the saved stack. Dice preview changes only that candidate; kept layers retain identity and settings until an explicit decision. The numbered strip shows five slots initially and allows access to existing sixth through eighth layers. A transient candidate may be auditioned at capacity; it cannot become a ninth saved layer. Use as starting template remains a separate deliberate action. This should improve visual exploration without adding a permanent global control row.
...
## September 21 checkpoint — readable titles in delivered media

Shipped the prepared title-compositing correction (`7ba19031`), with separately measured preview/JPEG/SVG/project/Stage/video evidence and a failing old-public-build control. [Release](TITLE_CONTRAST_RELEASE.md). No new inference dependency or permanent control. Existing fonts, audio portability, native-art drafts and the large viewport are preserved.

Next substantial portability increment remains moving originals in manual `.collage` archives, with original-byte integrity, poster-to-clip bindings and authored trim/requested speed/mute/level/source fade plus length-sync mode. Stage intent must be saved rather than temporary monitor/solo/decoder state. Validate candidates before adopting them; retain stale-open disposal and dirty-edit guards. Reuse the existing soundtrack archive tests and owned media fixtures. Full take settings, recovery media, shot sequencing and local lyric inference stay separately qualified future work.
