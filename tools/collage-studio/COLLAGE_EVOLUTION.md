...
- **C3722 — BROWSE, AUDITION, KEEP (2026-09-07), candidate; final validation and publication pending:** a horizontal twelve-instrument library previews one candidate over kept art, then explicit Keep/Replace promotes it. Five visible numbered slots retain access to all eight saved layers. Knot Foundry, Crystal Vault, Stellar Passage and Tidal Surface add original deterministic projected CPU 3D geometry through Canvas. Nine new model/renderer unit groups passed, including frozen command parity with the old eight painters; this is not yet live evidence. Source commit, Pages/health, final browser/export results, public artifact hashes and shared fleet close remain pending. [Candidate release](C3722_RELEASE.md).
...
- [ ] **BROWSE, AUDITION, KEEP (C3722 candidate)** — try one look over kept art, promote it explicitly, and return through numbered layers. Twelve templates include four original dimensional instruments. Unit geometry/legacy-command evidence exists; final browser/export/deploy/public and fleet gates are pending. [Candidate receipt](C3722_RELEASE.md).
...
- **2026-09-07 · [AXIS:COLLAGE] C3722 — BROWSE, AUDITION, KEEP, candidate implementation; not yet a shipped cycle.** The user's new direction asks for previewing multiple visual instruments against the existing art, deliberate promotion of a variation, quick numbered layers and a more dimensional/cinematic library. The candidate changes C3713's primary whole-stack replacement into temporary audition. Keep/selected-layer Replace/start are explicit; saved v1 remains eight layers, with five slots visible initially and all eight accessible. Four original analytic families add a tube knot, crystal spires, a perspective star tunnel and reflective wave geometry using the shared explicit-time Canvas renderer. Geometry is bounded; no runtime dependency or third-party engine is added. **Completed evidence:** nine dimensional unit groups; exact frozen Canvas-command parity against source `e4e19604` for all eight original painters and the default composition. **Pending:** integrated audition/history/capacity checks, final typecheck/build, muted mobile/landscape visual and encoded-export gates, existing media/project regressions, source commit, successful Pages/health, public artifact hashes and real-page verification. Global photo/video overlays, complete portable audiovisual projects and physical material rendering remain outside this increment. The existing Monday 10 a.m. Pacific weekly evolution stays in place; final shared fleet delivery/readback is required and pending. [Release, primary DD and final receipt fields](C3722_RELEASE.md).
