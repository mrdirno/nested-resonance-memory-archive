...
- **C3722 — BROWSE, AUDITION, KEEP (2026-09-07), shipped and verified live:** preview a visual instrument over kept art, then Keep/Replace the exact variation. Horizontal twelve-template library; five immediately visible numbered slots, all eight saved layers retained; four original projected 3D families. Also fixes music arriving before photo decoding completes. [Code 506b6c62](https://github.com/mrdirno/nested-resonance-memory-archive/commit/506b6c625592d247152ea7a86b4f6a60f4a1260a), [Pages 34190005150](https://github.com/mrdirno/nested-resonance-memory-archive/actions/runs/34190005150) and Archive health 34190005148 succeeded. **96/96 built and 96/96 public browser cases** passed, with exact old-eight PNG parity, real native MP4s and matching public artifacts. [Release and limits](C3722_RELEASE.md).
- **C3721 — MUSIC TRAVELS WITH THE PROJECT (2026-09-07), shipped and verified live:** manual `.collage` saves now preserve original soundtrack bytes/MIME/name, SHA-256 and authored mute/level/trim/source-fade. Direct restore preserves motion and retires monitor/solo/audition; malformed media/settings fail before adoption and stale Open candidates release their URLs. Video clips still return as frames; SVG/recovery do not carry music. Code `1e89d3d9`; [Pages 34184898401](https://github.com/mrdirno/nested-resonance-memory-archive/actions/runs/34184898401) succeeded; **58/58 public browser cases**, 18 final soundtrack cases and 18 unit groups passed. Public bundles match the tested build. [Release and boundaries](C3721_RELEASE.md).
- **C3719 — SOLO, AND SOUND THAT STARTS ITSELF (2026-09-07), served from the well:** adding music now starts it playing and opens Details on the row of sources, and every source — each clip and the music — has a SOLO button that puts everything else out of the room so you can decide what to keep. Solo is monitor state only: it never writes intent, never reaches the offline mix, is refused for the whole duration of a take, and releases itself when its source is removed. Decision logic is `src/lib/solo.ts` (pure; the pre-existing cut audition folded into the same rule), swept by `tests/unit/solo.invariants.mjs` — **7,354 checks, 0 failures, three mutants killed**. Code `6702d390b21097888ff53b5f18e998efb60f44fd`; [Pages 34149075444](https://github.com/mrdirno/nested-resonance-memory-archive/actions/runs/34149075444) succeeded. The September 7 closeout audit confirmed the solo implementation in the public bundle. Retained owner output `biudjcirz.output` records **10/10 solo cases**, Chromium + Mobile Chrome, one worker, 19.7 seconds, exit 0; the owner identifies this as the production run, but the output itself does not print the target URL. No fresh browser run was performed for this reconciliation. Chip layout became two rows (name over buttons) on a panel finding, so the fourth 44px button costs no name at 320px.
...
- [x] **BROWSE, AUDITION, KEEP (C3722)** — twelve instruments, explicit audition/Keep/Replace, numbered layers and four original dimensional families. 96 built + 96 public checks, exact legacy pixels, native MP4s and deployment verified. [Release receipt](C3722_RELEASE.md).
- [x] **TEMPLATE INTENT AND ART CONTEXT (C3713)** — Use replaces one native artwork stack; explicit Add layers. Validated draft history restores selection and dice scope. UI-only descriptors explain real renderer controls without changing recipe v1. [Ring: observation, semantics, counterexample and proof](ART_CONTEXT_RING.md).
...
- [ ] **ORIGINAL AUDIO AND VIDEO MUST TRAVEL IN THE PROJECT** — highest-priority
      independence gap. C3721 manual files preserve original music and its source
      settings alongside images, pins and lyrics. Moving-video originals, complete
      take settings and music in incremental recovery remain unbundled. Extend
      versioned original/checksum contracts and deliberate relinking. Acceptance:
...
`Clear all sources` sat as the third row of a collapsed "Project actions" disclosure at the foot of the Add panel: below the fold on every phone (the pane fits exactly three 52px tiles on an SE, ~217px on an iPhone 12, and iOS draws no scrollbar), and labelled with a word the person looking for it never used — they said "reset canvas" and "new canvas". The report that arrived was "no way", not "hard to find"; from the user's chair those are the same thing. Rule: a state-clearing action gets a home in chrome that is present whenever the state is (here the panel heading, the one row that costs no vertical space) and is named in the user's noun. The judge panel was asked with four shapes on the table; all three lenses chose the heading row independently, and the one dissent (reset the style too, 1 of 3) survives in the copy — "The look, motion and shape stay" — rather than being averaged away.

### 2026-09-07 (C3718) — A GATE THAT THROWS ON THE MISSING CONTROL TELLS YOU NOTHING ABOUT THE OTHER SIXTEEN

The first live run of `tools/toolkit-gates/docspec-startover.mjs` — against the engine before its deploy, the red-before proof — died with a TimeoutError inside `scrollIntoViewIfNeeded()` on the first trade instead of reporting "Start over is not on the page" seventeen times. Fixed to fail closed per trade and go on: `17 trade(s), 34 checks, 17 failed` is a record that names what was missing where; a stack trace is not. Same lesson as docspec-config's own header ("a gate that dies on one document tells you nothing about the other hundred"), relearned by writing a new gate.

### 2026-09-07 (C3718) — SOUNDTRACK T3 WAITS FOR A BUTTON THAT MOVED

`soundtrack.spec.ts` › "T3 — the music chip and button are watertight on a phone" fails at its line 294 on HEAD before and after this cycle's change: after adding music it waits for a `Remove the music` button that has lived in the stage Details since C3712's preview-first shell. This cycle changed only its name list at line 317 (`Clear all` → `New canvas`), which the run never reaches. Recorded, not patched — the same class as C3717's project-roundtrip scar; both specs need to drive the current shell before they can grade anything.

### 2026-09-06 (C3717) — A CONTROL SAVED FOR "WHEN THERE IS ROOM" IS A CONTROL REMOVED

C3712 guarded the wish-it-better trigger on `!hasImages` to buy 48px of topbar for Expand and Export, and moved it into the Add panel as a fourth 76px tile. Measured live: on every phone that tile sat below the fold of the 42dvh inspector, and on desktop the topbar trigger was gone too. The well existed only in the one state where nobody has anything to report — before a project is loaded. A trigger that is present in the empty state and absent in the loaded one passed every gate, because every gate imported files and then measured the topbar only before the import. Rule: a feedback trigger is graded in the LOADED state, on every screen, and its label counts — the report said the WORDS were gone. Second lesson, from the judge panel rather than the measurement: the topbar is not every surface. Under 550px tall the topbar hides while editing, so a topbar-only trigger would have shipped green on the tall-screen gate and been gone again at 360×448; the trigger now rides the panel heading exactly where the topbar hides. Third: a `min-height` is a floor, not a size — 52px tiles were 59px until `line-height` was pinned, and the arithmetic that "fits the SE" only holds when the number in the CSS is the number on the screen.

### 2026-09-06 (C3717) — A SPEC THAT NAMES A TAB THAT NO LONGER EXISTS IS NOT A REGRESSION GATE

`project-roundtrip.spec.ts` › "a reopened project can still switch layout mode" clicks a `Settings` button that C3712 replaced with Add/Layout/Look/Motion/Text. It fails on HEAD before and after this cycle's change. Recorded rather than patched: the round-trip it guards (a reopened project's stencil layout resolving) still matters, and the fix is to drive the current Layout inspector, which is a cycle of its own, not a rename.

### 2026-09-05 (C3713) — CHOOSING A LOOK IS NOT AN ADD OPERATION

The live C3712 gallery made three layers become four after A, then five after B. Its tests also clicked Add and expected accumulation. C3713 separates the primary Use action from explicit Add and retains the old additive regression under the correct intent. A full eight-layer stack still accepts Use; only Add refuses a ninth. Undo snapshots include selected layer and dice scope, validated before history changes. Source IDs stay outside draft history because Apply creates immutable revisions. Dice locks protect randomization only; manual edits and explicit replacement remain available. A visible scope dropdown replaces competing rack/layer dice buttons. The [Art context Ring](ART_CONTEXT_RING.md) records the source and public counterexample, target boundaries and downloaded-project proof.
...
- **2026-09-05 · [AXIS:COLLAGE] C3713 — TEMPLATE INTENT AND CONTEXT.** Browser observation: primary template browsing accumulated 3→4→5. Use now replaces one native stack and Add alone appends; changing an existing saved project does not migrate its layers. Validated draft history restores selection and dice scope; no parent source ID is restored by local undo. All eight families expose their renderer-backed controls with concrete density help. **Gates:** 45 distinct local and 45/45 live cases across Chromium, Mobile Chrome and Mobile Safari; three relevant unit suites, typecheck/build; actual encoded 8-second loops; matching public bundle bytes. Code `0b29eb28`; [Pages](https://github.com/mrdirno/nested-resonance-memory-archive/actions/runs/33996665537). Independent review corrected validation ordering, exact selected-ID targeting, ambiguous navigation wording and dropdown discoverability. Unchanged C3712 preview-first shell retained. Wells: 0 new; cleanup: 0 candidates. Draft undo ends when Art Room closes and does not undo applied source revisions. No property/region dice, model inference or recipe-schema extension shipped. Next owner reads [Ring](ART_CONTEXT_RING.md), release notes and fleet `persona500-collage-C3713-release`; extend context only with actual supported renderer/export behavior.

- **2026-09-06 · [AXIS:WELL] C3717 — THE WELL STAYS PUT.** Wish 0f6d6145 (anonymous, via the halo well): "wish it better is gone … the add audio and aspect ratio are not visible". Before: the topbar wish trigger was guarded on `!hasImages` (C3712), so with a project loaded the only trigger was the 4th 76px tile of the Add panel, below the fold on every phone; Add music was clipped on an iPhone 12 and 8px tall on an SE; the aspect picker sat ~1250px down a 178px pane. After: the trigger is in the topbar in every state (labelled "Wish" from 360px; Open icon-only once loaded under 600px; topbar padding/gap 8 under 400px, mark tracking .04em at 320) and rides the editing-panel heading exactly where the short-screen rule hides the topbar; the Add panel tightens under 1000px to 52px tiles so Art Room / Add images / Add music sit unscrolled inside the pane on an SE; Canvas is the first row of Canvas & crop. Judge panel before build (two independent lenses, 8/9 and 6/6): the hidden label, the hidden topbar at 360×448 and the 52px-floor arithmetic all came from the panel and all changed the shape. **Gates:** typecheck/build clean; new `tests/e2e/reachable.spec.ts` 16/16 on dev and **18/18 against the LIVE url** (Mobile Chrome + Mobile Safari, seven screens 320..1280 plus the 360×448 editing case, plus the well opening from the loaded state), red on its first assertion against the previous deploy; mobile-watertight 7/7 dev and live; live JS/CSS byte-identical to the local build. Regression sweep at close: === roundtrip === [1A[2K  1 failed === soundtrack === (project-roundtrip's failure targets the pre-C3712 Settings tab — a stale spec, scarred, not this change). Code `8a90c610`; [Pages success](https://github.com/mrdirno/nested-resonance-memory-archive/actions/runs/34081662911); live https://mrdirno.github.io/nested-resonance-memory-archive/collage/ ; wish shipped with that url; credit appended to av/credits.json and on the page (Canvas section). **BACKPORT rider fired:** all 17 trade toolkits swept live (hub + first tool, 390×664 and 320×568, fresh and after the first action): 68/68 trigger measurements pass — the trades mount their well in the sticky av-bar and never hide it, so nothing to carry. Fleet receipt msg **17848** from `mac-toolkit-claude`, thread `persona500-collage-C3717-release`. Storefront untouched (Collage is not a trade tool). Next: the stale round-trip spec needs to drive the current Layout inspector; and the vibe-cards well holds GT-001 (new card request, es).

- **2026-09-07 · [AXIS:WELL] C3718 — NEW CANVAS, WHERE THE PANEL IS.** Wish 1885c683 (anonymous, bug, about_tool=upload): "How to reset canvas no way to create new canvas". Before: `Clear all sources` was the third row of a collapsed Project actions disclosure at the foot of the Add panel — below the fold of a pane that fits exactly three tiles on an SE, named with a word the wisher never used. After: a `New canvas` button in the Add panel's heading beside Done (one 48px row at every width; at 320 the title ellipsizes to 90px and the button measures 116×44), rendered only for the Add panel and only while its question is down; the first tap swaps the tiles for "Start over? Your images, music, captions and title go. The look, motion and shape stay." with Keep working focused; Start over runs handleClear plus clears the title's words, closes the panel to the start screen and says "Fresh canvas." — no undo claimed (the taskbar that reaches History unmounts with the pool; music and clip URLs are revoked); Escape while the question is up means Keep working; Clear all sources retired (one door). Judge panel before build — three lenses, independently: mobile editor 8, safety 7, product 8; all three chose the heading row; 2 of 3 keep the style and the dissent became the copy. **Gates:** typecheck/build clean; new `tests/e2e/new-canvas.spec.ts` (seven screens × two engines: heading ≤48.5px, button ≥44 inside the pane, first tap clears nothing, both answers 44px inside the pane; Keep/Escape/Start over behaviour at 390×664 and 1280×720: title words gone, title size kept, no browser dialog, status line promises no undo) with reachable.spec **34/34 dev, 36/36 against the LIVE url** (Mobile Chrome + Mobile Safari); mobile-watertight 28/28; stage-room R6/R8 migrated to `Save editable project` and green; soundtrack T3 fails before the migrated line (scarred). Live `index-6224ed4d.js` + `index-5c54a7ce.css` byte-identical to the local build. Code `413f110f`; [Pages 34104522168](https://github.com/mrdirno/nested-resonance-memory-archive/actions/runs/34104522168) success; live https://mrdirno.github.io/nested-resonance-memory-archive/collage/ ; wish shipped with that url; credit in av/credits.json and on the page (foot of the Add panel). **BACKPORT rider fired:** all 17 trades swept — every tool page carries a Clear / Reset / Start over except `write-up.html` on every trade (the shared DOCS engine had none) → `shared/docspec.js` gets Start over at the foot of Make it yours (two taps in the trades' own "Tap again to wipe it" pattern; keeps the AI preference and the anonymous use counts; wipes screen and storage, a reload agrees), new gate `tools/toolkit-gates/docspec-startover.mjs` **17 trades / 272 checks / 0 failing LIVE** (17/17 red against the engine before its deploy); docspec-config 15/15 (av), docspec-desk, docspec-needs 2240/2240, docspec-say 3696/3696 and mobile-watertight (av, electrical write-up) green; live `shared/docspec.js` + `docspec.css` byte-identical. Code `e4b8f4e4`; [Pages 34105281250](https://github.com/mrdirno/nested-resonance-memory-archive/actions/runs/34105281250) success. Fleet receipt msg **17868** from `mac-toolkit-claude`, thread `persona500-collage-C3718-release`, read back (id, thread, kind, metadata). Storefront untouched (Collage is not a trade tool; the engine change adds no tool). Wells at close: toolkit 1 new (48ec201e, audio → details/solo — next cycle's #1 under the one-wish rule); cards 1 building (GT-001: another lane's uncommitted archive work sits in the vibe-cards tree, modified 22:21 PDT — left with its owner). Named and not done: the write-up house-rules sub-text (`.ticks .sb`) wraps to up to 8 lines at 390, measured live and filed in the ideas ledger; the two stale specs (project-roundtrip Settings, soundtrack T3) need to drive the current shell.

- **2026-09-07 · [AXIS:WELL] C3719 — SOLO, AND SOUND THAT STARTS ITSELF.** Wish 48ec201e asked for music to arrive in Details and for individual sources to be heard alone. Before: new music arrived silent and the source rows had no solo control. After: music starts and opens Details; clip and music SOLO isolate the monitor without changing mute/export intent, reject solo during a take and release it when its source is removed; source names sit above 44px actions. Code `6702d390b21097888ff53b5f18e998efb60f44fd`; [Pages 34149075444](https://github.com/mrdirno/nested-resonance-memory-archive/actions/runs/34149075444) succeeded at 17:49 UTC on September 7. **Closeout reconstructed from existing evidence, without a new browser/build run:** the retained owner output `biudjcirz.output` (SHA-256 `b6dcd2fd934a3a57b9380f713420086399abbb53135fd43e9b618cc8d45d8c6a`) shows five `solo.spec.ts` cases across Chromium and Mobile Chrome, **10 passed / one worker / 19.7 seconds / exit 0**. The owner identifies the target as production; the output does not independently record its URL. Earlier documentation records the pure decision gate at 7,354 checks, zero failures and three mutants killed; that gate was not rerun in this audit. Independent deployment inspection found C3719's solo/source/sound-arrival implementation in live `index-4f74babb.js` (SHA-256 `303adef0c4b5263fda74863e0f348353cfc797dcdc4f133ee7a2869a48ff5ed2`); later successful Pages `34173501413` contains identical Collage source. This is deployed-byte evidence, not a new functional or encoded-audio assertion. The earlier parallel solo run reported 4 failed / 6 passed; serial success does not establish its cause, and C3720's one-worker remote harness change preserves that distinction. **Handoff:** the C3719 fleet release thread had no receipt at closeout audit; its reconciled close is included in C3721's shared publication thread. C3721 also reran all ten Solo cases successfully on the public site. The recorded soundtrack T3 and project-roundtrip navigation debts remain open.


- **2026-09-07 · [AXIS:COLLAGE] C3721 — MUSIC TRAVELS WITH THE PROJECT, shipped and verified live.** Scoped well: zero new and zero building; selected the existing roadmap's portable-originals gap. The real public writer failed the new music archive assertion after an edited soundtrack was imported. Manual project saves now carry its original bytes/MIME/name, integrity and authored source mix; direct restoration preserves motion and resets preview-only sound/solo/audition. Strict metadata and bounded ZIP checks reject missing/corrupt media before editor adoption. Newer Open/source changes invalidate old reads, and ongoing takes/intake own their sources until complete. Independent review found and repaired load ownership, duration-zero recovery, owned Still and unpaired-trim discrepancies. Local: **58/58 built-app browser regressions** on muted Chromium/Mobile Chrome; **18/18 soundtrack cases** on the final rebuilt candidate; **18 new unit groups**, existing project-lock/Art Rack persistence and 2,070 SVG checks; typecheck/build passed. Actual reopened exports preserve the selected middle tone at quarter gain with repeating source fades. Existing build warnings: stale Browserslist data and a >500 kB chunk. No new dependency, tool, trade or permanent control; no cross-trade backport applies to this media-archive format. Original videos/master take fields and music in SVG/recovery remain unimplemented. [Release and evidence](C3721_RELEASE.md).

  **C3721 production close:** [code 1e89d3d9](https://github.com/mrdirno/nested-resonance-memory-archive/commit/1e89d3d92543e9749d96beae9abae52ec873c59e); [Pages 34184898401](https://github.com/mrdirno/nested-resonance-memory-archive/actions/runs/34184898401) and Archive health `34184898399` succeeded. The public page passed **58/58** serial Chromium/Mobile Chrome cases; exact JS/CSS/worker/SW bytes match the final candidate. Reopened MP4s fully decode, keep the selected tone at measured gain 0.2503843093 and the repeating fades. Fleet thread `persona500-collage-C3721-release`; final delivery/readback ID lives in the source task receipt. Existing weekly evolution remains Monday 10 a.m. Pacific.

- **2026-09-07 · [AXIS:COLLAGE] C3722 — BROWSE, AUDITION, KEEP, shipped and verified live.** The user requested in-context audition, deliberate promotion, numbered layer access and a more dimensional library. Four original CPU3D instruments join the unchanged eight families; native recipes remain v1 with eight kept layers. Browse/Dice preview are transient; Keep/Replace/start are explicit and undoable. Independent review repaired pending Apply navigation, replacement naming, focus and short-screen layout. Release gates also proved an existing early music-arrival loss; the event now survives pending photo decoding and is retired with its track. Ordinary Solo fixture readiness/media targeting were repaired without removing the dedicated failing-order regression. Final **96/96 built and 96/96 live** Chromium/Mobile Chrome cases passed. Typecheck/build, geometry/audition/recipe/HTML/SVG invariants, old-eight exact PNG parity, distinct new-family pixels and four full native-MP4 decodes passed. [Code 506b6c62](https://github.com/mrdirno/nested-resonance-memory-archive/commit/506b6c625592d247152ea7a86b4f6a60f4a1260a); [Pages 34190005150](https://github.com/mrdirno/nested-resonance-memory-archive/actions/runs/34190005150) and Archive health 34190005148 succeeded; all five runtime artifacts match. Existing warnings and failed intermediate runs remain documented. Scope is internal Art Room layers, not global video tracks or complete CapCut equivalence. Weekly Monday 10 a.m. Pacific evolution remains active; fleet thread `persona500-collage-C3722-release` carries broadcast **17937**, with exact body and metadata read back; peer acknowledgements were empty at close. [Release](C3722_RELEASE.md).

- **2026-09-09 · [AXIS:WELL] C3724 — UNDO WHERE THE THUMB IS, AND NO MORE ASKING WHICH PART.** Wish 55e079f9 (anonymous, bug, about_tool=layout): *"Missing undo — Missing info button in art room also prolly get rid of the which part drop down less clicks better."* Before: Undo and Redo lived inside the collapsed `Canvas & recipe` disclosure at the foot of the desk pane, below the fold on every phone, while the Keep notice promised "Undo restores the previous stack"; the twelve instruments' written `description`s existed in `ART_TEMPLATES` and were rendered nowhere; the footer armed a dice scope in a `<select>` (Art composition / <layer name>) that Dice then read back — two taps before every roll and a sticky mode that sends a roll meant for one layer at the whole composition; at 320px that select was crushed to ~88px and truncated the very name it existed to show. **Judge panel before build — three lenses, independently, no repository access: daily creator 9, mobile interaction 9, product skeptic 8.** All three read "the which part drop down" as the Dice scope select (the only control that literally asks which part, and the one tapped before every roll); all three refused the header as first specced at 320px (title 95 + four controls + gaps ≈ 335px into 296px usable) and demanded icon-only 44×44 controls with the title shrunk first; all three moved the instrument description out from behind the help card to the moment of decision; B insisted the visible dice labels stay fixed-width with the target only in the accessible name, and that Undo never sit beside Close. **Dissent recorded:** B and C would have cut or hidden Redo on a keyboardless device; A wanted it always rendered so the row never reflows under a thumb — built A's way, both buttons always present and honestly dark. After: Undo · Redo · Help ride the top bar in that order, then Close (icon + word above 760px, icon-only 44×44 in portrait phones, title 18px at ≤360, Close an × like the sheet's own dismiss); Undo names the step it took back — "Undid: Keep Knot Foundry as layer 4." / "Undid: Opacity on Knot Foundry." — from labels carried on the UI-only history stack (recipe v1 untouched), and hands focus to its partner when it goes dark instead of dropping it on `<body>`; Help (accessible name `Help and report`, the MIDI Room instruments' house pattern) opens a nested native `<dialog>`: four lines in the room's own vocabulary, shortcuts behind a disclosure hidden on coarse pointers, a report button that leaves the room first because the feedback widget is a fixed overlay that would render behind the modal, and the on-page credit; Escape and the backdrop close only the card (the room's `onCancel` bails while it is open), and a second Escape still closes the room; the instrument's own sentence sits inside the preview sheet while auditioning and under the selected layer's heading while editing, with "is held … unlock it in Layer options" / "is off … Turn it on" when Dice layer is dark, and the stage readout gains "· held" / "· off"; `Dice art` and `Dice layer` are two fixed-width buttons (solid and dashed), one tap each, `Dice layer: Petal Engine` as the accessible name, the single `Dice preview` unchanged during an audition; the disclosure's Undo/Redo and the scope select are deleted, not hidden. **The gate that earned its keep:** the panel's literal placement — the sentence in the stage label row — went red on the C3722 canvas-height assertion (`art-audition.spec.ts:213`, 116px against a 140px floor at 664 tall): the stage is a fixed `clamp()` on phones, so a line of text there is paid for by the artwork. Moved into the desk; the assertion that "the sentence never taxes the artwork" now compares a fresh room against a previewing one. Measured while writing that assertion and left alone: the footer status line moves the desktop stage by ~21px whenever a notice appears — pre-existing, every notice, named here for a later cycle. Pure half: `src/lib/artGuide.ts` (`artDiceTargets`, `artInstrumentNote`, `artUndoNotice`, the four-line `ART_ROOM_GUIDE`) with `tests/unit/artGuide.invariants.mjs` — **4,660 checks** across 24 seeds × 9 stacks × every pick, the real module transpiled by esbuild. **Gates:** typecheck and build clean; new `tests/e2e/art-room-help.spec.ts` (top bar at 320/360/390/430 default and bumped text, undo naming its step and focus handoff, one-tap dice targets, the sentence at both decision points, the help card's Escape/backdrop/report/credit) **15/15 dev** on chromium + Mobile Chrome + Mobile Safari; art-intent-ring, art-room-ux, art-rack and art-audition migrated to the new controls and green; `mobile-watertight.spec.ts` gains an Art Room case (top bar, footer, help card) **8/8 dev and 8/8 LIVE**; **24/24 against the LIVE url** for art-room-help + art-room-ux on the three engines, one worker; live `index-2265ca48.js` + `index-37aabdf9.css` byte-identical to the local build. The Chrome extension was not connected this cycle, so the hands-on live drive was Playwright on three real engines, not a human-driven session. Code [`a40e4bce`](https://github.com/mrdirno/nested-resonance-memory-archive/commit/a40e4bce938936c839e4a079a8a6ce8ff8e24706); [Pages 34335186310](https://github.com/mrdirno/nested-resonance-memory-archive/actions/runs/34335186310) success; live https://mrdirno.github.io/nested-resonance-memory-archive/collage/ ; wish shipped with that url; credit appended to `av/credits.json` and on the help card. **BACKPORT rider fired:** 184 pages across all 17 trades swept for the same class — an undo/reset/clear inside a closed `<details>` (0), a `<select>` gating the primary send/copy in the action bar (0), a tool with no sentence of its own (0 — every registered tool renders its `desc` line, which is exactly what the Art Room lacked); nothing to carry. Fleet receipt msg **18137** from `mac-toolkit-claude`, thread `persona500-collage-C3724-release`, read back (id, body, metadata). Ideas round `6eb2788e` closed at **1 tap against a baseline of 3**, reviewed and archived; `operator_autonomy` recorded in the cycle feed. Storefront untouched (Collage is not a trade tool). Wells at close: toolkit empty; vibe-cards holds 7ff1381e (MIDI Room tempo sync — a persona500 page whose owner shipped the sibling "sadder" wish this morning, left with them). Named and not done: the 21px notice jump on desktop; the two stale specs (project-roundtrip Settings, soundtrack T3) still need to drive the current shell.

- **2026-09-14 · [AXIS:COLLAGE] STEADY FEEDBACK — shipped and verified live.** Existing scoped well: zero new/building. Resumed C3724 fleet18137 measured issue under atomic task T-18986. Live baseline: desktop canvas 498→476.609px and short-landscape 198→179.609px on Dice. Persistent feedback occupies the existing wide footer row and a stable two-line portrait row. Full text and status/alert roles retained; occupied feedback supports keyboard scrolling. Portrait expanded view pays the reserved height once. Reused current source, Help/audition/export tests, and C3721 serialization assessment; no new video claim. [Release](STEADY_ART_ROOM_RELEASE.md).

  **Steady-feedback production close:** [source 2f97494c](https://github.com/mrdirno/nested-resonance-memory-archive/commit/2f97494cc46525f7b85798299017c79cb2b7a69f); [Pages 34873757476](https://github.com/mrdirno/nested-resonance-memory-archive/actions/runs/34873757476) and [Archive health 34873757505](https://github.com/mrdirno/nested-resonance-memory-archive/actions/runs/34873757505) succeeded. **46/46 final built and 46/46 public browser cases passed** (34 existing regressions on desktop/mobile Chromium, plus 12 feedback cases on desktop/mobile Chromium and WebKit), with all five public runtime files matching. Four newly exported live native MP4s fully decode without errors. Live before/after geometry matches the measured build: desktop fitted artwork stays 498px high through Dice instead of shrinking to 476.609px; short-landscape stays 198px instead of 179.609px. Status, Undo, real import alerts, clearing and enlarged-text keyboard scrolling preserve geometry within 1px. The standalone fixed-viewport measurements record zero notice-driven movement after the fix. No new runtime dependency or shared-trade behavior; cross-trade backport does not apply to this React Art Room layout. Failed baseline and the corrected default-font overflow assumption remain in the receipt.
...
- **2026-09-17 · [AXIS:COLLAGE] C3733 — THE TITLE GETS A COLOUR, AND THE SCRIM CHOOSES ITS OWN SIDE, shipped and verified live.** Both wells dry at open (AV toolkit 0 new / 0 building; vibe-cards 0 new, one `building` on `open-archive` left with the cards lane); no trade owed; four straight toolkit cycles (C3729–C3732) tripped the operator's golf-cart rotation, and COLLAGE was tied-stalest since 09-14 — so the rung-3 axis was COLLAGE. **Before:** `THE TITLE` could only be white on a fixed dark plate — `TITLE_INK`/`TITLE_PLATE` were module constants the two emitters read directly, and `TitleSpec` had no colour. A person wanting a dark title over a bright photo, or a title in a chosen colour, had no control at all. **After:** a `TitleColor` of six (`white`/`black`/`yellow`/`red`/`blue`/`pink`) offered as swatches under Text → Title; the swatch dot is `titleInk(id)`, the exact hex the render paints (one source of truth, no drift). The scrim polarity is **derived, not a second control**: `titlePalette` keeps whichever of near-black `rgba(0,0,0,0.42)` / near-white `rgba(255,255,255,0.60)` separates further from the ink by WCAG contrast — so `black` flips to the light plate and stays legible, and every luminous ink keeps the legacy dark plate. `white` resolves to the exact legacy `{#ffffff, rgba(0,0,0,0.42)}`, so an untitled or white render is byte-for-byte what it was. **The one-plan seam held:** the colour is resolved ONCE in `planTitle` and carried as `ink`/`scrim` on the `TitlePlan`; the two shared emitters `drawTitlePlan` (still preview `renderer.ts`, live Stage `stage.ts`, video worker `render.worker.ts`) and `titlePlanToSvg` (`vectorExport.ts`) paint those strings, so no surface can diverge — the defect the title module exists to prevent. `scaleTitlePlan` carries the pair at every k; `color` rides `.collage`, session and the SVG's whole-state metadata, restored at the single load path with `?? 'white'`; captions plan through the same `planTitle` with no colour and stay white. **Pure half:** `titlePalette`/`relLuminance`/`contrastRatio` swept by `tests/unit/title-colour.invariants.mjs` — **101 checks**: identity (`white` == legacy), argmax polarity per colour, the large-text contrast floor, determinism, and that the plan's `ink`/`scrim` are the EXACT strings both emitters emit (a recording fake ctx + SVG substring). The pre-existing `title.invariants.mjs` **containment** sweep (plate in safe area, lines in plate) still **82,871 checks / 0 failures** against the changed module — geometry untouched. **Gates:** `tsc --noEmit` clean; production `vite build` clean; new `tests/e2e/title-colour.spec.ts` drives the real UI (drop image → Text → Title → type → tap swatch → Export → Vector SVG) and asserts the downloaded SVG's Title-group fills — **4/4 dev and 4/4 against the LIVE url** on Chromium, Mobile Chrome, Mobile Safari and WebKit (white → `#ffffff` on `rgba(0,0,0,0.42)`; black → `#111111` on `rgba(255,255,255,0.60)`, never the dark plate; yellow → `#ffd400` on the dark plate); collage `mobile-watertight.spec.ts` **16/16** (the six-swatch row wraps in `ui-titler__chips`, each button ≥44px). **Independent adversarial audit (fork, read-only): CLEAN** — verified all four surfaces route through the two emitters, zero `TITLE_INK`/`TITLE_PLATE` paint refs remain outside `title.ts`, no hand-built `TitlePlan` literal exists so no plan can lack the fields, and both project formats funnel through one restore path. Code [`a8f4cbe8`](https://github.com/mrdirno/nested-resonance-memory-archive/commit/a8f4cbe8); [Pages 35275044209](https://github.com/mrdirno/nested-resonance-memory-archive/actions/runs/35275044209) success; live https://mrdirno.github.io/nested-resonance-memory-archive/collage/ . The Chrome extension was not driven this cycle; the hands-on live drive was Playwright on four real engines exporting real SVGs, not a human session. **BACKPORT rider fired — nothing to carry:** title colour is a Collage-only render feature (a caption plate over an image); the 18 trade toolkits are list/document builders with no caption-over-image surface, so the class does not exist elsewhere (as C3721's media-archive format also had no cross-trade sibling). **Storefront untouched** — Collage is not a trade tool, so no `fieldToolkits.ts` entry. **Wells at close:** AV toolkit empty; vibe-cards still holds the one `open-archive` `building` claim (cards lane's). **Named and not done:** the pre-existing desktop notice jump (~21px) and the two stale specs (project-roundtrip Settings, soundtrack T3) still need to drive the current shell; a title colour could later ride an arbitrary hex picker rather than six presets, and could carry into the composition code if a recipe ever wants it (today it deliberately does not, like the rest of the title).

- **2026-09-19 · [AXIS:COLLAGE] C3742 — THE TITLE FONT: FOUR FACES ON THE ONE PLAN, shipped and verified live.** Both wells dry at open (AV toolkit 0 new / 0 building; vibe-cards 0 new / 0 building) and no trade owed, so the stalest axis was COLLAGE (6 lane-cycles cold) — but the tree already carried a title-font rung, uncommitted, from the **C3741 cycle that appended its model-ledger row (08:19:44Z) and DIED before a single commit** (three src files + three test files last touched 08:22-08:25Z, ~3.7h cold, no live sibling on the shared tree; the :5199 dev server it left running was reused for verification). Per the dead-cycle rule this was rung #1 to SHIP after re-verification, not to rebuild. **Before:** `THE TITLE` (and C3733's colour) could only be drawn in the one legacy `TITLE_FAMILY` — `titleFont(px)` hard-coded it and every measurer/emitter asked for that one string; a person wanting an editorial serif, a typewriter mono or a poster shout for their caption had no control. **After:** a `TitleFont` roster of four — `sans` / `serif` / `mono` / `poster` — offered as chips under Text → Title, each chip DRAWN IN THE FACE IT SELECTS (`fontFamily: titleFontFamily(f.id)`) so the choice reads with no legend. The four stacks are ordinary SYSTEM fonts (`Georgia…`, `"Courier New"…`, `Impact…`), never an `@font-face` — nothing is fetched, so the caption paints the instant it is typed, offline and on the video worker thread alike, and every stack ends in a generic family so a device missing the named face still lands legible. **The one-plan seam held, exactly as C3733's colour did:** the face is resolved ONCE in `planTitle` to `family` on the `TitlePlan`, and the four surfaces paint it — `drawTitlePlan` (still preview `renderer.ts`, live Stage `stage.ts`, video worker `render.worker.ts`) sets `ctx.font = titleFont(plan.fontPx, plan.family)`, `titlePlanToSvg` (`vectorExport.ts`) names `plan.family` — so no surface can disagree about the face. **The correctness invariant that earned the rung (F4):** the wrap and every line width are MEASURED in the family that will be PAINTED — a wide face wrapped against a narrow face's metrics overruns the plate on the delivered surface, the exact silent-wrong-picture defect the "measure once" design exists to prevent — so `planTitle` binds the resolved family into the measurer (`measFam`) before a single `wrapText`/`ellipsize`/width call. `sans` IS the exact legacy `TITLE_FAMILY`, so a no-font plan is byte-for-byte the `font:'sans'` plan and every untouched title is unchanged (F1 identity); `scaleTitlePlan` carries `family` at every k; `font` rides `.collage`, session and the SVG's whole-state metadata and restores at the single load path with `?? 'sans'`; captions plan through the same `planTitle` with no `font` field and stay `sans`. **Pure half:** `tests/unit/title-font.invariants.mjs` transpiles the REAL `title.ts` via esbuild (not a re-implementation) and sweeps **26,958 checks / 0 failures** — F0 roster, F1 identity (no-font ≡ sans, JSON-equal across every aspect×place×size×text), F2 family resolution + junk→sans, F3 plan carries family, F4 EVERY measurement taken in the painted face (a family-sensitive synthetic measurer that scales advances per stack, so a dropped family fails), F5 scale preserves family, F6 SVG names it, F7 one-arg `titleFont` byte-identical; plus a RED PROOF that a font-ignoring mutant fails F3/F4 on 3 of 4 faces, so the checks are not vacuous. **Gates:** `tsc --noEmit` clean; production `vite build` clean; `tests/e2e/title-font.spec.ts` drives the real UI (drop image → Text → Title → type → tap font chip → Export → Vector SVG) and asserts the downloaded SVG's Title-GROUP `font-family` switches sans→serif→mono→poster with the sans stack GONE after the switch — **2/2 dev and 2/2 against the LIVE url** on Chromium + Mobile Chrome; broad `mobile-watertight.spec.ts` **16/16**; and — because that spec does NOT open the titler (a real coverage gap, named below) — a direct measurement of the new chip row on the LIVE page: **0px page overflow and all four chips ≥44px (49.4–54.5 × 44.0) at 320 and 360px**. Live entry `index-73d3ddb5.js` is byte-identical to the local build and carries the font stacks + chip testids. Ship [`1ec703f0`](https://github.com/mrdirno/nested-resonance-memory-archive/commit/1ec703f0); [Pages 35442247374](https://github.com/mrdirno/nested-resonance-memory-archive/actions/runs/35442247374) success; live https://mrdirno.github.io/nested-resonance-memory-archive/collage/ . The Chrome extension was not driven this cycle; the hands-on live drive was Playwright on real engines exporting real SVGs. **BACKPORT rider fired — nothing to carry:** title font is a Collage-only render feature (a caption over an image); the 19 trade toolkits are list/document builders with no caption-over-image surface, so the class does not exist elsewhere (the same reason C3733's title colour and C3721's media-archive format had no cross-trade sibling). **Storefront untouched** — Collage is not a trade tool, so no `fieldToolkits.ts` entry. **Wells at close:** AV toolkit 0 new / 0 building; vibe-cards 0 new / 0 building. **Named and not done:** `mobile-watertight.spec.ts` should OPEN the titler and assert the chip row in-suite rather than leaning on a side script; the pre-existing desktop notice jump (~21px) and the two stale specs (project-roundtrip Settings, soundtrack T3) still need to drive the current shell; a title font could later carry into the composition code if a recipe ever wants it (today it deliberately does not, like the colour and the rest of the title).


### [AXIS:COLLAGE] Weekly evolution — 2026-09-21: title contrast after compositing
