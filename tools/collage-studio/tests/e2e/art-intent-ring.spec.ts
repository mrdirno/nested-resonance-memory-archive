// Rings C3713/C3722: browse is transient; Keep adds and starting-template intent replaces.
...
async function previewSettings(room:Locator,open=true){const d=room.locator('details.art-preview-options');if(await d.count()&&(await d.getAttribute('open')!==null)!==open)await d.locator(':scope > summary').click();}
async function settings(room:Locator){await previewSettings(room,false);const d=room.locator('details.art-project-settings');if(await d.getAttribute('open')===null)await d.locator(':scope > summary').click();}
...
test('Ring C3713 explicit starting templates A then B replace the stack in one undoable transaction',async({page},info)=>{
...
test('Ring C3713 explicit Keep preserves siblings and replacement undo restores selected locks and solo',async({page})=>{
...
test('Ring C3713 browsing replacement does not rewrite existing art or imported originals before Apply',async({page})=>{
 ...
 await room.getByRole('button',{name:'Use in Studio',exact:true}).click();await expect(room.locator('.art-footer [role=status]')).toContainText('Editable artwork applied');await room.getByRole('button',{name:'Close Art Room',exact:true}).click();
 ...
 room=await openRoom(page);await room.getByRole('button',{name:'Update in Studio',exact:true}).click();await expect(room.locator('.art-footer [role=status]')).toContainText('Editable artwork applied');await room.getByRole('button',{name:'Close Art Room',exact:true}).click();
...
test('Ring C3713 Preview and Keep remain separate reachable targets at narrow widths',async({page},info)=>{
...
test('Ring C3713 a full eight-layer stack previews a ninth and permits explicit replacement',async({page})=>{
 const room=await boot(page);
 for(const name of ['Orbit Press','Ribbon Choir','Branch Fans','Prism Garden','Woven Circuit'])await add(room,name);
 const full=await recipe(page,room);expect(full.layers).toHaveLength(8);
 await preview(room,'Contour Atlas');await expect(room.getByTestId('art-audition')).toBeVisible();await expect(room.getByRole('button',{name:'Keep layer',exact:true})).toBeDisabled();await expect(room.locator('.art-studio-pending')).toBeDisabled();expect(await recipe(page,room)).toEqual(full);
 await previewSettings(room);await room.getByLabel('Preview placement',{exact:true}).selectOption('replace');await room.getByRole('button',{name:'Replace layer 8',exact:true}).click();const replaced=await recipe(page,room);expect(replaced.layers).toHaveLength(8);expect(replaced.layers.slice(0,-1)).toEqual(full.layers.slice(0,-1));expect(replaced.layers.at(-1).kind).toBe('contour');await undo(room);expect(await recipe(page,room)).toEqual(full);
