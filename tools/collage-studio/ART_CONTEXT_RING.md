...
## September 14 — feedback must not move the monitor

C3724 explicitly left a notice-driven desktop jump. Fresh public Dice reproduced a 21.391px loss of fitted height at 1280×720 and 18.391px at 844×390. The conditional footer paragraph was the mechanism: the flex-none footer grew and the flex-one workspace shrank. A persistent fixed-height feedback slot now shares the wide action row; portrait has a stable two-line row. Error and success occupy the same geometry, with full text and keyboard scrolling. No timer hides the notice. Expanded portrait pays the reserve once; it is not an enlargement claim. Independent review checked the CSS sizing and required narrow-landscape and text-scaling evidence. Final negative-control, built/public gates and deployment are tracked in [the release](STEADY_ART_ROOM_RELEASE.md).
