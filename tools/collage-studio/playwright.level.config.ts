...
/**
 * Standalone config for THE LEVEL proof (how loud each source sits in the mix).
 *
 ...
 *
 * SERIAL, one worker: L1 drives TWO full offline renders and divides one by the
 * other, and parallel renders on one machine contend for the decoder the
 * measurement is about.
 */
export default defineConfig({
  testDir: './tests/e2e',
  testMatch: /level\.spec\.ts$/,
  timeout: 900_000,
  workers: 1,
  use: {
