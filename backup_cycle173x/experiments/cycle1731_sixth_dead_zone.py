#!/usr/bin/env python3
"""
CYCLE 1731: SIXTH DEAD ZONE TEST
Formula: N = 29 + 14.5k
Predicted: k=5 → N≈101.5
"""
...
def main():
    print(f"CYCLE 1731: Sixth Dead Zone | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 70)
    print("Goal: Test prediction N = 29 + 14.5*5 ≈ 101.5")
    print("=" * 70)

    seeds = list(range(1731001, 1731051))
    n_values = list(range(96, 111))
    ...
    if in_zone:
        ...
        print(f"\nSixth dead zone: N={min(in_zone)}-{max(in_zone)}, min={min_n} ({min_c:.0f}%)")
