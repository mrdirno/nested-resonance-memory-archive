...
**Birth Rate Saturation Mechanism:**
Further analysis reveals **why** spawn rate has diminishing effect: energy cap creates universal saturation point in birth rate (~0.0005 per agent per cycle) regardless of configured spawn probability. Observed birth rate efficiency drops dramatically from 69% (f_spawn=0.001) to 5% (f_spawn=0.01) as population approaches energy cap. This occurs because E_avg declines as population increases (E_avg = E_cap / N), reducing the fraction of agents with sufficient energy (> spawn_cost = 5.0) to reproduce. Higher spawn rates reach the energy cap faster, experiencing more severe saturation: at f_spawn=0.001, population is at 86% of maximum with moderate constraint; at f_spawn=0.01, population is at 99.9% of maximum with extreme constraint. The 10-fold increase in spawn rate yields only a 16% increase in observed population (17,161 → 19,980) because the constraint is energetic, not reproductive. This mechanistic insight explains the conditional parameter activation: spawn rate modulates composition dynamics only when energetically feasible, and its influence weakens as the energy cap is approached.

**Buffer Factor Mechanism:**
The energetic floor identified in birth rate saturation analysis exhibits precise mathematical structure: average energy per agent approaches a universal minimum E_min ≈ 473 units across all spawn rates (coefficient of variation = 0.059). This minimum demonstrates exact spawn cost scaling: E_min = 94.69 × spawn_cost (R² = 0.9999), revealing a fundamental **buffer factor k ≈ 95**. This constant represents the minimum viable energy reserve required for sustained population under capacity constraints. Population pressure drives E_avg down to this floor (strong correlation with final population: r = -0.984, p < 1e-37), at which point most agents hover near the spawn threshold, creating the observed birth rate saturation. The exponential model E_avg = 473.46 + 1188.48 × exp(-2803.08 × f_spawn) captures the transient approach with exceptional precision (MAPE = 0.05%), confirming rapid convergence to a spawn-rate-independent minimum. At true equilibrium (t→∞), carrying capacity converges to K_equilibrium ≈ E_cap / E_min ≈ 21,127 agents for all spawn rates, resolving the apparent paradox of spawn rate independence: spawn rate affects **approach dynamics** (how fast equilibrium is reached) but not **final capacity** (universal energy floor).
