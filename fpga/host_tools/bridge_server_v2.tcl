...
if {[llength $master_paths] == 0} {
    ...
    puts "Devices found: [get_service_paths device]"
    # exit
    return
}
...
proc handle_data {sock} {
    ...
    if {$cmd eq "PING"} {
        ...
        if {[llength $parts] == 3} {
            ...
            # Handle hex strings
            if {[scan $val %x v] == 1} {
                 # Tcl 32-bit write expects list of bytes or int? 
                 # master_write_32 expects list of 32-bit values
                 if {[catch {master_write_32 $master_path $addr [list $val]} err]} {
                     puts $sock "ERR: $err"
                 } else {
                     puts $sock "OK"
                 }
