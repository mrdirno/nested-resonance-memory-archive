...
const N = 262144;
const RUNS = [
  { name: 'SC exact mag0.3',              scn: 'spinchladni', lorentz: 'boris', step: 9028, cosmos: { mag: 0.3 } },
  { name: 'SC exact helix1.0',            scn: 'spinchladni', lorentz: 'boris', step: 9028, cosmos: { helix: 1.0 } },
  { name: 'SC Euler helix0.5',            scn: 'spinchladni', lorentz: 'euler', step: 9028, cosmos: { helix: 0.5 } },
  { name: 'SC Euler substeps4',           scn: 'spinchladni', lorentz: 'euler', step: 9028, cosmos: {}, substeps: 4 },
  { name: 'SC exact mag0.3 sg0.45',       scn: 'spinchladni', lorentz: 'boris', step: 9028, cosmos: { mag: 0.3, selfgrav: 0.45 } },
];
(async () => {
  ...
  for (const run of RUNS) {
    ...
    fs.writeFileSync(path.join(__dirname, 'boris3_test.json'), JSON.stringify(results, null, 1));
  }
  console.log('done');
  await browser.close();
