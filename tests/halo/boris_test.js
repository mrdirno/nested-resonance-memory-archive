...
const N = 262144;
const RUNS = [
  { name: 'Razor Disc / Euler',              scn: 'razordisc',   lorentz: 'euler', step: 0 },
  { name: 'Razor Disc / exact rotation',     scn: 'razordisc',   lorentz: 'boris', step: 0 },
  { name: 'Spinning Chladni / Euler',        scn: 'spinchladni', lorentz: 'euler', step: 9028 },
  { name: 'Spinning Chladni / exact rot.',   scn: 'spinchladni', lorentz: 'boris', step: 9028 },
];
(async () => {
  ...
  for (const run of RUNS) {
    ...
    results.push(row);
    console.log(row.name.padEnd(34), 'mean|y|', row.meanY.toFixed(4), 'p90|y|', row.p90Y.toFixed(3), 'r', row.r.toFixed(2),
    ...
    fs.writeFileSync(path.join(__dirname, 'boris_test.json'), JSON.stringify(results, null, 1));
  }
  console.log('done');
  await browser.close();
