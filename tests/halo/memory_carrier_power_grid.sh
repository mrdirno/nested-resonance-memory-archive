#!/bin/sh
# memory_carrier_power_grid.sh - the Ring 32 runs, in the order fixed by the pre-registration
# (analysis/2026-09-24_within_run_carrier_statistic.md):
...
#   2. for each registered (self-gravity, seed) pair below, in this order: identity, then
#      invert_all (the positive control);
#   3. the closing repeat: identity sg0.3 seed 14142 again, which must reproduce step 2's mesh.
...
# Every launch is first written to data/results/halo/memory_carrier_launches.tsv (UTC time,
...
# left no record and no void (killed from outside; its .partial is kept aside as attempt 1); a
# third counted launch is refused. A launch that ended in a 'refused' void (the page or path was
# not the registered one, before the first tick) is not a run and is not counted.
# Console output of each run goes to $LOGDIR (default: a directory in $HOME, outside the repo).
# Usage (from anywhere): sh tests/halo/memory_carrier_power_grid.sh
...
D=data/results/halo
LEDGER=$D/memory_carrier_launches.tsv
LOGDIR="${LOGDIR:-$HOME/.halo_runs/ring32_logs}"
mkdir -p "$LOGDIR"
PAIRS="0.3:14142 0.3:17320 0.3:22360 0.32:24494 0.32:26457 0.32:28284 0.35:31622 0.35:33166 0.35:34641"
...
if n == 1:
    ...
    if os.path.exists(part):
        os.replace(part, os.path.join(d, tag + '.attempt1.mesh.f32.partial'))
...
run identity 0.32 777 "$D/memory_carrier_canary"
same_mesh "$D/memory_carrier_canary/spinchladni_sg0.32_gl0_seed777_n4194304_e24_fieldExp1.7_ividentity.mesh.f32" \
          ce15f08155e34fff22c234a802ef1784549c8bf3e6af1e282d72beb946e34177 "canary reproduces Ring 29"

for p in $PAIRS; do
  run identity   "${p%%:*}" "${p##*:}" "$D/memory_carrier_identity"
  run invert_all "${p%%:*}" "${p##*:}" "$D/memory_carrier_invert_all"
done

run identity 0.3 14142 "$D/memory_carrier_identity_close"
T=spinchladni_sg0.3_gl0_seed14142_n4194304_e24_fieldExp1.7_ividentity.mesh.f32
same_mesh "$D/memory_carrier_identity_close/$T" "$D/memory_carrier_identity/$T" "closing repeat reproduces the first fresh run"
