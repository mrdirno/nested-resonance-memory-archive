...
public class MeshAnalyzer {
    ...
    /// Simplifies the mesh using Voxel Remeshing.
    /// - Parameters:
    ///   - url: Input URL.
    ///   - outputUrl: Output URL.
    ///   - resolution: Voxel grid divisions (e.g., 32, 64, 128). Lower = simpler.
    public func simplify(url: URL, outputUrl: URL, resolution: Int) throws {
        ...
        // This creates a "water-tight" mesh, excellent for printing too.
        if let newMesh = voxelArray.mesh(using: allocator) {
            ...
            // Fallback if voxelization fails (e.g. open surface)
