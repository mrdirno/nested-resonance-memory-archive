    ...
    var body: some View {
        HSplitView {
            ...
            VStack(alignment: .leading, spacing: 20) {
                ...
                // Input
                Group {
                    Text("Input").font(.subheadline).foregroundStyle(.secondary)
                    Button("Select Image Folder") { selectFolder() }
                        .controlSize(.large)
                        .buttonStyle(.bordered)
                    if let folder = selectedFolder {
                        Text(folder.lastPathComponent).font(.caption).foregroundStyle(.secondary).truncationMode(.middle)
                    }
                }
                ...
                // Generation
                Group {
                    Text("Generation").font(.subheadline).foregroundStyle(.secondary)
                ...
                // Optimization
                Group {
                    Text("Optimization").font(.subheadline).foregroundStyle(.secondary)
                    VStack(alignment: .leading) {
                        Text("Voxel Res: \(Int(voxelResolution))").font(.caption)
                        Slider(value: $voxelResolution, in: 32...256, step: 32)
                    }
                    Button("Remesh (Voxelize)") { remeshModel() }
                ...
                // Stats
                Group {
                    Text("Analysis").font(.subheadline).foregroundStyle(.secondary)
                    Text(meshStats).font(.caption).textSelection(.enabled)
                }
                
                Spacer()
                Text(engineStatus).font(.caption2).foregroundStyle(.gray).padding(.bottom)
            }
            .padding()
            ...
            ZStack {
                ...
                if let modelURL = generatedModelURL {
                    ModelViewer(modelURL: modelURL).edgesIgnoringSafeArea(.all).id(modelURL)
                } else {
                    ContentUnavailableView("No Model Loaded", systemImage: "view.3d")
                }
            }
