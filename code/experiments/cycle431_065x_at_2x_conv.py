#!/usr/bin/env python3
"""
CYCLE 431: 0.65× ATTACK AT 2.0× CONVERSION
...
Purpose:
  Fill 0.65× column at 2.0× conversion.

Design:
  - K declines from 600 to 200 over 40 cycles
  - Attack: 0.65×, Conversion: 2.0×
  - 20 seeds
"""
...
# 0.65× attack + 2.0× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.00195, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.6},
    {"attack": 0.00325, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.5},
    {"attack": 0.0052, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.4},
    {"attack": 0.0078, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.3},
    {"attack": 0.00975, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.24},
    {"attack": 0.0117, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.2}
]

SEEDS = list(range(6600, 6620))


def run_experiment(seed: int, cycles: int) -> dict:
    n_levels = 7
    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c431_seed{seed}.db"
    ...
    return {"seed": seed, "finals": {f"L{i}": float(finals[i]) for i in range(n_levels)}, "coexist": bool(coexist), "runtime": runtime, "n_recordings": len(histories[0])}


def main():
    print("=" * 80)
    print(f"CYCLE 431: 0.65× ATTACK AT 2.0× CONVERSION")
    ...
    output = {"cycle_id": CYCLE_ID, "cycle_name": CYCLE_NAME, "date": datetime.now().isoformat(), "author": "Aldrin Payopay <aldrin.gdf@gmail.com>", "attack_multiplier": 0.65, "conversion_multiplier": 2.0, "n_seeds": len(SEEDS), "coexistence_rate": coexist_count / len(results) if results else 0, "results": results}
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c431_065x_at_2x_conv_results.json", 'w') as f:
