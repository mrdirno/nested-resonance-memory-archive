#!/usr/bin/env python3
"""
CYCLE 547: 0.89× ATTACK AT 1.3× CONVERSION
...
Purpose:
  Test 0.89× in suppression window at 1.3× conversion.
"""
...
SEEDS = list(range(8921, 8941))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c547_seed{seed}.db", n_populations=n_levels, mode="089X_13XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 547: 0.89× + 1.3× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c547_089x_at_13x_conv_results.json", 'w') as f:
