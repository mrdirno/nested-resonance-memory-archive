#!/usr/bin/env python3
"""
CYCLE 430: 0.65× ATTACK AT 2.5× CONVERSION
...
Purpose:
  Extend phase diagram to lower attack values.

Background:
  Phase diagram shows saddle point at 0.71875× + 2.0×
  Testing lower attack to find stable region boundary

  C430: 0.65× + 2.5× - extend grid left

Design:
  - K declines from 600 to 200 over 40 cycles
  - Attack: 0.65×, Conversion: 2.5×
  - 20 seeds
"""
...
# 0.65× attack + 2.5× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.00195, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.75},
    {"attack": 0.00325, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.625},
    {"attack": 0.0052, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.5},
    {"attack": 0.0078, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.375},
    {"attack": 0.00975, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.3},
    {"attack": 0.0117, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.25}
]

SEEDS = list(range(6580, 6600))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run 0.65× attack at 2.5× conversion experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c430_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 430: 0.65× ATTACK AT 2.5× CONVERSION")
    ...
    print(f"Attack: 0.65×, Conversion: 2.5×")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c430_065x_at_25x_conv_results.json"
