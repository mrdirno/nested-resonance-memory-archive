#!/usr/bin/env python3
"""
C186 V6c - Net-Negative Collapse Regime (Three-Regime Campaign Part 3)

Purpose: Test hierarchical spawning dynamics under net-negative energy conditions
         predicting population collapse (net energy = -0.5).

Duration: TBD (450,000 cycles or early collapse, likely very fast)
Conditions: 5 spawn rates (0.10%-1.00%) × 10 seeds = 50 experiments
Energy: E_consume = 1.5, E_recharge = 1.0 (net -0.5, collapse regime)

This is Phase 3 of the three-regime V6 campaign:
- V6a: Net-zero energy (homeostasis, K ~ 201)
- V6b: Net-positive energy (growth, K ~ 19,320)
- V6c: Net-negative energy (collapse, K → 0, predicted)

Background:
- V6a (net=0): 100% success, homeostasis at 201 ± 1.2 agents
- V6b (net=+0.5): 100% success, growth to 19,320 ± 1,102 agents
- Energy primacy hypothesis: Energy balance determines population fate
- V6c tests lower boundary: net-negative should cause 100% collapse

Hypothesis:
- Net-negative energy (-0.5) causes inevitable population collapse (K → 0)
- All 50 experiments should reach zero population before 450,000 cycles
- Spawn rate may affect collapse *speed* but not *outcome* (all collapse)
- Completes three-regime phase space: homeostasis, growth, collapse

Predictions:
1. 100% population collapse (50/50 experiments reach zero agents)
2. Time-to-collapse may vary by spawn rate (higher rate = faster collapse due to spawn cost)
...
Decision Tree:
- If 100% collapse → Energy balance theory validated (lower boundary confirmed)
- If any experiment sustains → Energy balance theory falsified (revise)
- If spawn rate affects collapse speed → Conditional dynamics extend to collapse regime
...
class SimpleAgent:
    """Minimal agent for V6c experiments."""
...
# V6c Parameters (NET-NEGATIVE COLLAPSE)
E_CONSUME = 1.5  # Energy consumed per cycle (INCREASED from V6a's 1.0)
...
def run_experiment(f_spawn, spawn_label, seed):
    ...
    print(f"  Condition: HIERARCHICAL_COLLAPSE_{spawn_label}")
    print(f"  Seed: {seed}")
    ...
    condition_name = f"HIERARCHICAL_COLLAPSE_{spawn_label.replace('.', '_').replace('%', 'pct')}"
    db_path = RESULTS_DIR / f"c186_v6c_{condition_name}_seed{seed}.db"
    heartbeat_path = RESULTS_DIR / f"c186_v6c_{condition_name}_seed{seed}_heartbeat.log"
    json_path = RESULTS_DIR / f"c186_v6c_{condition_name}_seed{seed}.json"
    ...
    for cycle in range(CYCLES):
        ...
        if cycle > 0 and cycle % JSON_BACKUP_INTERVAL == 0:
            summary = {
                'experiment': 'C186_V6c_NET_NEGATIVE_COLLAPSE',
    ...
    if db_size == 0:
        ...
        if final_pop == 0:
            print("✓ PASS: Population collapsed as predicted")
            print("✓ PASS: Net-negative collapse regime validated")
            print()
            print("  → Note: Complete collapse confirms energy balance theory (net < 0 → extinction)")
        elif final_pop < INITIAL_AGENTS / 2:
            print("⚠ PARTIAL: Population declining but not yet collapsed")
            print()
            print("  → Note: Net-negative energy causing population decline")
        else:
            print("✗ UNEXPECTED: Population sustained despite net-negative energy")
            print()
            print("  → Note: Energy balance theory falsified (population should collapse at net < 0)")
    ...
    summary = {
        ...
        'verdict': {
            ...
            'population_collapsed': final_pop == 0,
            'collapse_as_predicted': final_pop == 0 and db_size > 1024
        },
        'timestamp': time.time()
    }
...
def run_v6c_campaign():
    """Execute all 50 V6c experiments (5 conditions × 10 seeds)."""
    print()
    print("="*80)
    print("C186 V6c CAMPAIGN - NET-NEGATIVE COLLAPSE REGIME")
    print("="*80)
    print()
    print(f"Total experiments: {len(F_SPAWN_VALUES)} conditions × {len(SEEDS)} seeds = {len(F_SPAWN_VALUES) * len(SEEDS)}")
    print(f"Estimated duration: TBD (likely very fast due to rapid collapse)")
    print(f"Energy regime: E_consume={E_CONSUME}, E_recharge={E_RECHARGE} (net=-0.5, collapse)")
    print()
    ...
    campaign_summary = {
        'campaign': 'C186_V6c_NET_NEGATIVE_COLLAPSE',
...
if __name__ == '__main__':
    try:
        ...
        campaign_summary = run_v6c_campaign()
