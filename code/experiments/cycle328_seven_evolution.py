#!/usr/bin/env python3
"""
CYCLE 328: SEVEN-TROPHIC EVOLUTIONARY DYNAMICS
...
Purpose:
  Test evolutionary trait dynamics in seven-level food chain.

Research Question:
  Do traits evolve adaptively across trophic levels?

Background:
  C292-294: Evolutionary dynamics at two-trophic
  C325: Seven-trophic 95% stable with N=2
  C328: Add trait variation to stable system

Design:
  - 7 populations with trait variation
  - Attack rate heritable with mutation
  - K=600, L6=N=2
  - 9 seeds
  - Total: 9 experiments
"""
...
CYCLE_ID = "C328"
CYCLE_NAME = "Seven-Trophic Evolution"
MODE = "SEVEN_EVO"

CYCLES = 30000
K = 600
INITIAL = [300, 30, 10, 5, 3, 2, 2]  # L6 at N=2 for stability
...
MUTATION_RATE = 0.1
MUTATION_STRENGTH = 0.2  # 20% variation

SEEDS = list(range(3500, 3509))  # 9 seeds


def run_experiment(seed: int, cycles: int) -> dict:
    """Run seven-trophic evolutionary experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c328_seed{seed}.db"
    reality = RealityInterface(db_path=db_path, n_populations=n_levels, mode=MODE)
    np.random.seed(seed)

    # Initialize with trait variation
    traits = {}  # agent_id -> attack_multiplier
    for lvl in range(n_levels):
        for i in range(INITIAL[lvl]):
            agent_id = f"L{lvl}_{i}"
            e = 1.0 + lvl * 0.5
            reality.add_agent(FractalAgent(agent_id, lvl, e), lvl)
            if lvl > 0:  # Predators have attack trait
                traits[agent_id] = 1.0 + np.random.uniform(-0.1, 0.1)

    histories = {i: [] for i in range(n_levels)}
    trait_histories = {i: [] for i in range(1, n_levels)}  # Track mean traits
    ...
    for cycle in range(cycles):
        ...
        # Predation cascade with trait-modified attack
...
def main():
    print("=" * 80)
    print(f"CYCLE 328: SEVEN-TROPHIC EVOLUTIONARY DYNAMICS")
    ...
    print("\nMean final traits (evolution direction):")
    for lvl in range(1, 7):
        mean_trait = np.mean([r["final_traits"][f"L{lvl}"] for r in results if r["coexist"]])
        direction = "↑" if mean_trait > 1.05 else "↓" if mean_trait < 0.95 else "→"
        print(f"  L{lvl}: {mean_trait:.3f} {direction}")
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c328_seven_evolution_results.json"
