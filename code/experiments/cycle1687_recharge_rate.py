...
Does optimal N depend on energy recharge rate?
Standard rate is 0.1/cycle. Test higher/lower rates.
"""
...
def run_experiment(seed, n_initial=100, recharge_rate=0.1):
    """Run with variable recharge rate."""
...
def main():
    print(f"CYCLE 1687: Recharge Rate | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 70)
    print("Goal: Does optimal N depend on recharge rate?")
    print("=" * 70)

    seeds = list(range(1687001, 1687051))  # 50 seeds

    recharge_rates = [0.05, 0.1, 0.15, 0.2]
    ...
    print("\n" + "=" * 70)
    print("OPTIMAL N BY RECHARGE RATE")
    print("=" * 70)
    ...
    for rate in recharge_rates:
        ...
        print(f"\nRecharge Rate = {rate}:")
        for n_init in population_sizes:
            subset = [r for r in all_results if r["recharge_rate"] == rate and r["n_initial"] == n_init]
        ...
        for n_init in population_sizes:
            subset = [r for r in all_results if r["recharge_rate"] == rate and r["n_initial"] == n_init]
            success = sum(1 for r in subset if r["coexist"]) / len(subset)
            marker = " ← OPTIMAL" if n_init == best_n else ""
            print(f"  n={n_init:2d}: {success*100:.0f}%{marker}")

    # Summary table
    print("\n" + "=" * 70)
    print("SUMMARY: OPTIMAL N BY RECHARGE RATE")
    print("=" * 70)
    print(f"\n{'Rate':>8} | {'Optimal N':>10} | {'Success':>10}")
    print("-" * 35)
    ...
    for rate in recharge_rates:
        ...
        for n_init in population_sizes:
            ...
            if success > best_rate:
                best_rate = success
                best_n = n_init
        print(f"{rate:8.2f} | {best_n:10d} | {best_rate*100:9.0f}%")
