#!/usr/bin/env python3
"""
CYCLE 402: BASELINE ATTACK + 2.25× CONVERSION - 40 CYCLES
...
Purpose:
  Test if non-monotonic pattern extends to 1.0× attack.

Background:
  At 0.75× attack: Non-monotonic curve with peak at 2.25×

  At 1.0× attack:
  - C371: 1.5× conv - 80%
  - C372: 0.5× conv - 25%

  Need to test higher conversion at baseline attack.

Design:
  - K declines from 600 to 200 over 40 cycles
  - Attack: 1.0×, Conversion: 2.25×
  - 20 seeds
"""
...
CYCLE_ID = "C402"
CYCLE_NAME = "Baseline Attack 2.25x Conv"
MODE = "BASE_ATK_225XCONV"
...
# 1.0× attack + 2.25× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.003, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.675},
    {"attack": 0.005, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.5625},
    {"attack": 0.008, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.45},
    {"attack": 0.012, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.3375},
    {"attack": 0.015, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.27},
    {"attack": 0.018, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.225}
]

SEEDS = list(range(6020, 6040))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run baseline attack + 2.25× conversion experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c402_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 402: BASELINE ATTACK + 2.25× CONVERSION - 40 CYCLES")
    ...
    print(f"Attack: 1.0×, Conversion: 2.25×")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c402_baseline_attack_225x_conv_results.json"
