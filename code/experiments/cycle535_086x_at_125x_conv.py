#!/usr/bin/env python3
"""
CYCLE 535: 0.86× ATTACK AT 1.25× CONVERSION
...
Purpose:
  Map 0.86× rise pattern from 30% to 90%.
"""
...
SEEDS = list(range(8681, 8701))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c535_seed{seed}.db", n_populations=n_levels, mode="086X_125XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 535: 0.86× + 1.25× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c535_086x_at_125x_conv_results.json", 'w') as f:
