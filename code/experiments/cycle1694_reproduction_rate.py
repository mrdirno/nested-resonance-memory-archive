...
Does optimal N depend on reproduction rate (currently BASE_REPRO=0.1)?
"""
...
def run_experiment(seed, n_initial=25, base_repro=0.1):
    """Run with variable reproduction rate."""
...
def main():
    print(f"CYCLE 1694: Reproduction Rate | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 70)
    print("Goal: Does optimal N depend on reproduction rate?")
    print("=" * 70)

    seeds = list(range(1694001, 1694051))  # 50 seeds

    repro_rates = [0.0, 0.05, 0.1, 0.2, 0.3]
    population_sizes = [20, 25, 30, 35, 50]

    all_results = []

    for repro in repro_rates:
        print(f"\nReproduction Rate = {repro}")
        print("-" * 40)

        for n_init in population_sizes:
            results = [run_experiment(seed, n_init, repro) for seed in seeds]
    ...
    # Summary
    print("\n" + "=" * 70)
    print("SUMMARY: OPTIMAL N BY REPRODUCTION RATE")
    print("=" * 70)
    print(f"\n{'Repro':>8} | {'Optimal N':>10} | {'Success':>10}")
    print("-" * 35)
    ...
    for repro in repro_rates:
        ...
        best_rate = 0
        for n_init in population_sizes:
            subset = [r for r in all_results if r["base_repro"] == repro and r["n_initial"] == n_init]
            success = sum(1 for r in subset if r["coexist"]) / len(subset)
            if success > best_rate:
                best_rate = success
                best_n = n_init
        print(f"{repro:8.2f} | {best_n:10d} | {best_rate*100:9.0f}%")
