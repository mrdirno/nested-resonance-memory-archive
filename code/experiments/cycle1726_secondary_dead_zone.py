...
def main():
    print(f"CYCLE 1726: Secondary Dead Zone | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 70)
    print("Goal: Verify dead zone at N=41-44")
    print("=" * 70)

    seeds = list(range(1726001, 1726051))  # 50 seeds

    # Fine-grained N values around potential secondary dead zone
    n_values = list(range(38, 53))  # 38-52
    ...
    if in_zone:
        print(f"\nSecondary dead zone (coexist < 80%): N = {min(in_zone)}-{max(in_zone)}")
        min_n, min_c = min(results, key=lambda x: x[1])
        print(f"Minimum: N={min_n} ({min_c:.0f}%)")
    else:
        print("\nNo secondary dead zone found")
