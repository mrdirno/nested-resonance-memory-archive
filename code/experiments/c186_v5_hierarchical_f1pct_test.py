#!/usr/bin/env python3
"""
Cycle 186 V5: Hierarchical Deep Failure Test at f=1.0%

Purpose: Validate complete collapse well below predicted critical frequency
...
Background:
    ...
    - C186 V4 (f=1.5%): [To be determined - midpoint test]
    - Discovery: α < 1.25, f_hier_crit < 2.5%

Hypothesis: f_intra = 1.0% will produce 0% Basin A (deep failure)
            At f=1.0%, well below ANY predicted threshold
            Should fail regardless of α value (unless α < 0.5?!)

Critical Test: Validate lower bound for f_hier_crit
    - Expected: Basin A = 0% (complete collapse)
    - If Basin A > 0%: Extremely low α (< 0.5), hierarchical advantage!
...
Expected Outcome:
    Basin A = 0% (deep failure, complete collapse)
    Mean population → 0 across all seeds

Validation:
    Compare to:
    - C186 V3 (f=2.0%): Threshold or above
    - C186 V4 (f=1.5%): Expected failure

    Confirms: f_hier_crit > 1.0% (lower bound established)
...
F_INTRA_PCT = 1.0  # 1.0% intra-population spawn rate (deep failure test)
...
SPAWN_INTERVAL = max(1, int(100.0 / F_INTRA_PCT))  # 100 / 1.0 = 100 cycles
...
def main():
    """Execute C186 V5 deep failure test at f=1.0%"""

    print("=" * 80)
    print("CYCLE 186 V5: HIERARCHICAL DEEP FAILURE TEST AT f=1.0%")
    print("=" * 80)
    print()
    print(f"Parameters:")
    ...
    print()
    print(f"Expected: Basin A = 0% (deep failure, well below any threshold)")
    print()
    print("-" * 80)
    ...
    if basin_a_pct == 0:
        print("✓ DEEP FAILURE CONFIRMED: 0% Basin A (complete collapse)")
        print("  Validates f_hier_crit > 1.0% (lower bound established)")
    else:
        print("⚠ UNEXPECTED: Some experiments viable")
        print(f"  Expected 0%, observed {basin_a_pct:.1f}%")
        print("  Implies f_hier_crit < 1.0% (extremely low α!)")

    output = {
        'experiment': 'C186_V5_HIERARCHICAL_F1PCT_TEST',
        ...
        'metadata': {
            ...
            'hypothesis': 'Testing deep failure well below critical frequency'
        },
    ...
    results_file = results_dir / "c186_v5_hierarchical_f1pct_test.json"
