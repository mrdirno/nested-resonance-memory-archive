#!/usr/bin/env python3
"""
CYCLE 539: 0.95× ATTACK AT 1.1× CONVERSION
...
Purpose:
  Characterize 0.95× first peak at 1.1-1.15× region.
"""
...
SEEDS = list(range(8761, 8781))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c539_seed{seed}.db", n_populations=n_levels, mode="095X_11XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 539: 0.95× + 1.1× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c539_095x_at_11x_conv_results.json", 'w') as f:
