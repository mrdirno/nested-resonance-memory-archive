#!/usr/bin/env python3
"""
CYCLE 532: 0.92× ATTACK AT 1.5× CONVERSION
...
Purpose:
  Test if 0.92× invariance extends to 1.5×.
"""
...
SEEDS = list(range(8621, 8641))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c532_seed{seed}.db", n_populations=n_levels, mode="092X_15XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 532: 0.92× + 1.5× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c532_092x_at_15x_conv_results.json", 'w') as f:
