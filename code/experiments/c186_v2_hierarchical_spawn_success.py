#!/usr/bin/env python3
"""
Cycle 186 V2: Hierarchical Viability Threshold - Critical Frequency Test

Purpose: Validate that hierarchical critical frequency is ~2× single-scale
         critical frequency due to energy compartmentalization overhead
...
Mechanism:
    ...
    mixed outcomes characteristic of critical transition.
...
Validation:
    ...
    - C186 V7: Precision sigmoid fit (extract exact α ± error)
...
def run_single_experiment(seed: int) -> Dict:
    ...
    db_path = db_dir / f"c186_v2_seed{seed}.db"
...
def main():
    """Execute C186 V2 threshold viability test"""

    print("=" * 80)
    print("CYCLE 186 V2: HIERARCHICAL VIABILITY THRESHOLD - CRITICAL FREQUENCY TEST")
    print("=" * 80)
    print()
    print(f"Parameters:")
    ...
    print()
    print(f"Expected: Basin A = 50-60% (threshold viability, α ≈ 2.5)")
    print()
    print("-" * 80)
    ...
    if 40 <= basin_a_pct <= 70:
        print("✓ VALIDATED: f_intra=5.0% produces threshold viability")
        print(f"  Basin A = {basin_a_pct:.1f}% (within 40-70% expected range)")
        print(f"  Confirms α ≈ {F_INTRA/0.020:.2f} (hierarchical scaling coefficient)")
        print("  Energy compartmentalization overhead: ~2-2.5× single-scale")
    elif basin_a_pct < 40:
        print("⚠ BELOW THRESHOLD: Lower Basin A than expected")
        print(f"  Expected 40-70%, observed {basin_a_pct:.1f}%")
        print("  May indicate α > 2.5 (higher overhead than predicted)")
    else:
        print("⚠ ABOVE THRESHOLD: Higher Basin A than expected")
        print(f"  Expected 40-70%, observed {basin_a_pct:.1f}%")
        print("  May indicate α < 2.5 (lower overhead than predicted)")

    # Save results
    output = {
        'experiment': 'C186_V2_HIERARCHICAL_SPAWN_SUCCESS',
    ...
    results_file = results_dir / "c186_v2_hierarchical_spawn_success.json"
