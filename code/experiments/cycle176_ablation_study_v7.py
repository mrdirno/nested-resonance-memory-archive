#!/usr/bin/env python3
"""
CYCLE 176 V7: ABLATION STUDY - Energy-Regulated Population Dynamics (CORRECTED)
...
V6 Discovery (Cycle 891):
  - V4/V5 added agent removal (C171 didn't have this!)
  - Caused population collapse: agents compose → removed → extinction
  - V6 correctly identified energy-regulated mechanism
  - V6 condition classes incompletely implemented (still had V4/V5 conditions)

V7 Corrections (Cycle 897):
  - Properly implement redesigned ablation conditions from V6 header
  - BASELINE: C171 exact replication (energy-regulated, no removal)
  - NO_ENERGY_CONSTRAINT: Test energy regulation necessity
  - FORCED_DEATH: Test if removal causes collapse
  - Additional controls: SMALL_WINDOW, DETERMINISTIC, ALT_BASIS

Ablation Conditions:
  1. BASELINE: C171 exact replication (energy-regulated spawning, no removal)
     - Expected: ~18-20 agents, CV < 15% (homeostatic)

  2. NO_ENERGY_CONSTRAINT: Bypass energy check (always spawn regardless of parent energy)
     - Expected: Population explosion → 100 agent cap (no natural regulation)

  3. FORCED_DEATH: Add agent removal on composition (test V4/V5 mechanism)
     - Expected: Population collapse (energy depletion + removal = extinction)

  4. SMALL_WINDOW: window=25 instead of 100 (test measurement sensitivity)
     - Expected: ~18-20 agents (homeostasis not window-dependent)

  5. DETERMINISTIC: Control condition (already deterministic)
     - Expected: ~18-20 agents (identical to BASELINE)

  6. ALT_BASIS: e, φ only - remove π oscillator (test substrate importance)
     - Expected: Unknown (tests transcendental substrate hypothesis)

Expected Key Findings:
  - BASELINE = DETERMINISTIC (both should show homeostasis)
  - NO_ENERGY_CONSTRAINT ≠ BASELINE (population explosion proves energy regulation critical)
  - FORCED_DEATH ≠ BASELINE (population collapse proves removal harmful)
...
Cycle: 176 V7 (Corrected V6 condition implementation)
...
class AblationCondition:
    ...
    def bypass_energy_constraint(self) -> bool:
        """Whether to bypass energy constraint (always spawn). Default: False."""
        return False

    def should_remove_agents(self) -> bool:
        """Whether to remove agents on composition. Default: False (C171 behavior)."""
        return False
...
class BaselineCondition(AblationCondition):
    ...
    def __init__(self):
        super().__init__(
            name="BASELINE",
            description="Energy-regulated spawning, NO agent removal (C171 exact)"
        )


class NoEnergyConstraintCondition(AblationCondition):
    """Condition 2: NO ENERGY CONSTRAINT (Bypass energy check - always spawn)."""

    def __init__(self):
        super().__init__(
            name="NO_ENERGY_CONSTRAINT",
            description="Bypass energy check - always spawn regardless of parent energy"
        )

    def bypass_energy_constraint(self) -> bool:
        """Bypass energy check - create agents directly without parent.spawn_child()."""
        return True


class ForcedDeathCondition(AblationCondition):
    """Condition 3: FORCED DEATH (Explicitly remove agents on composition)."""

    def __init__(self):
        super().__init__(
            name="FORCED_DEATH",
            description="Add agent removal on composition (test V4/V5 mechanism)"
        )

    def should_remove_agents(self) -> bool:
        """Signal that this condition removes agents on composition."""
        return True

    # Note: Agent removal logic is handled in run_ablation_experiment() main loop
    # This condition tests whether V4/V5's agent removal causes population collapse
...
def run_ablation_experiment(condition: AblationCondition, frequency: float,
    ...
    for cycle_idx in range(cycles):
        ...
        # V7: Spawn logic with condition-dependent energy bypass
        if should_spawn and len(agents) < MAX_AGENTS:
            ...
            if condition.bypass_energy_constraint():
                # NO_ENERGY_CONSTRAINT: Create agent directly (bypass parent.spawn_child)
                # This tests whether energy regulation is necessary for homeostasis
                ...
                # BASELINE/other conditions: Use parent.spawn_child (C171 exact)
        ...
        # V7: Condition-dependent agent removal
        if cluster_events and condition.should_remove_agents():
            # FORCED_DEATH: Remove agents on composition (test V4/V5 mechanism)
            # This tests whether V4/V5's agent removal causes population collapse
        ...
        # BASELINE/other conditions: C171 never removed agents on composition
...
def main():
    """Execute Cycle 176 V7 ablation study."""
    print("=" * 80)
    print("CYCLE 176 V7: ABLATION STUDY - Energy-Regulated Population Dynamics")
    print("=" * 80)
    print()
    print("Purpose: Test energy-regulated population mechanism through systematic ablation")
    print("Strategy: Isolate and test individual components of C171 mechanism")
    print()
    print("V7 Corrections (from V6):")
    print("  - Properly implement redesigned ablation conditions")
    print("  - BASELINE: C171 exact replication (energy-regulated, no removal)")
    print("  - NO_ENERGY_CONSTRAINT: Test energy regulation necessity")
    print("  - FORCED_DEATH: Test if V4/V5 removal causes collapse")
    print("  - Additional controls: SMALL_WINDOW, DETERMINISTIC, ALT_BASIS")
    print()

    # Define ablation conditions (V7 corrected)
    conditions = [
        BaselineCondition(),
        NoEnergyConstraintCondition(),
        ForcedDeathCondition(),
    ...
    output_data = {
        'metadata': {
            'cycle': '176',
            'version': 'V7',
            'scenario': 'Ablation Study - Energy-Regulated Population Dynamics (Corrected)',
            ...
            'v6_discovery_cycle': 891,
            'v7_corrections': 'Properly implemented redesigned ablation conditions from V6 header',
            'mechanism_tested': 'Energy-regulated spawning (parent.spawn_child with energy constraint)',
            'conditions': [
    ...
    output_path = Path(__file__).parent / "results" / "cycle176_ablation_study_v7.json"
