#!/usr/bin/env python3
"""
Cycle 186 V3: Hierarchical Critical Frequency Test at f=2.0%

Purpose: Test if hierarchical critical frequency equals single-scale frequency (α ≈ 1.0)

Background:
    - C171 single-scale: f_crit ≈ 2.0%, 100% Basin A at f=2.0-3.0%
    - C186 V1 (f=2.5%): 100% Basin A (unexpected viability)
    - C186 V2 (f=5.0%): 100% Basin A (well above threshold)
    - Discovery: α < 1.25, f_hier_crit < 2.5%

Hypothesis: f_intra = 2.0% will produce 50-60% Basin A (threshold viability)
            If α ≈ 1.0: Hierarchical threshold = single-scale threshold
            If α > 1.0: Hierarchical threshold < single-scale threshold

Critical Test: Does hierarchical structure add ANY overhead?
    - If Basin A ≈ 50%: α ≈ 1.0 (minimal/no overhead)
    - If Basin A = 100%: α < 1.0 (hierarchical ADVANTAGE?!)
    - If Basin A = 0%: α > 1.0 (some overhead exists)
...
Energy Dynamics:
    - Agents spawn within populations at f_intra = 2.0%
    - Populations exchange agents via migration at f_migrate = 0.5%
    - Each population has isolated energy pool (compartmentalization)

Design:
    f_intra = 2.0% (matching single-scale critical frequency)
    f_migrate = 0.5% (same as V1/V2)
...
Expected Outcome:
    ...
    Mean population distribution split between collapse and homeostasis

Validation:
    Compare to:
    - C171 (f=2.0%): Need to verify single-scale baseline
    - C186 V1 (f=2.5%): 100% Basin A (above threshold)
    - C186 V4 (f=1.5%): Will test if below threshold

    Determines: Is α ≈ 1.0 (no overhead) or α > 1.0 (minimal overhead)?
...
F_INTRA_PCT = 2.0  # 2.0% intra-population spawn rate (threshold test)
...
def main():
    """Execute C186 V3 threshold test at f=2.0%"""

    print("=" * 80)
    print("CYCLE 186 V3: HIERARCHICAL CRITICAL FREQUENCY TEST AT f=2.0%")
    print("=" * 80)
    print()
    print(f"Parameters:")
    ...
    print()
    print(f"Expected: Basin A = 50-60% (threshold viability, testing α ≈ 1.0)")
    print()
    print("-" * 80)
    ...
    if 40 <= basin_a_pct <= 70:
        print("✓ THRESHOLD BEHAVIOR: ~50% Basin A (bimodal distribution)")
        print("  Confirms f_hier_crit ≈ 2.0%, α ≈ 1.0 (minimal/no overhead)")
    elif basin_a_pct > 70:
        print("⚠ ABOVE THRESHOLD: High Basin A percentage")
        print(f"  Expected 50%, observed {basin_a_pct:.1f}%")
        print("  Implies f_hier_crit < 2.0%, α < 1.0 (hierarchical advantage?!)")
    else:
        print("⚠ BELOW THRESHOLD: Low Basin A percentage")
        print(f"  Expected 50%, observed {basin_a_pct:.1f}%")
        print("  Implies f_hier_crit > 2.0%, 1.0 < α < 1.25")

    # Save results
    output = {
        'experiment': 'C186_V3_HIERARCHICAL_F2PCT_TEST',
        ...
        'metadata': {
            ...
            'hypothesis': 'Testing if hierarchical critical frequency equals single-scale (α ≈ 1.0)'
        },
    ...
    results_file = results_dir / "c186_v3_hierarchical_f2pct_test.json"
