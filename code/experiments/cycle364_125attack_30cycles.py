#!/usr/bin/env python3
"""
CYCLE 364: 1.25× ATTACK RATE - 30 CYCLES
...
Purpose:
  Test parameter interaction - optimal attack at different K decline rate.

Background:
  C356: 30 cycles, 1.0× attack - 45% survival
  C363: 40 cycles, 1.25× attack - 60% survival
  C364: 30 cycles, 1.25× attack - test interaction

Hypothesis:
  Optimal attack may shift with K decline rate

Design:
  - K declines from 600 to 200 over 30 cycles
  - Rate: 13.3 K/cycle
  - Attack rates: 1.25× baseline
  - 20 seeds
"""
...
CYCLE_ID = "C364"
CYCLE_NAME = "1.25x Attack Rate 30 Cycles"
MODE = "125_ATTACK_30"
...
BASE_PARAMS = [
    ...
    {"attack": 0.00375, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.3},
    {"attack": 0.00625, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.25},
    {"attack": 0.01, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.2},
    {"attack": 0.015, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.15},
    {"attack": 0.01875, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.12},
    {"attack": 0.0225, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.10}
]

SEEDS = list(range(5260, 5280))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run 1.25× attack at 30 cycles."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c364_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 364: 1.25× ATTACK RATE - 30 CYCLES")
    ...
    print(f"Attack rates: 1.25× baseline")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c364_125attack_30cycles_results.json"
