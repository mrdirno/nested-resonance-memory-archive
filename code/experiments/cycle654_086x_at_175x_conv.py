#!/usr/bin/env python3
"""
CYCLE 654: 0.86× ATTACK AT 1.75× CONVERSION
...
Purpose:
  Test if 0.86× takes lead at 1.75×.
"""
...
SEEDS = list(range(11061, 11081))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c654_seed{seed}.db", n_populations=n_levels, mode="086X_175XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 654: 0.86× + 1.75× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c654_086x_at_175x_conv_results.json", 'w') as f:
