#!/usr/bin/env python3
"""
CYCLE 493: 0.95× ATTACK AT 1.25× CONVERSION
...
Purpose:
  Map transition between 1.0× (notch) and 1.5× (no notch).
"""
...
SEEDS = list(range(7840, 7860))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c493_seed{seed}.db", n_populations=n_levels, mode="095X_125XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 493: 0.95× + 1.25× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c493_095x_at_125x_conv_results.json", 'w') as f:
