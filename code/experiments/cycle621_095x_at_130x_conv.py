#!/usr/bin/env python3
"""
CYCLE 621: 0.95× ATTACK AT 1.3× CONVERSION
...
SEEDS = list(range(10401, 10421))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c621_seed{seed}.db", n_populations=n_levels, mode="095X_130XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 621: 0.95× + 1.3× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c621_095x_at_130x_conv_results.json", 'w') as f:
