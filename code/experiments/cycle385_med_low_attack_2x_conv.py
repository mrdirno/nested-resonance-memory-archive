#!/usr/bin/env python3
"""
CYCLE 385: MEDIUM-LOW ATTACK + 2.0× CONVERSION - 40 CYCLES
...
Purpose:
  Find exact threshold for 2.0× conversion.

Background:
  C381: 0.25× attack + 2.0× conv - 15%
  C380: 0.5× attack + 2.0× conv - 100%
  C385: 0.375× attack + 2.0× conv - expect 50-90%?

Design:
  - K declines from 600 to 200 over 40 cycles
  - Attack: 0.375×, Conversion: 2.0×
  - 20 seeds
"""
...
CYCLE_ID = "C385"
CYCLE_NAME = "Med-Low Attack 2x Conv"
MODE = "MLOW_ATK_2XCONV"
...
# 0.375× attack + 2.0× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.001125, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 0.6},
    {"attack": 0.001875, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 0.5},
    {"attack": 0.003, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.4},
    {"attack": 0.0045, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.3},
    {"attack": 0.005625, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.24},
    {"attack": 0.00675, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.2}
]

SEEDS = list(range(5680, 5700))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run medium-low attack + 2.0× conversion experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c385_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 385: MEDIUM-LOW ATTACK + 2.0× CONVERSION - 40 CYCLES")
    ...
    print(f"Attack: 0.375×, Conversion: 2.0×")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c385_med_low_attack_2x_conv_results.json"
