#!/usr/bin/env python3
"""
CYCLE 1714: WHY IS N=28 THE MINIMUM?
Investigate what makes N=28 worse than adjacent values.
"""
...
def run_detailed_analysis(seed, n_initial):
    """Get detailed metrics for each N."""
...
def main():
    print(f"CYCLE 1714: N=28 Analysis | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 70)
    print("Goal: Why is N=28 the minimum?")
    print("=" * 70)

    seeds = list(range(1714001, 1714031))  # 30 seeds
    n_values = [26, 27, 28, 29, 30, 31, 32]

    print("\n" + "=" * 70)
    print("DETAILED METRICS BY N")
    print("=" * 70)
    print(f"\n{'N':>4} | {'D0→D1':>7} | {'D1→D2':>7} | {'Decomp':>7} | {'Ratio':>6} | {'Coex':>6}")
    print("-" * 50)

    for n in n_values:
        results = [run_detailed_analysis(seed, n) for seed in seeds]
        ...
        print(f"{n:4d} | {avg_d0d1:7.1f} | {avg_d1d2:7.1f} | {avg_decomp:7.1f} | {avg_ratio:6.2f} | {coexist:5.0f}%")
