#!/usr/bin/env python3
"""
CYCLE 549: 1.0× ATTACK AT 1.5× CONVERSION
...
Purpose:
  Test upper attack boundary at crossover point.
"""
...
SEEDS = list(range(8961, 8981))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c549_seed{seed}.db", n_populations=n_levels, mode="10X_15XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 549: 1.0× + 1.5× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c549_10x_at_15x_conv_results.json", 'w') as f:
        json.dump({"cycle_id": CYCLE_ID, "attack_multiplier": 1.0, "conversion_multiplier": 1.5, "coexistence_rate": coexist_count/len(results), "results": results}, f, indent=2)
