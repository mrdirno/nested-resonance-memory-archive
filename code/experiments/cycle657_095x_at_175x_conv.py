#!/usr/bin/env python3
"""
CYCLE 657: 0.95× ATTACK AT 1.75× CONVERSION
...
SEEDS = list(range(11121, 11141))


def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c657_seed{seed}.db", n_populations=n_levels, mode="095X_175XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 657: 0.95× + 1.75× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c657_095x_at_175x_conv_results.json", 'w') as f:
