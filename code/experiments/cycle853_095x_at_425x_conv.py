#!/usr/bin/env python3
"""
CYCLE 853: 0.95× ATTACK AT 4.25× CONVERSION
"""
...
BASE_PARAMS = [
    ...
    {"attack": 0.00285, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 1.275},
    {"attack": 0.00475, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 1.0625},
    {"attack": 0.0076, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 0.85},
    {"attack": 0.0114, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.6375},
    {"attack": 0.01425, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.51},
    {"attack": 0.0171, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.425}
]
SEEDS = list(range(15041, 15061))

def run_experiment(seed):
    n_levels = 7
    reality = RealityInterface(db_path=f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c853_seed{seed}.db", n_populations=n_levels, mode="095X_425XCONV")
    np.random.seed(seed)
...
def main():
    print(f"CYCLE 853: 0.95× + 4.25× conv | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    results = []
    ...
    with open("/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c853_095x_at_425x_conv_results.json", 'w') as f:
        json.dump({"cycle_id": CYCLE_ID, "attack_multiplier": 0.95, "conversion_multiplier": 4.25, "coexistence_rate": coexist_count/len(results), "results": results}, f, indent=2)
