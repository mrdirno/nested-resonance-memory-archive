#!/usr/bin/env python3
"""
CYCLE 180: H1+H5 FACTORIAL EXPERIMENT - Energy Pooling × Multi-Generational Recovery

Purpose: Test synergistic effects of combining H1 (Energy Pooling) with H5 (Multi-Generational Recovery)
...
Synergy Prediction:
  Combined H1+H5 should produce super-additive effects:
  ...
  - Predicted: Effect(H1+H5) > Effect(H1) + Effect(H5)

Experimental Design (2×2 Factorial):
  4 Conditions:
    1. BASELINE: Neither H1 nor H5 (no pooling, no multi-generational coordination)
    2. H1-only: Energy pooling enabled (α=0.10), no temporal coordination
    3. H5-only: No energy pooling, multi-generational coordination (offset=300)
    4. H1+H5: Both mechanisms active (pooling α=0.10, offset=300)
...
Statistical Analysis:
  ...
  - Main effect of H1, Main effect of H5, Interaction (H1×H5)
  - Post-hoc: Tukey HSD pairwise comparisons (Bonferroni α=0.0083)
  - Effect sizes: Cohen's d for all pairwise comparisons
  - Synergy Index: SI = [Effect(H1+H5) - (Effect(H1) + Effect(H5))] / Effect(BASELINE)
...
Cycle: 180 (Paper 3 experimental battery, combination 3 of 6)
...
# Experimental parameters (C180: H1+H5 factorial design)
...
class FactorialCondition:
    ...
    def __init__(self, name: str, pooling_enabled: bool, multigenerational_enabled: bool):
        """
        Initialize factorial condition.

        Args:
            name: Condition name (BASELINE, H1-only, H5-only, H1+H5)
    ...
    def __repr__(self):
        return f"FactorialCondition({self.name}, H1={self.pooling_enabled}, H5={self.multigenerational_enabled})"
...
def run_single_experiment(
    ...
    """
    Run single factorial experiment (H1×H5).

    Args:
        condition: Factorial condition (BASELINE, H1-only, H5-only, H1+H5)
...
def main():
    """Run C180 H1×H5 factorial experiment."""
    print("=" * 80)
    print("CYCLE 180: H1×H5 FACTORIAL EXPERIMENT - Energy Pooling × Multi-Generational Recovery")
    print("=" * 80)
    print()
    print("Purpose: Test synergistic effects of H1+H5 combination")
    ...
    print("  2. H1-only: Energy pooling (α=0.10), no multi-generational coordination")
    print("  3. H5-only: Multi-generational coordination (offset=300), no pooling")
    print("  4. H1+H5: Both mechanisms active")
    print()
    ...
    conditions = [
        FactorialCondition("BASELINE", pooling_enabled=False, multigenerational_enabled=False),
        FactorialCondition("H1-only", pooling_enabled=True, multigenerational_enabled=False),
        FactorialCondition("H5-only", pooling_enabled=False, multigenerational_enabled=True),
        FactorialCondition("H1+H5", pooling_enabled=True, multigenerational_enabled=True)
    ]
    ...
    results_file = output_dir / "cycle180_h1h5_factorial_results.json"

    output = {
        'metadata': {
            'cycle': '180',
            'experiment': 'H1×H5 Factorial - Energy Pooling × Multi-Generational Recovery',
