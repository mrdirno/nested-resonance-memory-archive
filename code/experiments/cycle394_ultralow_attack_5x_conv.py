#!/usr/bin/env python3
"""
CYCLE 394: ULTRA-LOW ATTACK + 5.0× CONVERSION - 40 CYCLES
...
Purpose:
  Test if 5.0× conversion enables 0.25× attack to achieve 100% survival.

Background:
  Linear scaling pattern discovered:
  - C381: 0.25× attack + 2.0× conv - 15%
  - C383: 0.25× attack + 2.5× conv - 40%
  - C386: 0.25× attack + 3.0× conv - 65%
  - C393: 0.25× attack + 4.0× conv - 90%
  - C394: 0.25× attack + 5.0× conv - expect 100%?

Design:
  - K declines from 600 to 200 over 40 cycles
  - Attack: 0.25×, Conversion: 5.0×
  - 20 seeds
"""
...
CYCLE_ID = "C394"
CYCLE_NAME = "Ultra-Low Attack 5x Conv"
MODE = "ULOW_ATK_5XCONV"
...
# 0.25× attack + 5.0× conversion
BASE_PARAMS = [
    {"f": 0.1, "e_con": 0.2, "e_rech": 0.4},
    {"attack": 0.00075, "h": 0.02, "e_con": 0.3, "e_gain": 0.5, "conv": 1.5},
    {"attack": 0.00125, "h": 0.03, "e_con": 0.4, "e_gain": 0.6, "conv": 1.25},
    {"attack": 0.002, "h": 0.04, "e_con": 0.5, "e_gain": 0.8, "conv": 1.0},
    {"attack": 0.003, "h": 0.05, "e_con": 0.6, "e_gain": 1.0, "conv": 0.75},
    {"attack": 0.00375, "h": 0.06, "e_con": 0.7, "e_gain": 1.2, "conv": 0.6},
    {"attack": 0.0045, "h": 0.07, "e_con": 0.8, "e_gain": 1.4, "conv": 0.5}
]

SEEDS = list(range(5860, 5880))


def run_experiment(seed: int, cycles: int) -> dict:
    """Run ultra-low attack + 5.0× conversion experiment."""

    n_levels = 7

    db_path = f"/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c394_seed{seed}.db"
...
def main():
    print("=" * 80)
    print(f"CYCLE 394: ULTRA-LOW ATTACK + 5.0× CONVERSION - 40 CYCLES")
    ...
    print(f"Attack: 0.25×, Conversion: 5.0×")
    print("=" * 80)
    print()
    ...
    output_path = "/Volumes/dual/DUALITY-ZERO-V2/experiments/results/c394_ultralow_attack_5x_conv_results.json"
